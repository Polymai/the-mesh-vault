  ...
  <div class="key-custody-map">
    ...
    <div class="process-output-stack">
      <article class="process-output process-output--control">
        <span>Connected mesh participants</span><strong>Public and protected control data</strong>
      ...
      <article class="process-output process-output--share">
        ...
        <p>The capability stays in the local/mesh control plane and is opened by a 256-bit recipient secret kept in the URL fragment.</p>
...
export const coldStartProcessDiagram = `<figure class="architecture-process architecture-process--cold-start" aria-labelledby="cold-start-title">
  <header class="architecture-process__head">
    <div><span>First contact</span><h3 id="cold-start-title">A new device enters through Patient Zero once</h3></div>
    <p>Supabase is a temporary introduction channel, not the storage or control plane.</p>
  </header>
  <div class="process-step-grid process-step-grid--recovery">
    <article class="process-step process-step--key"><b>01</b><span>Pin trust</span><strong>Immutable genesis key</strong><p>The deployed app contains one Ed25519 genesis public key. It anchors a verifiable public rotation chain; no private authority key is shipped.</p></article>
    <article class="process-step process-step--network"><b>02</b><span>Open once</span><strong>Temporary anonymous session</strong><p>The new profile opens a bounded Supabase session, loads the public authority chain and publishes a temporary public node description.</p></article>
    <article class="process-step process-step--manifest"><b>03</b><span>Find one node</span><strong>Verified current responder</strong><p>A targeted database function may return only the current responder named by the chain after every signature from genesis has verified—not a general peer list.</p></article>
    <article class="process-step process-step--network"><b>04</b><span>Negotiate</span><strong>Recipient-scoped WebRTC signaling</strong><p>SDP, ICE candidates and the answer pass through a temporary Supabase Realtime channel until the direct connection forms.</p></article>
    <article class="process-step process-step--crypto"><b>05</b><span>Verify</span><strong>Authenticated mesh peer</strong><p>The client accepts the route only after the WebRTC peer proves the current responder identity from the verified authority chain.</p></article>
    <article class="process-step process-step--network"><b>06</b><span>Close the gate</span><strong>Mesh-only operation</strong><p>The temporary row is deleted best-effort, the local anonymous session is cleared and Supabase stays closed for the rest of this running session. A later disconnected cold start may briefly use the same bounded gate again.</p></article>
  </div>
  <div class="verification-gate">
    <span><b>Supabase can observe during first contact</b>IP and request metadata, anonymous ID, the public signed authority chain, temporary public node details, SDP/ICE and timing.</span>
    <span class="is-failed"><b>Supabase does not receive</b>Mesh Key, private keys, files, shards, manifests, placements, repair state or the continuing mesh directory.</span>
  </div>
  <figcaption>Patient Zero is the sole runtime exception: it keeps one minimal Supabase listener and a sparse presence heartbeat so completely new profiles can reach it. Public authority rotations are retained as an append-only signed chain; private authority and recovery keys never enter Supabase.</figcaption>
</figure>`;
...
export const uploadProtectionProcessDiagram = `<figure class="architecture-process architecture-process--upload" aria-labelledby="upload-protection-title">
  ...
  <div class="process-destination-row">
    ...
    <span><b>Mesh control plane</b>Signed or encrypted manifests, hashes, placements and repair records exchanged only with connected peers</span>
  </div>
...
export const nativeBackgroundProcessDiagram = `<figure class="architecture-process architecture-process--native" aria-labelledby="native-background-title">
  ...
  <div class="process-step-grid process-step-grid--native">
    ...
    <article class="process-step"><b>02</b><span>Narrow bridge</span><strong>Public configuration only</strong><p>The bridge sends Vault ID, the Patient Zero genesis and verified responder keys, temporary first-contact configuration, node label, coarse location and quotas. Secret-looking fields are rejected.</p></article>
    ...
    <article class="process-step process-step--manifest"><b>04</b><span>Android protection</span><strong>Private app storage + Keystore</strong><p>Shard bytes stay in the app sandbox. The native device seed is protected with Android Keystore; the temporary first-contact token is erased after the first verified peer.</p></article>
  </div>
  <div class="verification-gate">
