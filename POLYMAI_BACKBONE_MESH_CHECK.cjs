...
(async () => {
  ...
  assert.match(node, /if \(!supabaseAutomaticFallbackEnabled\(\)\) return;/);
  ...
  console.log("PASS Backbone mesh: automatic control participation, pinned WSS identity, disabled automatic Supabase fallback, honest host/disk domains and keyless Android participation. No network requests executed.");
})().catch((error) => {
