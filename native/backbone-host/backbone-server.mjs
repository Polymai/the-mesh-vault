...
export function createBackboneServer(config, layout, identityPath) {
  ...
  async function managementRequest(request, response) {
    ...
    if (request.method === "GET" && url.pathname === `${MANAGEMENT_PATH}/status`) {
      const aggregate = store.aggregateStatus();
      const remotelyReachable = /^wss:\/\//i.test(String(config.publicUrl || ""));
      managementResponse(response, 200, {
        ...
        remotelyReachable, publicUrl: remotelyReachable ? config.publicUrl : null,
        advertisedCapacityBytes: remotelyReachable ? aggregate.capacityBytes : 0,
        storedBytes: aggregate.usedBytes,
        instances: store.instanceStatuses(),
      }, origin);
      return;
    }
    if (request.method === "POST" && url.pathname === `${MANAGEMENT_PATH}/shutdown`) {
      await readJson(request);
      managementResponse(response, 202, { ok: true, stopping: true }, origin);
      setTimeout(() => shutdownHandler?.(), 50);
      return;
    }
  ...
  return {
    ...
    setShutdownHandler(handler) { shutdownHandler = typeof handler === "function" ? handler : null; },
