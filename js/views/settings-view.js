...
function installPresentation(install = {}) {
  ...
  if (install.platform === "ios") return {
    label: "Browser menu",
    tone: "neutral",
    detail: "In Safari, choose Share, then Add to Home Screen.",
    button: "",
  };
  return {
    label: "Browser menu",
...
export function renderSettingsView(state) {
  ...
  <div class="settings-grid">
    <section class="panel settings-section pwa-install-panel"><div class="settings-section-head"><h2>Install or share TheMeshVault</h2></div><div class="pwa-install-row"><span class="pwa-install-icon" aria-hidden="true"><svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2.5" width="14" height="19" rx="3"/><path d="M9 6.5h6M12 16v-5m0 5-2-2m2 2 2-2"/><circle cx="12" cy="18.5" r=".5"/></svg></span><div><strong>${esc(install.label)}</strong><span>${esc(install.detail)}</span></div>${install.button}</div><div class="pwa-share-row"><a class="pwa-public-link" href="${esc(publicUrl)}" target="_blank" rel="noopener" aria-label="Open the public TheMeshVault link">${esc(publicUrl)}</a><div class="pwa-share-actions"><button class="button button--ghost" type="button" data-share-app>Share TheMeshVault</button><button class="button button--ghost" type="button" data-copy-app-link>Copy link</button></div></div><p class="form-status" data-install-status role="status"></p></section>
    <section class="panel settings-section"><div class="settings-section-head"><h2>Storage you share</h2></div><form data-capacity-form><label class="range-label"><span><strong>Maximum on this browser</strong><output>${gb(state.node.capacityBytes)} GB</output></span><input type="range" name="capacity" min="0" max="10" step="0.25" value="${gb(state.node.capacityBytes)}"></label><button class="button" type="submit">Save limit</button><p class="form-status" role="status"></p></form></section>
    ${state.identity ? `<section class="panel settings-section"><div class="settings-section-head"><h2>File protection</h2></div><form data-resilience-form><label><span>16-node profile</span><select name="resilienceClass"><option value="standard" ${state.erasureClass !== "high" ? "selected" : ""}>Balanced · survives 4 offline</option><option value="high" ${state.erasureClass === "high" ? "selected" : ""}>Extra · survives 6 offline</option></select></label><details class="settings-technical"><summary>Technical details</summary><p>Balanced uses 12+4. Extra uses 10+6. Smaller meshes adapt automatically from 3+2 through 7+3.</p></details><button class="button" type="submit">Save protection</button><p class="form-status" role="status"></p></form></section>` : ""}
