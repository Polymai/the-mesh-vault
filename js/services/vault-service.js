...
export async function getVaultProfile(vaultId, identity = null) {
  ...
  return { ...local.profile, revision: local.revision, synced: false, syncStatus: "local" };
}
...
export async function setVaultProfile(identity, profile = {}) {
  ...
  const value = cleanProfile(profile);
  const local = await readLocalProfile(vaultId);
  return publishProfile(identity, value, local.revision).catch(async () => {
...
export async function ensureVaultRegistration(identity) {
  ...
  // Registration is deliberately local in v3. Supabase is a one-shot
  // rendezvous channel, not a vault registry or metadata authority.
  const row = {
