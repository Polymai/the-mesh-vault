# TheMeshVault Android availability
...
The Android build uses the live TheMeshVault website for its user interface and adds a native foreground service that can keep an ordinary storage participant available after the WebView is hidden or the screen turns off. This is a platform capability, not a separate user-visible node role. The native service is not a second cryptographic vault.
...
The native worker creates a separate random Ed25519 device identity. Its identity seed is encrypted with AES-GCM under an app-specific key held by Android Keystore. A temporary anonymous Supabase credential may exist only while this native profile performs first contact and is erased after a verified mesh peer is established. This does not turn the WebView's IndexedDB root into hardware-backed storage.
...
The current Android native service follows the same bounded per-lifecycle bootstrap contract as the browser client. A disconnected native profile may create a temporary anonymous Supabase session and node row, discover only the deployment-pinned Patient Zero identity and use recipient-scoped Realtime signaling for a WebRTC connection. After the peer proves that pinned identity, the worker marks bootstrap complete, deletes the temporary row best-effort, erases the temporary credential and closes the hosted path for the rest of that lifecycle. Direct WebRTC and authenticated mesh peers carry subsequent shard and control traffic.

The browser/WebView path uses the same lifecycle boundary. A disconnected local profile can use Supabase for one bounded introduction to Patient Zero; after a verified handoff it uses cached peers, connected participants and invites and cannot reopen Supabase for discovery, files, placements, repair or Live Mesh during that lifecycle. A later disconnected cold start may use a new bounded window. Patient Zero is the sole intentional hosted exception and keeps only the minimal listener and sparse heartbeat required for future first contacts.

## Lifecycle

The user must explicitly turn on **Keep helping when closed** under **This device** in Settings. Android then starts a foreground service and shows a persistent notification. Turning it off records the opt-out and stops the service. If the user left it enabled, the boot receiver may request restart after the device reboots. Transient authentication, network, signaling or Realtime failures keep the service alive in a bounded reconnect loop instead of terminating the process.
...
3. Open Settings and turn on **Keep helping when closed**.
...
10. Inspect Supabase and confirm that the temporary anonymous native session/node row exists only during first contact, contains no root, Mesh Key or shard payload, and is removed/closed after the first verified peer. Confirm zero ordinary steady-state Supabase calls afterward.
