...
import { appTable, appConfig, currentAccessToken, ensureAnonymousSession, supabase, supabaseFetch } from "./services/supabase.js";
...
async function refreshPrivateData() {
  ...
  const [files, pendingDeletions, folders, network, transfers, log, myStorageNodeIds, vaultStorage, vaultProfile] = await Promise.all([
    listFiles(vaultId), listPendingFileDeletions(vaultId).catch(() => state.pendingDeletions || []), listFolders(vaultId).catch(() => state.folders || []), loadNetworkOverview({ includeNodes: state.route === "mesh" }).catch(() => emptyNetwork),
...
const actions = {
  ...
  settings: {
    ...
    async anchor({ enabled, nodeName, cacheLimitBytes }) {
      ...
      announce(enabled
        ? nativeActive ? "Anchor mode is active in the Android background node." : "Anchor mode is active while this tab remains awake."
        : "Anchor mode stopped.", enabled ? "success" : "info", 7000);
    },
