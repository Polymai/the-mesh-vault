...
async function locateLocalBackbone(launch) {
  ...
  const ports = Array.from({ length: BACKBONE_PORT_MAX - BACKBONE_PORT_MIN + 1 }, (_, index) => BACKBONE_PORT_MIN + index)
    .filter((port) => port !== hint)
    .sort((left, right) => Math.abs(left - hint) - Math.abs(right - hint));
  // Browsers limit parallel loopback requests. Small, nearest-first batches find the
  // usual requested-port-plus-one case without queueing one hundred failed fetches.
  for (let offset = 0; offset < ports.length; offset += 8) {
    const results = await Promise.all(ports.slice(offset, offset + 8).map(async (port) => {
      try { return { port, status: await localBackboneRequest(port, "/status", {}, 550) }; } catch { return null; }
    }));
