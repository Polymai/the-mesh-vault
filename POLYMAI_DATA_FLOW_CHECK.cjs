...
async function main() {
  ...
  expect(log.describeSupabaseRequest("https://project.supabase.co/rest/v1/rpc/discover_bootstrap_gateways", "POST", "{}") === "Find Patient Zero", "Supabase first-contact discovery is not labelled clearly.");
  ...
  expect(/supabase:\s*"Supabase · first contact"/.test(panelSource), "The summary must identify Supabase explicitly instead of hiding it behind a purpose label.");
  expect(/app-observed payload/.test(panelSource) && /Browser and OS transport overhead is outside/.test(panelSource), "The panel does not state the honest boundary of web traffic observability.");
  ...
  console.log("Verified explicit aggregated Peer, Backbone and Supabase first-contact observability without extra measurement requests or mesh rerenders.");
}
