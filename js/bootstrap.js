...
async function startDevice(deviceIdentity, vaultId) {
  ...
  try {
    ...
    const [iceResult, browserHint, capacityBytes, nativeStatus] = await Promise.all([
...
async function initialize() {
  ...
  try {
    ...
    if (storageNodeOnly) {
      ...
      catch (error) { announce("Coordination is offline. Opening cached node state.", "warning", 10000); }
      const deviceIdentity = await loadOrCreateDeviceIdentity();
