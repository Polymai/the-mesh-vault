...
async function acquireShards(segment, connectedPeers, { signal, onProgress } = {}) {
  ...
  const fetchCandidate = async ({ index, shardHash, preferredNodeId }) => {
    ...
    const orderedPeers = preferredNodeId
      ...
      : availablePeers;
    const { bytes, source } = await findFragment(shardHash, { connectedPeers: orderedPeers, propagate: propagateFragmentRequest, connectToNode, signal });
    if (!bytes) return null;
    if (source && source !== "local") await announceHeldFragment(shardHash, currentNode()?.id).catch(() => {});
    acquired += 1;
    onProgress?.({ acquired: Math.min(acquired, required), required });
