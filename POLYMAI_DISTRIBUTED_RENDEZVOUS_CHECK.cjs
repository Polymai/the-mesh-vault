...
const authorityChain = read("js/network/patient-zero-authority-chain.js");
...
expect(/bootstrapCompletedThisLifecycle/.test(policy), "Successful first contact must close the gate for the active app lifecycle.");
expect(!/localStorage|sessionStorage/.test(policy), "A stale persisted cutoff must not prevent a disconnected cold start from finding Patient Zero.");
expect(/policyConfigured && activeNodeId === nextNodeId && patientZeroListener === nextPatientZero/.test(policy)
  && /bootstrapCompletedThisLifecycle = false/.test(policy), "A new node may reset first contact, while repeated configuration of the same browser node must preserve its one-shot lifecycle state.");
expect(/fallbackAttemptedThisLifecycle/.test(policy)
  && /if \(!force && fallbackAttemptedThisLifecycle\) return false/.test(policy), "An ordinary app lifecycle must never reopen first contact after its one attempt.");
...
expect(/const needsRealtime = supabaseFallbackOpened &&/.test(nodeService), "Rendering or coordination refresh must not open Realtime before the explicit first-contact transaction.");
expect(/let deviceStartPromise = null/.test(bootstrap)
  && /if \(deviceStartPromise && deviceStartKey === key\) return deviceStartPromise/.test(bootstrap), "Overlapping PWA/native lifecycle events must share one node activation and one first-contact window.");
...
expect(!/scheduleOrdinaryBootstrapRetry|bootstrapReconnect|bootstrapRetryCount/.test(nodeService), "Cold start must not poll Supabase after an empty Patient Zero result.");
expect(/const writeBackbone = activeNode\._supabaseRegistered[\s\S]{0,160}&& patientZeroListener/.test(nodeService)
  && !/const writeBackbone[\s\S]{0,400}supabaseFallbackOpened/.test(nodeService), "Only Patient Zero may send scheduled hosted heartbeats.");
expect(/withPatientZeroAuthorityRead\(async \(\) =>/.test(authorityChain)
  && /await ensureAnonymousSession\(\)/.test(authorityChain), "The signed public authority chain must load inside its narrow authenticated scope before runtime role selection.");
expect(/isCurrentPatientZeroResponder\(devicePublicKey\)/.test(patientZeroOperator)
  && /configuredAuthorityKeys\(\)\.includes\(devicePublicKey\)/.test(patientZeroOperator), "The verified current responder, with genesis fallback before the first rotation, must deterministically activate Patient Zero without a fragile local UI flag.");
expect(/markSupabaseBootstrapComplete/.test(nodeService), "Ordinary nodes must close the post-bootstrap gate for the active lifecycle.");
...
expect(/operatorSetup = patientZeroOperatorEnabled\(\) \|\| publicConfigurationPublished/.test(anchorView), "A pinned Patient Zero profile must recover its operator panel after a local UI flag is cleared.");
expect(/pinnedPatientZero[\s\S]{0,180}patientZeroOperatorEnabled\(runtimeNode\.device_public_key\)/.test(settingsView), "Settings must recognize the pinned Patient Zero identity without relying only on localStorage.");
expect(/Authority verified/.test(anchorView) && /First-contact listener/.test(anchorView), "Patient Zero UI must distinguish verified authority from live listener status.");
...
expect(/retired_test_genesis/.test(schema)
  && /active_authority_public_key=current_genesis/.test(schema)
  && /responder_device_public_key=current_genesis/.test(schema), "Provisioning must migrate the retired pre-release Mainframe key to the public v3 genesis key without overwriting a rotated authority chain.");
expect(/first contact/i.test(security) && /Patient Zero/.test(security), "Public documentation must explain the bounded gate and Patient Zero exception.");
