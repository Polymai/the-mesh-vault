...
function validateFiles() {
  ...
  assert(workerRegistration.includes('updateViaCache: "none"'), "Installed clients must bypass an old HTTP-cached service-worker script.");
  assert(workerRegistration.includes("RELEASE_CHECK_INTERVAL_MS") && workerRegistration.includes("release-check=1"), "Long-running installed clients must periodically compare their visible build with the published build.");
  assert(workerRegistration.includes("publishedBuild > runningBuild"), "A stale installed shell must reload when a newer published runtime is observed.");
  assert(worker.includes("meshvault:skip-waiting"), "The application worker must accept an explicit activation request.");
  assert(worker.includes("replacedShell") && worker.includes("client.navigate(client.url)"), "A newly activated worker must refresh clients that are too old to run the current update coordinator.");
  assert(workerRegistration.includes('window.addEventListener("focus"') && workerRegistration.includes('window.addEventListener("pageshow"'), "Installed clients must check for updates when their app window is resumed.");
  ...
  const release = runtime.match(/release:\s*Object\.freeze\(\{\s*version:\s*"(\d+\.\d+)",\s*build:\s*(\d+),\s*label:\s*"v(\d+\.\d+)\.(\d+)"/);
  assert(release && release[1] === release[3] && release[2] === release[4], "Runtime configuration must expose one internally consistent visible release identifier.");
