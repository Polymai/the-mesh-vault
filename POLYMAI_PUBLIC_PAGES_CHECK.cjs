  ...
  try {
    ...
    for (const route of ["security", "terms", "privacy"]) {
      ...
      if (route === "security") {
        ...
        assert(simpleOverview.includes("Files, shards, manifests and placements are never written to Supabase."), "Security overview does not explain Supabase's simple data boundary.");
        ...
        assert(backbone.includes("one bounded first-contact phase only"), "Security does not state Supabase's bounded first-contact role.");
        assert(backbone.includes("Supabase is never queried for secret material"), "Security does not disclose the protocol-v3 Mesh Key boundary.");
        assert(backbone.includes("Patient Zero"), "Security omits the verified Patient Zero cold-start boundary.");
        ...
        assert.equal(await page.locator(".architecture-process").count(), 5, "Security must render the key, cold-start, Android boundary, upload and recovery process diagrams.");
        ...
        await page.locator(".architecture-process--cold-start").screenshot({ path: path.join(screenshots, "public-security-cold-start-desktop.png") });
      ...
      if (route === "privacy") {
        ...
        assert(coordination.includes("Supabase is a narrow first-contact gate"), "Privacy no longer explains the hosted first-contact provider.");
    ...
    assert.equal(await mobile.locator(".architecture-process").count(), 5, "Mobile Security lost one or more process diagrams.");
    for (const kind of ["keys", "cold-start", "native", "upload", "recovery"]) {
