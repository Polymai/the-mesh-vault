...
const DB_NAME = `${prefix}vault-index-v3`;
const STORE = "indexes";
...
export function normalizeVaultIndexPayload(payload = {}) {
  return {
    ...
    segments: cleanRows(payload.segments),
    versionSegments: cleanRows(payload.versionSegments),
...
export async function buildVaultIndexEnvelope(identity, payload, { revision, previousIndexHash = null } = {}) {
  ...
  const encryptedPayload = await encryptJson(normalized, `themeshvault/v3/vault-index:${identity.vaultId}:${nextRevision}`);
  const core = {
    format: FORMAT,
...
export async function openVaultIndexEnvelope(envelope, identity) {
  ...
  try {
    const payload = normalizeVaultIndexPayload(await decryptJson(envelope.encryptedPayload, `themeshvault/v3/vault-index:${identity.vaultId}:${envelope.revision}`));
