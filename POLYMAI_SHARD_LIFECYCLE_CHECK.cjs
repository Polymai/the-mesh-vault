...
const repairSendIndex = recoveryCoordinatorSource.indexOf("await peer.protocol.sendShard", reservationIndex);
const repairVerifyIndex = recoveryCoordinatorSource.indexOf('transitionShardPlacement(job.vault_id, reservation.id, "verified")', repairSendIndex);
...
const sharedLookupIndex = recoveryCoordinatorSource.indexOf("await hashHasOtherPlacementOnNode");
const sharedFailureIndex = recoveryCoordinatorSource.indexOf("if (sharedLocally === null) continue;", sharedLookupIndex);
const surplusRetireIndex = recoveryCoordinatorSource.indexOf('status: "retiring"', sharedFailureIndex);
assert(sharedLookupIndex >= 0 && sharedFailureIndex > sharedLookupIndex && surplusRetireIndex > sharedFailureIndex,
  "Surplus cleanup must fail closed before changing placement lifecycle metadata.");
