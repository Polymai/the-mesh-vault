...
export function renderAnchorView(state) {
  ...
  const rendezvousMarkup = operatorSetup ? `<section class="panel settings-section patient-zero-panel">
    ...
    <ol class="patient-zero-steps">
      ...
      <li><span>2</span><div><strong>${publicConfigurationPublished ? "App configuration published" : "Publish the app configuration"}</strong><small>${publicConfigurationPublished ? "The deployed app already pins this profile's public key." : "Copy the configuration into Polymai, then publish the app."}</small></div></li>
