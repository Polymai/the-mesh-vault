...
export class BackboneStore {
  ...
  advertisedInstances() {
    return this.layout.instances.filter((entry) => this.instanceState(entry.id)?.enabled !== false).map((entry) => ({
      nodeId: entry.id, nodeName: entry.label || entry.id, protocolVersion: 3,
      capabilities: ["mesh-control-lite-v3", "mesh-backbone-v3", "backbone-storage-v3"],
      devicePublicKey: this.layout.backbonePublicKey || null,
      failureDomainId: entry.failureDomainId, storageDeviceId: entry.storageDeviceId,
      capacityBytes: entry.quotaBytes, usedBytes: this.usedBytes(entry),
  ...
  startShard(instanceId, transferId, shardHash, expectedSize, expectedHash) {
    const instance = this.instance(instanceId); if (!instance) throw new Error("Unknown Backbone instance.");
    if (this.instanceState(instanceId)?.enabled === false) throw new Error("Backbone instance is stopped.");
  ...
  getShard(instanceId, hash) { const instance = this.instance(instanceId); if (!instance) return null; try { return readFileSync(this.shardPath(instance, hash)); } catch { return null; } }
  deleteShard(instanceId, hash) { const instance = this.instance(instanceId); if (!instance) return false; const path = this.shardPath(instance, hash); if (!existsSync(path)) return true; rmSync(path); this.writeIndex(instance); return true; }
  putControl(object) {
