...
export function receiveRelayedSignal(signal, sourcePeerId) {
  ...
  // A connected protocol peer is already authenticated. While forwarding an
  // introduction it preserves the original fromNodeId, so requiring that ID
  // to match the forwarding transport peer rejects Mainframe -> leaf signals.
  if (!signal || !sourceIsConnected || !signal.fromNodeId || !signal.toNodeId || Number(signal.relayHops || 0) > 3) return false;
