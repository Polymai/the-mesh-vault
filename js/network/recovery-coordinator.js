...
import { getShard } from "../storage/fragment-store.js";
...
import { recordShardPlacement, transitionShardPlacement, auditAllVersionResilience, queueShardRepair, refreshPhysicalStorageTotals, loadFilePlacementStateForSegment } from "../services/segment-service.js";
import { upgradeOneCodingProfile, cleanupRetiredCodingGeneration } from "./profile-upgrade-coordinator.js";
import { createFilePlacementContext, createPlacementContext, rankPlacementCandidates, recordFilePlacementChoice } from "./placement-engine.js";
import { ACTIVE_PLACEMENT_STATUSES, planReplicaReconciliation } from "./shard-lifecycle.js";
import {
  acknowledgeLifecycleShardDeletion,
  deleteShardIfUnreferenced,
  retryPendingVaultDeletions,
} from "../services/deletion-service.js";
import {
  claimDistributedRepairLease, listRepairAdvisories, publishRepairCompletion,
} from "./distributed-repair.js";
...
function onRecoverySignal(event) {
  const reason = eventReason(event);
  if (reason) requestRecoveryCycle(reason);
}
...
async function syncAnchorRepairAdvisories(node) {
  ...
  for (const request of rows) {
    ...
    const job = await queueShardRepair(request.vaultId, request.shardId, request.targetNodeId || null, {
      announce: false,
...
async function recoveryCycle(reason = "maintenance") {
  ...
  const repairRequested = fullMaintenance || ["repair-needed", "repair-advisory"].includes(reason);
  let changed = false;
  if (fullMaintenance) {
    const proofResults = await verifyPlacementSamples();
    changed = proofResults.some((result) => result !== null) || changed;
  }
  if (reason === "repair-advisory" || fullMaintenance) {
    changed = (await syncAnchorRepairAdvisories(node)) > 0 || changed;
  }
  if (["deletion-changed", "maintenance", "peer-connected", "online"].includes(reason)) {
    const deletionResult = await retryPendingVaultDeletions(node.vault_id, node.id, connectedProtocols());
    changed = !!deletionResult || changed;
  }
  if (repairRequested) {
    await auditAllVersionResilience(node.vault_id);
    changed = !!(await repairOnce()) || changed;
  }
  if (["maintenance", "peer-connected", "online", "file-changed", "deletion-changed"].includes(reason)) {
    changed = (await reconcileSurplusPlacements()) > 0 || changed;
  }
  if (["maintenance", "profile-changed"].includes(reason)) {
    changed = !!(await cleanupRetiredCodingGeneration()) || changed;
    changed = !!(await upgradeOneCodingProfile()) || changed;
  }
...
async function runRecoveryCycle(reason) {
  ...
  if (waitMs > 0) {
    requestRecoveryCycle(reason, waitMs);
    return false;
  }
  running = true;
  try {
    const completed = await recoveryCycle(reason);
    if (completed) lastCycleAt = Date.now();
    return completed;
  } finally {
    running = false;
  }
...
export async function reconcileSurplusPlacements() {
  ...
  for (const group of groups || []) {
    ...
    for (const action of actions) {
      ...
      if (sharedLocally === null) continue;
      await updatePlacementLifecycle(action.placement.id, { status: "retiring", retire_reason: "duplicate_after_repair" }, { status: "deleting" });
      if (sharedLocally) {
        const { error: deletedError } = await appTable("shard_placements").update({ status: "deleted", deleted_at: new Date().toISOString() }).eq("id", action.placement.id);
        if (deletedError) throw deletedError;
      } else if (action.placement.node_id === node.id) {
        const acknowledged = await acknowledgeLifecycleShardDeletion(node.id, shardHash).catch(() => false);
        if (!acknowledged) continue;
      } else {
        const acknowledged = await peer.protocol.requestDelete(shardHash).catch(() => false);
        if (!acknowledged) continue;
      }
      changed += 1;
    }
...
export async function repairOnce() {
  ...
  let filePlacementState;
  try { filePlacementState = await loadFilePlacementStateForSegment(job.vault_id, group.id); }
  catch { await requeue(job.id); return job; }
  const filePlacementContext = createFilePlacementContext(filePlacementState.placements, { expectedPlacements: filePlacementState.expectedPlacements, candidateCount: (nodes || []).length });
  const placementKey = `${filePlacementState.placementScope}:${group.id}:${generation}:${shard.shard_index}`;
  const ranked = rankPlacementCandidates((nodes || []).map((candidate) => ({ ...candidate, peer: peers.find((entry) => entry.nodeId === candidate.id) })), { context: placementContext, fileContext: filePlacementContext, placementKey, shardBytes: Number(shard.size_bytes || bytes.byteLength) });
  const target = ranked[0];
  const peer = peers.find((candidate) => candidate.nodeId === target?.id); if (!target || !peer) { await requeue(job.id); return job; }
  let reservation = null; let replacementVerified = false;
  try {
    ...
    reservation = await recordShardPlacement({ vaultId: job.vault_id, shardId: shard.id, node: target, role: "durable", replacementForPlacementId: replaced?.id || null, status: "pending" });
    await peer.protocol.sendShard(shard.shard_hash, bytes, shard.shard_hash);
    reservation = await transitionShardPlacement(job.vault_id, reservation.id, "verified");
    replacementVerified = true;
    recordFilePlacementChoice(filePlacementContext, target, bytes.byteLength, `${group.id}:${shard.id}:${target.id}`);
    ...
    if (localCache) {
      await updatePlacementLifecycle(localCache.id, { status: "deleting" }, { status: "deleting" });
      await acknowledgeLifecycleShardDeletion(node.id, shard.shard_hash);
    }
    await appTable("repair_jobs").update({ status: "complete", target_node_id: target.id }).eq("id", job.id);
    await publishRepairCompletion({
      requestId: job.request_id || `supabase:${job.id}`,
      vaultId: job.vault_id,
      shardId: job.shard_id,
    }, node.id, target.id).catch(() => {});
    await refreshPhysicalStorageTotals(job.vault_id, shard.content_segment_id).catch(() => {});
    return job;
  } catch {
    if (reservation && !replacementVerified) {
      try {
        reservation = await transitionShardPlacement(job.vault_id, reservation.id, "deleting");
        await peer.protocol.requestDelete(shard.shard_hash).catch(() => false);
      } catch {
        // Keep the pending/deleting placement as durable cleanup intent; never hide ambiguous remote bytes.
      }
    }
    await requeue(job.id); return job;
  }
}
