...
const ROUTE_LABELS = Object.freeze({ direct: "Peer", anchor: "Backbone", supabase: "Supabase · first contact" });
...
function flowPath(entry) {
  const other = entry.route === "supabase" ? "Supabase first contact" : entry.counterparty || ROUTE_LABELS[entry.route];
...
function renderContent(snapshot) {
  ...
  const events = snapshot.entries.slice(0, 5).map((entry) => `<li class="data-flow-event data-flow-event--${entry.route}">
    ...
    <span><strong>${esc(entry.kind)}${entry.count > 1 ? ` x${entry.count}` : ""}</strong><small>${esc(flowPath(entry))}</small></span>
...
export function renderDataFlowPanel() {
  return `<section class="panel data-flow-panel" data-data-flow-panel>
    <header class="data-flow-panel__head"><div><h2>Data flow</h2><span class="data-flow-live"><i aria-hidden="true"></i> Live</span></div><small>Last 5 min · app-observed payload</small></header>
    <div data-data-flow-content>${renderContent(dataFlowSnapshot())}</div>
    <p>Web session only. Supabase traffic is shown explicitly and is allowed only until this profile verifies its first mesh peer. Browser and OS transport overhead is outside the page's visibility.</p>
  </section>`;
}
