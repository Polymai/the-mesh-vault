    ...
    <div class="auth-card onboarding-card">
      <div class="onboarding-menu" data-onboard-menu>
        ...
        <button class="onboarding-option" type="button" data-onboard-toggle="restore">
          <span><strong>Restore an existing vault</strong><small>Use either form of your Mesh Key</small></span><i aria-hidden="true">→</i>
        </button>
      ...
      <form class="onboarding-panel" data-onboard-panel="restore" hidden>
        ...
        <section class="recovery-method-panel" data-recovery-method="phrase" hidden>
          <label class="onboarding-field"><span>24-word Mesh Key</span><textarea name="phrase" rows="4" autocomplete="off" spellcheck="false" placeholder="Enter all 24 words in order" aria-label="24-word Mesh Key"></textarea></label>
        </section>
...
export function bindOnboardingView(actions) {
  ...
  fileInput?.addEventListener("change", async () => {
    ...
    try {
      ...
      label.textContent = fileJson?.format === "themeshvault-mesh-key" && fileJson?.version === 3 && typeof fileJson?.meshKey === "string"
        ? `${file.name} - ready`
  ...
  restoreForm?.addEventListener("submit", async (event) => {
    ...
    try {
      if (mode === "file" && file && file.size) {
        ...
        await actions.restoreWithFile({ fileJson });
      }
