...
function defaultConfig() {
  return {
    ...
    supabase: { url: "", publishableKey: "", heartbeatMs: 600000 },
...
const config = load(); const layout = { ...validateStorageLayout(config), label: config.label, publicUrl: String(config.publicUrl || "") };
if (command === "check") {
...
let stopping = false;
const stop = async () => { if (stopping) return; stopping = true; await registry.stop(); await service.stop(); process.exit(0); };
service.setShutdownHandler(stop);
