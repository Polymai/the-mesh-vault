...
const MAX_CONTRIBUTION_BYTES = window.__DATA__?.limits?.maxContributionBytes || 2147483648000;
...
const REGISTRATION_RECHECK_MS = 7 * 86400000;
...
function registrationKey(vaultId) { return `coordinationRegistration:${vaultId}`; }

async function cachedRegistration(identity) {
  const cached = await idb((store) => store.get(registrationKey(identity.vaultId))).catch(() => null);
  if (!cached?.value?.row || Date.now() - Number(cached.value.checkedAt || 0) > REGISTRATION_RECHECK_MS) return null;
  try { return verifyRegisteredIdentity(cached.value.row, identity); }
  catch { return null; }
}

async function cacheRegistration(identity, row) {
  await idb((store) => store.put({
    key: registrationKey(identity.vaultId),
    value: { row, checkedAt: Date.now() },
  }), "readwrite").catch(() => {});
}
...
export async function ensureVaultRegistration(identity) {
  ...
  const cached = await cachedRegistration(identity);
  ...
  await cacheRegistration(identity, row);
  return row;
}
