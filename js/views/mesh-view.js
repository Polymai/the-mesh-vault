...
function topologyScopeMarkup({ renderedCount, activeCount, samplePage, samplePages }) {
  ...
  return `<div class="mesh-scope-toolbar">
    <button type="button" data-mesh-scope-step="1" ${index >= TOPOLOGY_SCOPES.length - 1 ? "disabled" : ""} aria-label="Zoom out to more nodes" title="Show more nodes">&minus;</button>
    <span><strong>${label}</strong><small>${renderedCount} of ${activeCount} active</small></span>
    <button type="button" data-mesh-scope-step="-1" ${index <= 0 ? "disabled" : ""} aria-label="Zoom in to fewer nodes" title="Show fewer nodes">+</button>
    ${paging}
  </div>`;
}
