...
(async () => {
  ...
  assert.match(anchorView, /Start Backbone server/);
  assert.doesNotMatch(anchorView, /github\.com\/Polymai\/the-mesh-vault\/tree\/main\/native\/backbone-host/);
  assert.match(anchorView, /Server \$\{number\}/);
  assert.match(anchorView, /themeshvault-backbone-host-v3\.2\.0\.zip/);
  assert.doesNotMatch(anchorView, /Total space to share/);
  assert.doesNotMatch(anchorView, /Public server address/);
  assert.match(serverGuide, /themeshvault-backbone-host-v3\.2\.0\.zip/);
  ...
  assert.match(read("native/backbone-host/quick-start.ps1"), /Sort-Object Free -Descending/);
