...
export async function connectPeer(nodeId, { forceOffer = false } = {}) {
  ...
  if (existing && !["failed", "closed", "disconnected"].includes(existing.pc.connectionState)) {
    // A relayed hello/ICE candidate can create the peer before discovery asks
    // us to connect. The deterministic initiator must still open the channel;
    // otherwise both sides keep a healthy-looking, half-open peer forever.
    if ((forceOffer || context.fromNodeId < nodeId) && !existing.channel) {
