...
Readable file content, the 256-bit root, owner private key and root-level symmetric keys remain inside authorised browser or app-WebView profiles during operation. Storage peers hold encrypted shard payloads without decryption keys. The optional Android foreground service is a keyless storage/Anchor peer with a separate native device identity and private opaque-shard store; its bridge rejects vault-secret fields. Supabase and Anchor control caches may hold public keys plus signed or encrypted coordination objects, including encrypted metadata, signed manifests, shard hashes, placement records and transient signaling, but receive neither file-shard payloads nor root-level vault secrets. An explicitly created share capability is a narrow exception: its encrypted coordination record contains only the segment-key grant for one named file version, and the recipient secret remains in the URL fragment.
...
The coordination plane provides discovery, signaling, metadata indexing, placement status, encrypted caches, repair scheduling and deletion progress. Its records remain subject to signature, hash and client-side decryption checks. Supabase and Anchor control caches carry coordination objects rather than encrypted shard payload bytes.
...
Either form deterministically reconstructs the same cryptographic identity and symmetric hierarchy inside the new browser without fetching secret key material. That local identity restoration must not be confused with remote vault discovery or file recovery. A browser starting with no cached network context still needs Supabase or an already reachable mesh path to locate current manifests, peers and placements. Anchors assist connected clients but do not yet provide a complete replacement for a wholly new cold start.
...
## 17. Supabase fallback and trust boundary

The current release uses Supabase Auth, Postgres, Realtime and an Edge Function as its persistent compatibility and transaction fallback. Browser discovery is mesh-first: local context, connected control participants, configured public-key-pinned WSS Backbones and invite links are tried before Supabase is opened after a short cold-start delay. Two healthy control participants or one Backbone keep the path mesh-first. Backbone nodes still publish sparse hosted compatibility leases, currently on an approximately ten-minute cadence, and durable records are periodically reconciled. This reduces steady-state fan-out but does not remove the persistent Supabase responsibilities below.
...
- delayed discovery and transient WebRTC signaling fallback through app-owned Realtime Broadcast recipient topics protected by random 256-bit tokens;
- encrypted file/folder metadata storage;
- durable fallback storage for the owner-signed encrypted vault index and immutable signed operation entries;
...
- signed encrypted capability publication and revocation;
...
- usable metadata, deduplication or segment keys, except that an explicit share record may carry a recipient-encrypted limited segment-key grant without its fragment secret;
- readable file content;
- encrypted file-shard payload bytes; or
...
### 17.3 Outage and cold-start boundary

An already-open vault may use cached records, ordinary control participants and Backbones for a bounded subset of control and transfer operations during a Supabase interruption. Local catalog edits can queue and later replay idempotently. A new web/PWA client can obtain initial mesh context from a pinned Patient Zero device through several public WebSocket relays. Both endpoints make outbound TLS connections, so Patient Zero may remain behind NAT. This is not general offline operation or proof that every recovery or repair can finish during an outage.

The application pins Patient Zero's Ed25519 public device key, never its private key. Patient Zero dynamically publishes its separate rendezvous transport public key in an Ed25519-signed descriptor. Requests, replies, peer presence and WebRTC signals are NIP-44 encrypted for their recipient. Every node signs its own short-lived presence with its device key, and the directory itself is signed by Patient Zero. A relay can observe the connecting IP, time, transport pseudonyms and ciphertext sizes, but is not given the readable directory, SDP, file metadata, shard bytes, Mesh Key or private device keys.

Entering the 24-word Mesh Key reconstructs identity and keys locally; it does not query a relay or Supabase for secret material. The browser first tries the encrypted Patient Zero directory and reachable mesh, then opens delayed Supabase fallback if no verified route forms. New file ingestion, durable segment/placement writes and the atomic repair-job mutation remain Supabase-backed. The current Android native worker also still uses Supabase for its own first contact and signaling.
...
### 17.4 Replaceable rendezvous transport

The public rendezvous relays are transport services rather than trust anchors. The same encrypted event is published through several independently configured relays. A forged or modified descriptor, directory or peer presence fails signature verification. The deployment can replace relay URLs without changing the vault or Patient Zero identity. The current configuration remains deliberately inactive until at least one public Patient Zero device key is pinned; with no pinned key, clients skip this path and preserve the delayed Supabase fallback.
...
- approximately ten-minute sparse Supabase compatibility reporting for Backbones, with jitter;
...
flowchart TB
    ...
    S[Supabase durable fallback] -. delayed fallback .-> B
...
Backbone selection uses deterministic weighted rendezvous ranking with lease health, failure-domain diversity, reliability, bounded cache headroom and reported connection load. Ordinary participants provide the first distributed path; one healthy Backbone or two participants are enough for mesh-first coordination. A pinned WSS Backbone authenticates server messages with Ed25519 and can bootstrap a new browser. Supabase is opened only after the mesh attempt fails. The seed list may initially be empty, in which case Supabase remains necessary for cold first contact.

The control repository maintains an encrypted owner-signed catalog snapshot and append-only signed local operation queue. It resolves reads in local, connected-participant/Backbone and Supabase-fallback order. Operation IDs make replay idempotent, and mesh and Supabase receipts are tracked separately.

Repair needs are signed, gossiped and cached as expiring Anchor control objects. Candidate repairers publish short signed leases and deterministically elect the earliest valid lease observed for a request before copying work into the hosted queue. Supabase still provides the actual atomic repair-job claim and persistent placement mutation, so this distributed lease is coordination and duplicate suppression rather than an independent repair database.
...
The native service has a separate random Ed25519 device identity, a SQLite index and private app files for opaque encrypted shards. It verifies shard hashes before an atomic local commit and bounds manifest/control records before caching them. The device seed and anonymous Supabase refresh token are encrypted at rest with an app-specific Android Keystore AES-GCM key. Native WebRTC uses the same bounded shard, proof, manifest and control messages as browser nodes. A two-minute partial CPU Wake Lock is acquired only during an active shard transfer; the service does not keep an indefinite Wake Lock.
...
- The 24-word phrase restores cryptographic identity locally. Browser cold start can use a configured pinned WSS Backbone, but Supabase remains necessary when none is reachable and for current durable placement/repair transactions and Android native first contact.
