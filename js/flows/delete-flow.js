...
import { getFileBundle, forgetCachedFile, tombstoneFile } from "../services/metadata-service.js";
import { beginAtomicFileDeletion, getVersionSegments, updatePlacementAuthorization } from "../services/segment-service.js";
...
export async function deleteFile(fileId, { vaultIdentity } = {}) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3 && !deletion; attempt += 1) {
      ...
      for (const item of prepared) await updatePlacementAuthorization(vaultId, item.placementId, item.authorization);
      try {
    ...
    await tombstoneFile(vaultId, fileId, authorizations);
