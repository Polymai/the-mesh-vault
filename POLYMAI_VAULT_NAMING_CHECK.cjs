  ...
  try {
    ...
    const storedPublicName = await page.evaluate(async () => {
      ...
      const db = await new Promise((resolve, reject) => {
        const request = indexedDB.open(`${prefix}device-identity-v3`, 1);
    ...
    const registryCheck = await page.evaluate(async ({ publicNodeName }) => {
      ...
      registry.upsertPeer({ peerId: "naming-check-peer", nodeName: publicNodeName, protocolVersion: 3, trustStatus: "gossiped" });
