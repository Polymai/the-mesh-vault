...
- **recovery file:** carry the encrypted envelope in a JSON file and combine it with the phrase locally.

The current release normally requires Supabase for a cold phrase-only lookup. An already available recovery file does not require that lookup. Anchors can cache recovery control material as an alternative provider, but the current implementation does not yet claim universal Supabase-independent cold discovery.
...
Eligible nodes are ranked lexicographically using measured or explicitly unknown attributes:

1. new country, then new region;
2. new salted network-domain hash;
3. measured availability/reliability;
4. successful transfer and proof quality;
5. lower concentration of the file on that node;
6. lower storage utilisation;
7. higher observed throughput; and
8. lower observed round-trip time.
...
The current release uses Supabase Auth, Postgres, Realtime and an Edge Function as its normal operational backbone.

### 17.1 Supabase currently performs

- anonymous-session creation and RLS scoping;
- vault and device public registration;
- presence, discovery and WebRTC signaling;
...
An already-open vault with cached manifests and established peer connections can continue some recovery and control operations while Supabase is unavailable. Current cold peer discovery, phrase-only recovery lookup, new metadata writes, coordinated repair and deletion normally depend on Supabase.
...
- replication target of two other Anchors;
- 30-second heartbeat;
...
```mermaid
flowchart TB
    S[Supabase normal fast path] --> B[Browser mesh]
...
Future embodiments can make the Anchor layer a fuller distributed control plane using a signed append-only log, CRDT, quorum register, DHT, gossip store or Byzantine-fault-aware consensus. The essential rule is that replicated coordination state remains subordinate to cryptographic vault authority and content verification.
...
- optional service-worker and push wake assistance; and
- a low-light, always-on operating view.

These mechanisms improve observable availability but cannot force a mobile operating system to keep a browser process alive after screen lock or resource pressure.
