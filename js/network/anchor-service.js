...
export const CONTROL_PARTICIPANT_CAPABILITY = "mesh-control-lite-v3";
export const BACKBONE_CAPABILITY = "mesh-backbone-v3";
...
const pendingRelaySignals = new Map();
const RELAY_SIGNAL_TTL_MS = 20_000;
const MAX_PENDING_RELAY_SIGNALS = 128;
...
function controlPeers() {
  return peers().filter((peer) => {
    const capabilities = runtime?.peerInfo?.(peer.nodeId)?.capabilities || [];
    return capabilities.some((value) => [CONTROL_PARTICIPANT_CAPABILITY, BACKBONE_CAPABILITY, ANCHOR_CAPABILITY].includes(value));
  });
}
function signalRelayPeers() {
  // Signalling is what establishes the next direct connection. A newly
  // connected peer may not have delivered its signed capability announcement
  // yet, so requiring that capability here creates a bootstrap deadlock. Keep
  // verified control participants first, then use every other authenticated,
  // open protocol peer as a bounded relay candidate.
  const preferred = controlPeers();
  const preferredIds = new Set(preferred.map((peer) => peer.nodeId));
  return [
    ...preferred,
    ...peers().filter((peer) => peer?.nodeId && peer?.protocol && !preferredIds.has(peer.nodeId)),
  ];
}
function signer() { return runtime?.signer || null; }
function cacheOptions() {
  const automaticLimit = Number(window.__DATA__?.coordination?.participantCacheBytes || 33554432);
  return { maxBytes: settings.enabled ? settings.cacheLimitBytes : automaticLimit };
}
...
async function sendObjectToAnchors(object) {
  let replicas = 0;
  const target = Number(window.__DATA__?.anchor?.replicationTarget || 3);
  const ranked = controlPeers().sort((left, right) => {
    const score = (peer) => {
      const values = runtime?.peerInfo?.(peer.nodeId)?.capabilities || [];
      return values.includes(BACKBONE_CAPABILITY) ? 3 : values.includes(ANCHOR_CAPABILITY) ? 2 : 1;
    };
    return score(right) - score(left);
  });
  const domains = new Set();
  for (const peer of ranked) {
    const info = runtime?.peerInfo?.(peer.nodeId) || {};
    const domain = info.failureDomainId || peer.nodeId;
    if (domains.has(domain)) continue;
    const accepted = await peer.protocol.sendControlObject?.(object).catch(() => false);
    if (accepted) {
      replicas += 1;
      domains.add(domain);
      await markControlReplica(object.objectId, peer.nodeId);
    }
...
export async function answerControlQuery({ kind, lookupKey, limit = 20 } = {}) {
  const bounded = settings.enabled ? Math.min(100, Number(limit) || 20) : Math.min(20, Number(limit) || 20);
  return findControlObjects(String(kind || ""), String(lookupKey || ""), bounded);
}
...
export async function publishManifestReplica(envelope) {
  ...
  let replicas = 0;
  for (const peer of controlPeers().slice(0, window.__DATA__?.anchor?.replicationTarget || 3)) {
...
export async function lookupManifestReplica(manifestId) {
  ...
  for (const peer of signalRelayPeers()) {
...
export function relaySignalThroughAnchors(signal) {
  ...
  let sent = 0;
  // A direct protocol peer is already authenticated by the WebRTC handshake.
  // Do not wait for its separate capability announcement before using it as a
  // signalling hop: that announcement itself can arrive after the peer-list
  // introduction which needs this relay.
  for (const peer of signalRelayPeers()) {
...
function prunePendingRelaySignals(now = Date.now()) {
  for (const [nodeId, entries] of pendingRelaySignals) {
    const current = entries.filter((entry) => entry.expiresAt > now);
    if (current.length) pendingRelaySignals.set(nodeId, current);
    else pendingRelaySignals.delete(nodeId);
  }
}

function queueRelayedSignal(signal) {
  if (!signal?.toNodeId) return false;
  prunePendingRelaySignals();
  const entries = pendingRelaySignals.get(signal.toNodeId) || [];
  if (entries.some((entry) => entry.signal.relayId === signal.relayId)) return true;
  entries.push({ signal, expiresAt: Date.now() + RELAY_SIGNAL_TTL_MS });
  pendingRelaySignals.set(signal.toNodeId, entries);
  let total = Array.from(pendingRelaySignals.values()).reduce((sum, values) => sum + values.length, 0);
  while (total > MAX_PENDING_RELAY_SIGNALS) {
    const firstNodeId = pendingRelaySignals.keys().next().value;
    const firstEntries = pendingRelaySignals.get(firstNodeId) || [];
    firstEntries.shift();
    if (firstEntries.length) pendingRelaySignals.set(firstNodeId, firstEntries);
    else pendingRelaySignals.delete(firstNodeId);
    total -= 1;
  }
  return true;
}

export function flushRelayedSignalsForPeer(nodeId) {
  prunePendingRelaySignals();
  const target = peers().find((peer) => peer.nodeId === nodeId);
  const entries = pendingRelaySignals.get(nodeId) || [];
  if (!target?.protocol?.sendRelayedSignal || !entries.length) return 0;
  let sent = 0;
  const remaining = [];
  for (const entry of entries) {
    if (target.protocol.sendRelayedSignal(entry.signal)) sent += 1;
    else remaining.push(entry);
  }
  if (remaining.length) pendingRelaySignals.set(nodeId, remaining);
  else pendingRelaySignals.delete(nodeId);
  return sent;
}

export function receiveRelayedSignal(signal, sourcePeerId) {
  const sourceCapabilities = runtime?.peerInfo?.(sourcePeerId)?.capabilities || [];
  const sourceIsRelay = sourceCapabilities.some((value) => [CONTROL_PARTICIPANT_CAPABILITY, BACKBONE_CAPABILITY, ANCHOR_CAPABILITY].includes(value));
  // A freshly connected Mainframe may forward an introduction before its
  // signed capability announcement has reached this peer. Accept that narrow
  // case only when the forwarding hop is an active protocol peer explicitly
  // named in the relay path.
  const sourceIsConnected = peers().some((peer) => peer.nodeId === sourcePeerId && peer.protocol);
  const sourceIsNamedRelay = sourceIsConnected && (signal?.relayVisited || []).includes(sourcePeerId);
  if (!signal || (signal.fromNodeId !== sourcePeerId && !sourceIsRelay && !sourceIsNamedRelay) || !signal.toNodeId || Number(signal.relayHops || 0) > 3) return false;
  ...
  if (target?.protocol?.sendRelayedSignal?.(forwarded)) return true;
  const queued = queueRelayedSignal(forwarded);
  let sent = 0;
  for (const peer of signalRelayPeers()) {
    if (visited.has(peer.nodeId)) continue;
    if (peer.protocol.sendRelayedSignal?.(forwarded)) sent += 1;
  }
  return queued || sent > 0;
}
...
export async function anchorStatus() {
  ...
  const connectedParticipants = controlPeers().length;
  const connectedBackbones = controlPeers().filter((peer) => runtime?.peerInfo?.(peer.nodeId)?.capabilities?.includes(BACKBONE_CAPABILITY)).length;
  ...
  return {
    ...
    connectedAnchors, connectedParticipants, connectedBackbones,
    servedPeers: Math.max(0, connected.length - connectedAnchors), participant: true,
...
export function nodeControlCapabilities() {
  return [
    CONTROL_PARTICIPANT_CAPABILITY,
    ...(settings.enabled ? [BACKBONE_CAPABILITY, ANCHOR_CAPABILITY] : []),
  ];
}
