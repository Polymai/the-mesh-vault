...
An already-open vault with cached manifests and established peer connections can continue discovery, multi-hop signaling relay, signed repair advisories, signed catalog operation replication, index reads, deletion-order delivery and some recovery/control operations while Supabase is unavailable. Folder creation and file-metadata moves can queue locally and replay idempotently. Current cold peer discovery and phrase-only recovery lookup can still require Supabase; new file ingestion, durable segment/placement writes and the atomic repair-job mutation remain Supabase-backed.
