...
import { currentPatientZeroResponderKey } from "../network/patient-zero-authority-chain.js";

const BRIDGE_VERSION = 2;
const PROTOCOL_VERSION = 3;
const MAX_CONTRIBUTION_BYTES = window.__DATA__?.limits?.maxContributionBytes || 2000 * 1024 * 1024 * 1024;
let cached = null;
...
export function nativeNodeConfiguration(state, contributionLimitBytes) {
  ...
  const genesisKeys = Array.from(window.__POLYMAI_RENDEZVOUS_CONFIG__?.authorityPublicKeys || []);
  const currentResponderKey = currentPatientZeroResponderKey();
  const bootstrapAuthorityKeys = [...new Set([...genesisKeys, currentResponderKey].filter((key) => /^[A-Za-z0-9_-]{43}$/.test(String(key))))].slice(0, 8);
  return {
    protocolVersion: PROTOCOL_VERSION,
    supabaseUrl: appConfig.url,
    publishableKey: appConfig.anonKey,
    // Genesis remains the immutable trust root. The verified current responder
    // is also supplied so the native first-contact query survives rotation.
    bootstrapAuthorityKeys,
