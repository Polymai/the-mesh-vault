...
(async () => {
  ...
  const anchors = Array.from({ length: 200 }, (_, index) => ({
    ...
    capabilities: ["anchor-control-v3"],
  ...
  assert.equal(policy.evaluateCoordination(start, { anchorCount: 1, supabaseUp: true, now: 2000 }).mode, policy.COORDINATION_MODES.HYBRID);
  ...
  const afterDrop = policy.evaluateCoordination(start, { anchorCount: 0, supabaseUp: true, now: 2000 });
