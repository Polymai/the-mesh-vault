...
async function main() {
  ...
  try {
    ...
    assert.equal(await page.locator("[name='capacity']").getAttribute("max"), "250", "Settings does not expose the configured 250 GB contribution ceiling.");
    const storedMaximum = await page.evaluate(async () => {
      const { setContributionLimit, getContributionLimit } = await import("/js/services/vault-service.js");
      await setContributionLimit(250 * 1024 ** 3);
      return getContributionLimit();
    });
    assert.equal(storedMaximum, 250 * 1024 ** 3, "The 250 GB contribution ceiling does not persist without being clamped to the former limit.");
    ...
    await reminderPanel.getByRole("button", { name: "Retry", exact: true }).click({ force: true });
    await reminderPanel.getByRole("button", { name: "Disable", exact: true }).waitFor();
    await reminderPanel.getByRole("button", { name: "Disable", exact: true }).click({ force: true });
    await reminderPanel.getByRole("button", { name: "Enable", exact: true }).waitFor();
    ...
    assert((await page.locator(".anchor-role-badge").count()) > 0,
      `The active Anchor badge was not rendered after disabling reminders: ${JSON.stringify(anchorAfterReminderDisable)}`);
