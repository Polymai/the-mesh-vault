...
assert.match(bootstrap, /meshRouteAvailable\s*=\s*connectedProtocols\(\)\.length\s*>\s*0/);
assert.match(bootstrap, /loadVaultCatalog\(\{ includeSupabase: !meshRouteAvailable \}\)/);
assert.match(bootstrap, /loadReplicatedOperations\(\{ force, includeSupabase: !meshRouteAvailable \}\)/);
...
assert.match(control, /includeSupabase = true/);
assert.match(control, /includeAnchors = true/);
...
assert.match(nodeService, /supabaseFallbackOpened && activeNode\._supabaseRegistered/);
...
const newColdStartRequestsPerNodeHour = 8 + 6 + 0 + 12;
const newHybridRequestsPerNodeHour = 0 + 6 + 0 + 12;
