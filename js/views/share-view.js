...
export function renderShareView() {
  return `<section class="auth-layout"><div class="auth-story"><span class="eyebrow">Shared file</span><h1>Someone shared an encrypted file with you.</h1><p>No account is needed. Your browser reconstructs and decrypts the file locally, then verifies it byte for byte before offering the download.</p></div><div class="auth-card"><form data-share-download-form><label>Password (only if the sender set one)<input type="password" name="password" autocomplete="off" aria-label="Share password"></label><button class="button" type="submit">Download shared file</button><p class="form-status" role="status"></p></form></div></section>`;
}
