...
export function normalizeErasureProfile(profile = {}) {
  ...
  return {
    ...
    profileVersion: Math.max(1, Math.trunc(finite(profile.profileVersion, 2))),
    dataShards,
    parityShards,
...
export function selectErasureProfile(profiles = [], independentNodes = 0, resilienceClass = "auto", context = {}) {
  const count = Math.max(0, Math.trunc(finite(independentNodes)));
  const requestedResilienceClass = ["auto", "standard", "high"].includes(resilienceClass) ? resilienceClass : "auto";
  const compatible = profiles
    .filter((profile) => (profile.resilienceClass || "standard") === requestedResilienceClass)
  ...
  if (!eligible && requestedResilienceClass !== "standard") {
    return {
      ...selectErasureProfile(profiles, count, "standard", context),
      requestedResilienceClass,
    };
  }
  const selected = eligible
    || compatible.at(-1)
    || normalizeErasureProfile({ resilienceClass: "auto", minimumIndependentNodes: 5, dataShards: 3, parityShards: 2, repairThreshold: 4 });
  const candidates = Array.isArray(context.candidates) ? context.candidates : [];
  const observedAvailability = candidates.length
    ? candidates.reduce((sum, candidate) => sum + Math.max(0, Math.min(1, finite(candidate.reliability_score, 0.5))), 0) / candidates.length
    : null;
  return {
    ...selected,
    resilienceClass: selected.resilienceClass || requestedResilienceClass,
    requestedResilienceClass,
    selectionContext: {
      independentNodes: count,
      segmentCount: Math.max(0, Math.trunc(finite(context.segmentCount))),
