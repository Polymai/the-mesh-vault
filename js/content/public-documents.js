...
export const LEGAL_VERSIONS = Object.freeze({
  terms: "2026-08-15.1-draft",
  privacy: "2026-08-15.1-draft",
});
...
const securityKeyFlowDiagram = `<svg viewBox="0 0 1240 760" role="img" aria-labelledby="security-key-flow-svg-title security-key-flow-svg-desc" xmlns="http://www.w3.org/2000/svg">
  ...
  <desc id="security-key-flow-svg-desc">A 256-bit browser-generated root becomes a 24-word BIP39 Mesh Key and deterministically derives the Ed25519 owner identity, metadata key, deduplication key and segment-key root. Root and private keys stay in the browser profile. Mesh participants, Backbones and the Supabase fallback receive only public keys and signed or encrypted coordination objects. Storage peers receive encrypted shards. Explicit share links expose only an encrypted, file-version-limited segment-key grant.</desc>
  <defs>
  ...
  <g font-family="Inter, ui-sans-serif, system-ui, sans-serif">
    ...
    <text x="722" y="100" fill="#f8fafc" font-size="23" font-weight="750">Mesh control + Supabase fallback</text>
...
const simpleArchitectureInfographic = `<figure class="architecture-simple" aria-labelledby="architecture-simple-title">
  ...
  <div class="architecture-simple__coordinator">
    ...
    <p><strong>Every online node helps with control data. Stable Backbones can provide verified first contact; Supabase is the durable fallback.</strong> The readable file is not stored in Supabase Storage.</p>
  </div>
...
const architecture = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>A user can deliberately copy or export the 24-word phrase for offline safekeeping. That phrase represents the complete root secret and must be protected like a private key. Mesh participants, Backbones and the Supabase fallback receive public keys and signed or encrypted coordination objects only. Storage peers receive encrypted file shards, never the root, owner private key or segment keys.</p>
    ...
    },
    {
      ...
      <details class="architecture-timers"><summary>How the installed app refreshes mesh state</summary><p>When an installed PWA or ordinary tab becomes visible, the client resumes its heartbeat and reuses recently discovered peers. Discovery tries local context, connected mesh participants, verified Backbones and invites before the delayed Supabase fallback. Windows in one browser profile share discovery metadata but own separate WebRTC links. This foreground refresh cannot keep a suspended mobile browser alive.</p></details>`,
    },
    {
      id: "data-locations",
      ...
      body: `<figure class="architecture-figure architecture-figure--network" aria-labelledby="network-flow-title">
        ...
        <div class="network-diagram">
          ...
          <article class="network-node network-node--coordination"><span>Control mesh + fallback</span><strong>Participants, Patient Zero, Backbones and Supabase</strong><small>Every online node can relay bounded signed or encrypted control data. A pinned Patient Zero device can publish an encrypted cold-start directory through replaceable public relays; Supabase remains the durable metadata and repair fallback.</small></article>
        ...
        <div class="architecture-boundary"><span><b>Never sent by the protocol</b>256-bit root · Mesh Key phrase · owner private key · metadata/deduplication roots · readable file bytes</span><span><b>May leave through bounded paths</b>Public keys and signed/encrypted coordination objects to mesh participants, Backbones and Supabase fallback · encrypted shard payloads to storage peers · an encrypted file-version-limited segment-key grant through an explicit share link</span></div>
        <figcaption>Discovery is mesh-first. A pinned Patient Zero device can bootstrap a browser through encrypted rendezvous even while it is behind NAT. Durable file/placement records and atomic repair still use Supabase in this release.</figcaption>
      </figure>
    ...
    },
    {
      ...
      body: `<p>Supabase is not the cryptographic owner of a vault, but it remains operationally important in the current release. A browser first tries local context, the encrypted Patient Zero rendezvous, connected peers, configured Backbones and invites. It creates an anonymous Supabase Auth session only when that mesh-first path needs the hosted fallback or a durable hosted operation. You provide no name, email address or password, but the resulting technical account is still pseudonymous and is used for Row Level Security.</p>
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-grid">
          ...
          <article class="backbone-card backbone-card--fast"><span>Durable fallback</span><strong>Supabase</strong><ul><li>Anonymous Auth session and RLS access</li><li>Vault and node public registration</li><li>Delayed discovery and signaling fallback</li><li>File/segment/shard metadata and placements</li><li>Atomic repair, deletion and audit state</li></ul><b>Operationally important today; no shard payloads or secret keys.</b></article>
        ...
        <div class="backbone-outage"><span><b>Normal path</b>Known peers, mesh participants, verified Backbones and invites are tried before Supabase. Two participants or one Backbone are enough to keep the control path mesh-first.</span><span><b>Cold start</b>The web/PWA client can ask the pinned Patient Zero device for a signed, short-lived peer directory through encrypted public relays. The current Android native worker still uses Supabase for its own first contact.</span><span><b>Current hard limits</b>New file ingestion, durable segment/placement records and the atomic repair job remain Supabase-backed. The rendezvous path removes a discovery dependency; it does not yet replace those durable transactions.</span></div>
      ...
      <p><strong>Cold-start boundary:</strong> entering the 24 words reconstructs the same identity and key hierarchy locally; no secret-key lookup occurs. Network discovery is separate. The web/PWA client first tries the pinned Patient Zero rendezvous and existing mesh context, then uses delayed Supabase fallback if no verified route forms. Rendezvous messages use NIP-44 authenticated encryption between separate transport keys. The public trust key is deployment data; the Patient Zero private device key and rendezvous transport secret never belong in source code.</p>`,
    },
    {
      id: "identity",
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      id: "service",
      title: "2. The service",
      body: `<p>TheMeshVault is a no-account, cryptographically owned peer-to-peer storage protocol and application. It encrypts files locally and distributes recovery shards across participating devices. Every online node helps exchange bounded control records; a pinned Patient Zero device can provide a signed cold-start directory through encrypted public relays, and stable opt-in Backbones can retain larger caches. Supabase remains the durable fallback for pseudonymous sessions, metadata, placement, repair and deletion transactions. None of these control roles is intended to hold readable files, encrypted shard payloads or secret vault keys.</p>
      <p>It is not a guaranteed backup service. Recoverability depends on verified independent placements, reachable devices and possession of the Mesh Key. The Patient Zero rendezvous can bootstrap the web/PWA control path without Supabase, but durable storage transactions and the current Android native first-contact path still depend on Supabase.</p>`,
    },
    {
      id: "eligibility",
      title: "3. Eligibility",
...
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>Supabase is currently used to make the service function, not for analytics: anonymous-session compatibility, durable segment and placement writes, delayed discovery/signaling fallback and the atomic repair queue depend on it. Web/PWA discovery first tries local context, the encrypted Patient Zero rendezvous, connected participants, public-key-pinned WSS Backbones and invites. Signed catalog operations, encrypted indexes, repair notices and deletion orders can travel through that mesh and later reconcile with Supabase. The current Android native worker still uses Supabase first contact. Backbones retain a sparse Supabase compatibility lease at an approximately ten-minute cadence. Neither Supabase nor mesh control caches receive shard payloads, the root or derived private keys.</p>
    ...
    },
    {
      ...
      body: `<p>Depending on the feature used, limited data can be received by participating peer devices, configured public encrypted-rendezvous relays, Supabase as the current hosted fallback and durable coordination provider, public STUN or configured TURN providers, hosting and CDN providers, browser and operating-system vendors, optional push-delivery services, and authorities where disclosure is legally required.</p>
