...
export async function buildEncryptedVaultProfile(identity, profile, revision = Date.now()) {
  ...
  const encryptedProfile = await encryptJson(value, `vault-profile-v2:${identity.vaultId}`);
  const signable = profileSignable(identity.vaultId, revision, encryptedProfile);
  return {
    ...signable,
...
export async function openEncryptedVaultProfile(record, identity) {
  ...
  try {
    ...
    return { ...cleanProfile(await decryptJson(record.encryptedProfile)), revision: signable.revision };
  } catch { return null; }
}
...
async function readLocalProfile(vaultId) {
  ...
  try { return { profile: cleanProfile(await decryptJson(encryptedProfile)), revision, encryptedProfile }; }
...
export async function setVaultProfile(identity, profile = {}) {
  ...
  return publishProfile(identity, value, Math.max(local.revision, opened?.revision || 0)).catch(async () => {
    const encryptedProfile = await encryptJson(value, `vault-profile-v2:${vaultId}`);
