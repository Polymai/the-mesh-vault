...
> **Resolved historical incident.** The protocol-v3 schema, policies, Realtime configuration and Edge artifacts were subsequently accepted by provisioning on 2026-08-05. The failed run and repair instructions below are retained only as operational history; current implementation and release status are documented in `DOCUMENTATION_INDEX.md` and `RELEASE_TEST_REPORT.md`.
