...
export function providerOrder(mode, kind) {
  const anchorFirst = ["local-peer", "distributed-rendezvous", "anchor-peer", "backbone-seed", "invite-link", "supabase"];
  const supabaseFirst = ["local-peer", "distributed-rendezvous", "backbone-seed", "anchor-peer", "invite-link", "supabase"];
  const meshOnly = ["local-peer", "distributed-rendezvous", "anchor-peer", "backbone-seed", "invite-link"];
...
export function shouldWriteSupabaseHeartbeat(mode, isAnchor, lastWriteAt = 0, now = Date.now(), options = {}) {
  const config = { ...DEFAULT_COORDINATION_CONFIG, ...options };
  // Supabase presence is a cold-start fallback, not a second heartbeat bus.
  // Once any verified mesh participant is available, presence remains inside
  // the mesh and the hosted lease expires naturally.
  if (mode !== COORDINATION_MODES.SUPABASE_PRIMARY) return false;
