...
import { renderMeshView, bindMeshView, disposeMeshView, meshViewSignature } from "./views/mesh-view.js";
...
import { renderBackboneServerView, bindBackboneServerView } from "./views/backbone-server-view.js";
import { renderHostedStorageView, bindHostedStorageView } from "./views/hosted-storage-view.js";
...
const allowed = new Set(["home", "onboarding", "security", "terms", "privacy", "dashboard", "drive", "mesh", "anchor", "backbone-server", "recovery", "settings", "hosted-storage", "account", "night-node"]);
const DOCUMENT_ROUTES = new Set(["security", "terms", "privacy"]);
const PUBLIC_ROUTES = new Set(["home", "onboarding", ...DOCUMENT_ROUTES]);
const VAULT_ONLY_ROUTES = new Set(["dashboard", "drive", "mesh", "recovery", "account"]);
let appActions = null; let rendering = false; let renderPending = false; let lastRenderKey = null; let lastMeshViewKey = null;
...
function nativeNodeRenderKey(nativeNode = {}) {
  return {
    supported: !!nativeNode.supported,
    enabled: !!nativeNode.enabled,
    running: !!nativeNode.running,
    status: nativeNode.status || "Browser only",
    anchor: !!nativeNode.anchor,
    usedBytes: Number(nativeNode.usedBytes || 0),
    shardCount: Number(nativeNode.shardCount || 0),
    peerCount: Number(nativeNode.peerCount || 0),
    lastError: nativeNode.lastError || null,
  };
}
function renderKey(state) {
  ...
  const common = {
    ...
    wakeAssistance: state.survival?.wakeAssistance || "unknown",
    wakeAssistanceReason: state.survival?.wakeAssistanceReason || null,
    wakeAssistanceMessage: state.survival?.wakeAssistanceMessage || null,
    activeTasks: state.activeTasks, pwaInstall: state.pwaInstall,
  };
  ...
  if (route === "settings") return JSON.stringify({ ...common, node: state.node, nativeNode: nativeNodeRenderKey(state.nativeNode), vaultProfile: state.vaultProfile, pwaInstall: state.pwaInstall });
  if (route === "anchor") return JSON.stringify({
    ...common,
    anchor: {
      enabled: !!state.anchor?.enabled,
      status: state.anchor?.status || "off",
      nodeName: state.anchor?.nodeName || "Browser node",
      cacheLimitBytes: Number(state.anchor?.cacheLimitBytes || 0),
      connectedAnchors: Number(state.anchor?.connectedAnchors || 0),
      coordinationMode: state.anchor?.coordinationMode || "supabase-primary",
      wakeLock: state.anchor?.wakeLock || "released",
    },
    survival: { wakeAssistance: state.survival?.wakeAssistance || "unknown" },
    nativeNode: nativeNodeRenderKey(state.nativeNode),
  });
...
function captureActiveForm() {
  ...
  const dialogMarker = dialog?.getAttributeNames().find((name) => name.startsWith("data-")) || null;
  ...
  return { marker, fields, activeName: active.name || null, dialogMarker, dialogOpen: !!dialog?.open };
}
function restoreActiveForm(snapshot) {
  ...
  if (snapshot.dialogOpen && snapshot.dialogMarker) {
    const dialog = document.querySelector(`dialog[${snapshot.dialogMarker}]`);
...
export function renderApp(force = false) {
  ...
  try {
    ...
    const route = guardedRoute(state);
    const nextMeshViewKey = !share && route === "mesh" ? meshViewSignature(state) : null;
    ...
    if (share) {
      document.querySelector("#app").innerHTML = renderShell({ ...state, route: "home", suppressPwaInstallPrompt: true }, renderShareView());
      bindShell({ onToggleNav: () => setState((s) => ({ mobileNavOpen: !s.mobileNavOpen })), onCloseNav: () => setState({ mobileNavOpen: false }), onDismiss: () => setState({ notice: null }), onScrollTarget: scrollToLandingSection, onInstallApp: appActions.settings.installApp });
      bindShareView(share.capabilityId, share.fragmentPayload);
      return;
    }
    let view;
    if (route !== "mesh") { disposeMeshView(); lastMeshViewKey = null; }
    ...
    else if (route === "hosted-storage") view = renderHostedStorageView(state);
    else if (route === "account") view = renderAccountLinkView(state);
    else if (route === "anchor") view = renderAnchorView(state);
    else if (route === "backbone-server") view = renderBackboneServerView();
    else if (route === "night-node") view = renderNightNodeView(state);
    else view = renderSettingsView(state);
    const appRoot = document.querySelector("#app");
    const currentMeshLayout = route === "mesh" && nextMeshViewKey === lastMeshViewKey
      ? appRoot.querySelector("[data-mesh-layout]")
      : null;
    const shellMarkup = renderShell({ ...state, route }, view);
    let meshLayoutPreserved = false;
    if (currentMeshLayout) {
      const template = document.createElement("template");
      template.innerHTML = shellMarkup;
      const placeholder = template.content.querySelector("[data-mesh-layout]");
      if (placeholder) {
        placeholder.replaceWith(currentMeshLayout);
        appRoot.replaceChildren(template.content);
        meshLayoutPreserved = true;
      } else appRoot.innerHTML = shellMarkup;
    } else appRoot.innerHTML = shellMarkup;
    if (route === "mesh") lastMeshViewKey = nextMeshViewKey;
    bindShell({ onToggleNav: () => setState((s) => ({ mobileNavOpen: !s.mobileNavOpen })), onCloseNav: () => setState({ mobileNavOpen: false }), onDismiss: () => setState({ notice: null }), onScrollTarget: scrollToLandingSection, onInstallApp: appActions.settings.installApp });
    ...
    if (route === "mesh" && !meshLayoutPreserved) bindMeshView(appActions.mesh);
    if (route === "recovery") bindRecoveryView(appActions.meshKey);
    if (route === "hosted-storage") bindHostedStorageView();
    if (route === "account") bindAccountLinkView(appActions.account);
    if (route === "anchor") bindAnchorView(appActions.settings);
    if (route === "backbone-server") bindBackboneServerView();
    if (route === "settings") bindSettingsView({
      ...appActions.settings,
      enableNotifications: () => appActions.nightNode.enableNotifications(),
      disableNotifications: () => appActions.nightNode.disableNotifications(),
    });
...
export function startRouter(actions) {
  appActions = actions; const sync = () => {
    const next = routeFromHash();
    const changed = getState().route !== next;
    setState({ route: next, mobileNavOpen: false });
    if (changed) {
      window.scrollTo(0, 0);
