...
const DB_NAME = `${prefix}fragment-location-cache-v3`;
...
async function requestFromPeers(peers, shardHash, timeoutMs, signal) {
  ...
  try {
    for (const peer of unique.values()) {
      const requestId = peer.protocol.requestShard(shardHash);
      if (requestId) requests.push({ peer, requestId });
    }
    const bytes = await waitForVerifiedShard(shardHash, timeoutMs, signal);
    return bytes ? { bytes, sourcePeerId } : null;
  } finally {
    for (const { peer, requestId } of requests) peer.protocol.cancelShardRequest?.(requestId);
