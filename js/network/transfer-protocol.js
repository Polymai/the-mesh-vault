...
export function attachTransferProtocol(channel, handlers = {}) {
  ...
  const send = (message) => {
    ...
    try { channel.send(encoded); handlers.onTraffic?.({ direction: "out", messageType: message.type, bytes: encoded.length }); return true; }
    catch (error) {
      if (channel.readyState !== "open" || error?.name === "InvalidStateError") return false;
      throw error;
    }
  };
  const requireSend = (message) => {
    if (!send(message)) throw new Error("Peer connection closed during transfer.");
  };
  const waitForWritableChannel = async (signal) => {
    while (channel.bufferedAmount > BUFFER_LIMIT) {
      ...
      if (channel.readyState !== "open") throw new Error("Peer connection closed during transfer.");
  ...
  channel.addEventListener("message", async ({ data }) => {
    ...
    handlers.onTraffic?.({ direction: "in", messageType: message.type, bytes: data.length });
  ...
  return {
    ...
    async sendShard(shardId, bytes, hash, onProgress = () => {}, { signal } = {}) {
      ...
      const configuredTimeout = Number(globalThis.window?.__DATA__?.transport?.uploadReceiptTimeoutMs);
      const receiptTimeoutMs = Number.isFinite(configuredTimeout) && configuredTimeout > 0 ? Math.min(STORED_TIMEOUT_MS, configuredTimeout) : STORED_TIMEOUT_MS;
      const stored = new Promise((resolve, reject) => { const timer = setTimeout(() => { pendingStored.delete(transferId); reject(new Error("Peer storage verification timed out.")); }, receiptTimeoutMs); pendingStored.set(transferId, { resolve, reject, timer }); });
      let cancelWait = null;
      let cancelled = null;
      try {
        requireSend({ type: "shard-start", transferId, shardId, chunks, hash });
        for (let index = 0; index < chunks; index++) {
          if (signal?.aborted) throw signal.reason || new DOMException("Shard transfer cancelled", "AbortError");
          await waitForWritableChannel(signal);
          const packet = new Uint8Array(bytes.slice(index * PACKET_BYTES, (index + 1) * PACKET_BYTES));
          requireSend({ type: "shard-chunk", transferId, index, data: btoa(String.fromCharCode(...packet)) });
          onProgress?.((index + 1) / chunks);
        }
        requireSend({ type: "shard-end", transferId });
        if (signal?.aborted) throw signal.reason || new DOMException("Shard transfer cancelled", "AbortError");
        cancelled = signal ? new Promise((_, reject) => {
