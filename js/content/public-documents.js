...
const securityKeyFlowDiagram = `<svg viewBox="0 0 1240 760" role="img" aria-labelledby="security-key-flow-svg-title security-key-flow-svg-desc" xmlns="http://www.w3.org/2000/svg">
  ...
  <desc id="security-key-flow-svg-desc">A 256-bit browser-generated root becomes a 24-word BIP39 Mesh Key and deterministically derives the Ed25519 owner identity, metadata key, deduplication key and segment-key root. Root and private keys stay in the browser profile. Mesh participants and Backbones receive only public keys and signed or encrypted coordination objects. Explicit durable features may send the same protected categories to Supabase. Storage peers receive encrypted shards. Explicit share links expose only an encrypted, file-version-limited segment-key grant.</desc>
  <defs>
  ...
  <g font-family="Inter, ui-sans-serif, system-ui, sans-serif">
    ...
    <text x="722" y="100" fill="#f8fafc" font-size="23" font-weight="750">Mesh control + explicit hosted writes</text>
...
const simpleArchitectureInfographic = `<figure class="architecture-simple" aria-labelledby="architecture-simple-title">
  ...
  <div class="architecture-simple__coordinator">
    ...
    <p><strong>Every online node helps with control data. Patient Zero and stable Backbones provide first contact; routine web/PWA mesh activity does not use Supabase.</strong> Explicit durable features still use hosted transactions, but readable files and shard payloads are not stored in Supabase Storage.</p>
  </div>
...
const supabaseDataDisclosure = `<div class="supabase-disclosure" data-supabase-disclosure>
  ...
  <div class="supabase-disclosure__list">
    <article>
      ...
      <dl>
        <div><dt>Supabase receives</dt><dd>An anonymous Auth user ID, authenticated requests and operational timestamps for each participating browser profile or enabled Android native node. IP address, user-agent and request timing may also occur in ordinary provider or security logs.</dd></div>
    ...
    <article>
      ...
      <dl>
        <div><dt>Supabase stores</dt><dd>Vault ID, node ID, anonymous Auth ID, owner and device public keys, browser- or native-app failure-domain ID, node implementation and the public node label you choose.</dd></div>
        ...
        <div><dt>What it reveals</dt><dd>Random IDs and public keys do not contain a civil identity and cannot be decoded into a name. They do provide stable links to the same vault, browser profile or native app installation. A personal node label can identify you directly.</dd></div>
      </dl>
    </article>
    <article>
...
const architecture = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>A user can deliberately copy or export the 24-word phrase for offline safekeeping. That phrase represents the complete root secret and must be protected like a private key. Mesh participants, Backbones and explicit Supabase durable transactions receive public keys and signed or encrypted coordination objects only. Storage peers receive encrypted file shards, never the root, owner private key or segment keys.</p>
    ...
    },
    {
      ...
      ${nativeBackgroundProcessDiagram}
      <p>The bridge accepts only public runtime values: protocol version, Supabase URL and publishable key, Edge Function URL, Vault ID, public node label, coarse country or region, storage and Anchor cache limits, role selection and ICE server configuration. The Java worker recursively rejects configuration keys that look like a Mesh Key, root seed, private key, file key, segment key, metadata key or deduplication key, and its status contract always reports <code>keysAccepted: false</code>.</p>
      <p>The native node creates its own Ed25519 device identity. Shards are hash-checked before an atomic local commit; manifests and control records are verified and bounded before caching. Its native identity seed and anonymous Supabase refresh token are encrypted with an app-specific Android Keystore AES-GCM key. Shards themselves are already protocol ciphertext and remain in the app's private files directory with SQLite metadata. A bounded partial Wake Lock is acquired only during an active shard transfer and expires after two minutes; the service does not hold an indefinite CPU lock.</p>
    ...
    },
    {
      ...
      <details class="architecture-timers"><summary>How the installed app refreshes mesh state</summary><p>When an installed PWA or ordinary tab becomes visible, the client resumes its local heartbeat and reuses recently discovered peers. Discovery uses local context, encrypted Patient Zero rendezvous, connected mesh participants, verified Backbones and invites. The web/PWA client does not start a timed Supabase fallback for discovery, signaling, presence or Live Mesh refreshes in this deployment. Windows in one browser profile share discovery metadata but own separate WebRTC links. This foreground refresh cannot keep a suspended mobile browser alive.</p></details>`,
    },
    {
      id: "data-locations",
      ...
      body: `<figure class="architecture-figure architecture-figure--network" aria-labelledby="network-flow-title">
        ...
        <div class="network-diagram">
          ...
          <article class="network-node network-node--coordination"><span>Control mesh + durable transactions</span><strong>Participants, Patient Zero and Backbones</strong><small>Online nodes relay bounded signed or encrypted control data. A pinned Patient Zero device publishes an encrypted cold-start directory through replaceable public relays. Supabase is contacted separately only by explicit features that still require hosted durable transactions.</small></article>
        ...
        <div class="architecture-boundary"><span><b>Never sent by the protocol</b>256-bit root · Mesh Key phrase · owner private key · metadata/deduplication roots · readable file bytes</span><span><b>May leave through bounded paths</b>Public keys and signed/encrypted coordination objects to mesh participants and Backbones, and to Supabase only for explicit durable features · encrypted shard payloads to storage peers · an encrypted file-version-limited segment-key grant through an explicit share link</span></div>
        <figcaption>Web/PWA discovery, presence, signaling and Live Mesh refreshes use the mesh path and do not automatically fall back to Supabase. Explicit file-ingestion, placement, sharing, deletion, account and atomic-repair transactions still use Supabase in this release.</figcaption>
      </figure>
      <div class="document-table-wrap"><table><thead><tr><th>Location</th><th>What can be there</th><th>What is not intended to be there</th></tr></thead><tbody>
        ...
        <tr><td>Android native node, when enabled</td><td>A separate native device identity; Android-Keystore-protected device seed and anonymous Supabase refresh token; public node configuration; SQLite indexes; hash-verified encrypted shards; and bounded verified manifest/control caches in private app storage.</td><td>The bridge and native worker do not receive the Mesh Key, root seed, owner private key, readable file bytes or usable metadata, deduplication or segment keys. Native storage is separate from Chrome, Brave or another browser profile.</td></tr>
    ...
    },
    {
      ...
      body: `<p>Supabase is not the cryptographic owner of a vault. In the current web/PWA deployment, startup, presence, discovery, signaling, Live Mesh views and idle maintenance do not automatically contact it. An anonymous Supabase Auth session is created only when you invoke an explicit feature that still needs a hosted durable transaction, such as initial file metadata and placement, sharing, deletion, account linking or atomic repair compatibility. You provide no name, email address or password, but the resulting technical account is still pseudonymous and is used for Row Level Security.</p>
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-grid">
          ...
          <article class="backbone-card backbone-card--fast"><span>Explicit durable path</span><strong>Supabase</strong><ul><li>Anonymous Auth session and RLS access when needed</li><li>Initial file/segment/shard metadata and placements</li><li>Sharing, deletion and optional account state</li><li>Atomic repair and audit compatibility</li><li>No automatic web/PWA discovery or presence polling</li></ul><b>Feature-scoped today; no shard payloads or secret keys.</b></article>
           <article class="backbone-card backbone-card--fallback"><span>Distributed active path</span><strong>Patient Zero, participants and Backbones</strong><ul><li>A pinned Patient Zero device publishes a short-lived signed peer directory</li><li>Replaceable relays carry that directory only as encrypted ciphertext</li><li>Every peer signs its own presence before it can enter the directory</li><li>Signals and bounded control records then travel through the mesh</li></ul><b>Preferred first. None of these nodes becomes vault authority.</b></article>
        </div>
        <div class="backbone-outage"><span><b>Normal path</b>Known peers, Patient Zero, mesh participants, verified Backbones and invites carry web/PWA discovery, presence, signaling and bounded control records without a timed hosted fallback.</span><span><b>Cold start</b>The web/PWA client asks the pinned Patient Zero device for a signed, short-lived peer directory through encrypted public relays. The current Android native worker still uses Supabase for its own first contact.</span><span><b>Current hard limits</b>Initial file ingestion, durable segment/placement records and the atomic repair job remain Supabase-backed user-triggered transactions. They are not background polling.</span></div>
      ...
      <p><strong>Backbone alternative:</strong> A reachable public-key-pinned WSS Backbone can bootstrap browser discovery directly. The encrypted Patient Zero rendezvous provides a separate path for a private Patient Zero device that can make outbound relay connections but cannot accept an inbound public connection.</p>
      <p><strong>Cold-start boundary:</strong> entering the 24 words reconstructs the same identity and key hierarchy locally; no secret-key lookup occurs. Network discovery is separate. The web/PWA client uses the pinned Patient Zero rendezvous and existing mesh context without an automatic Supabase timeout fallback. Rendezvous messages use NIP-44 authenticated encryption between separate transport keys. The public trust key is deployment data; the Patient Zero private device key and rendezvous transport secret never belong in source code.</p>`,
    },
    {
      id: "identity",
      ...
      <p>The current catalog also has an encrypted, owner-signed vault-index snapshot and a local append-only signed operation queue. The browser reads the newest valid index from local state, connected mesh participants or Backbones. Folder creation and metadata moves can remain locally queued and replay idempotently. Initial upload and durable segment/placement records still require the current Supabase-backed transaction workflow.</p>
      <p>The phrase itself is not uploaded. The 24-word phrase and a complete protocol-v3 Mesh Key file carry the same root secret. Typing the phrase or importing that file reconstructs the same root and identity locally; the file contains the 24 words and public Vault ID, not a hosted secret-key package. Possession of either form grants vault authority and it must be protected as a bearer secret. Identity restoration is deterministic, but data discovery is separate: a new web/PWA client asks the pinned Patient Zero device for a signed peer directory through encrypted rendezvous and continues through the mesh. File recovery still requires a signed manifest and at least K valid reachable shards per segment.</p>
    ...
    },
    {
      ...
      body: `<ul>
        ...
        <li>Encrypted Patient Zero rendezvous can bootstrap web/PWA discovery without exposing the peer list to the relay, but this release still depends on Supabase for durable metadata, placement transactions, atomic repair and Android native first contact. Supabase independence is therefore partial, not complete.</li>
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      id: "service",
      title: "2. The service",
      body: `<p>TheMeshVault is a no-account, cryptographically owned peer-to-peer storage protocol and application. It encrypts files locally and distributes recovery shards across participating devices. Every online node helps exchange bounded control records; a pinned Patient Zero device can provide a signed cold-start directory through encrypted public relays, and stable opt-in Backbones can retain larger caches. Web/PWA startup, presence, discovery, signaling, idle maintenance and Live Mesh views use this mesh path without automatic Supabase fallback. Supabase is contacted only by explicit features that still need durable hosted transactions. None of these control roles is intended to hold readable files, encrypted shard payloads or secret vault keys.</p>
      <p>It is not a guaranteed backup service. Recoverability depends on verified independent placements, reachable devices and possession of the Mesh Key. Initial file and placement transactions, sharing, deletion, optional account linking, atomic repair compatibility and the current Android native first-contact path still depend on Supabase.</p>`,
    },
    {
      id: "eligibility",
      title: "3. Eligibility",
    ...
    },
    {
      ...
      body: `<p>The current build uses Supabase Auth, Postgres, Realtime and an Edge Function for explicit durable transactions, not as an automatic web/PWA discovery, presence or Live Mesh loop. When one of those features is invoked, the browser creates a Supabase user marked as anonymous; this requires no email or password but still creates a pseudonymous technical session. Web/PWA cold start uses configured public WebSocket relays as replaceable encrypted transport to a pinned Patient Zero device. Relay operators can receive IP addresses, timing, transport pseudonyms and ciphertext sizes, but not the readable peer directory or vault secrets. The app also depends on browser and operating-system vendors, internet providers, public STUN services, optional TURN relay and optional push delivery. Public pages currently load fonts from Google Fonts and the Supabase browser library from jsDelivr. These services may receive ordinary connection metadata and have their own terms and availability limits.</p>
      <p>Before production launch, the responsible operator must publish the final provider list and contractual roles. Supabase remains a dependency for the named durable features, but ordinary web/PWA mesh participation no longer continually communicates with it.</p>`,
    },
    {
      id: "warranty",
  ...
  sections: [
    {
      ...
      body: `<p>This notice covers TheMeshVault's public site, browser- or app-local vault, optional Android native background node, peer-to-peer transfers, storage-node participation, current Supabase coordination and optional account linking.</p>
    ...
    },
    {
      ...
      <p>The native service creates a separate random device identity. Its private identity seed and anonymous Supabase refresh token are encrypted locally with an Android Keystore key. It stores public configuration such as Vault ID, node label, capacity, coarse location, Backbone role and Supabase endpoint. It does not accept the Mesh Key, root seed, owner private key, file content or usable derived file keys.</p>
      <p>When the service connects, Supabase receives a separate anonymous Auth session and native node record with the same categories of pseudonymous operational data described below. This can distinguish and correlate that app installation over time even though it does not contain a civil identity. Stopping the node ends active participation; removing Android pieces deletes its local shard/control store, while provider-side operational records follow the coordination retention lifecycle.</p>`,
    },
    {
      id: "mesh-data",
    ...
    },
    {
      ...
      <p>Supabase is currently used to make specific features function, not for analytics: explicit initial file/segment/placement writes, sharing, deletion, optional account linking, audit state, atomic repair compatibility and the current Android native first contact depend on it. Web/PWA startup, presence, discovery, signaling, idle maintenance and Live Mesh views use local context, encrypted Patient Zero rendezvous, connected participants, public-key-pinned WSS Backbones and invites without an automatic hosted fallback or compatibility lease. Neither Supabase nor mesh control caches receive shard payloads, the root or derived private keys.</p>
    ...
    },
    {
      ...
      body: `<p>Depending on the feature used, limited data can be received by participating peer devices, configured public encrypted-rendezvous relays, Supabase for explicit hosted durable transactions, public STUN or configured TURN providers, hosting and CDN providers, browser and operating-system vendors, optional push-delivery services, and authorities where disclosure is legally required.</p>
    ...
    },
    {
      ...
      body: `<p>Protocol v3 web/PWA signaling uses mesh providers rather than automatic Supabase Realtime fallback in this deployment. Browser-local data remains under the browser profile's storage lifecycle. Mesh control records expire by type: repair leases after about two minutes, repair requests after about 30 minutes, and signed indexes, operations, deletion records and completion records after up to 30 days in bounded caches. Supabase records created by explicit durable features, manifests, audit events and optional account links follow the service's operational and database lifecycle; the future controller must publish exact retention periods before production launch.</p>
