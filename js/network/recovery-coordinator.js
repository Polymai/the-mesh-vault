...
import { recordShardPlacement, auditAllVersionResilience, queueShardRepair, refreshPhysicalStorageTotals, loadFilePlacementStateForSegment } from "../services/segment-service.js";
...
import { queryAnchors } from "./anchor-service.js";

let timer = null;
let initialTimer = null;
async function syncAnchorRepairAdvisories(node) {
  if (!node?.vault_id) return 0;
  const rows = await queryAnchors("repair-request", "open", 20).catch(() => []);
  let queued = 0;
  for (const row of rows) {
    const request = row?.payload;
    if (request?.vaultId !== node.vault_id || !request?.shardId || Number(request.expiresAt || 0) <= Date.now()) continue;
    const job = await queueShardRepair(request.vaultId, request.shardId, request.targetNodeId || null).catch(() => null);
    if (job) queued += 1;
  }
  return queued;
}
