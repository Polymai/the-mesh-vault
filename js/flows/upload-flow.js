    ...
    for await (const input of streamFileSegments(file, segmentTarget, reportRead)) {
      ...
      if (segment) {
        ...
        await runConcurrent(shardRows, uploadConcurrency, async (shard, index, runSignal) => {
          const payload = shardPayloads[index]; let target = null; let stored = false; let role = "cache";
          ...
          if (!stored) {
            await storeLocalVerified(shard.shard_hash, payload.bytes, payload.hash); target = node;
          ...
          if (target) placementRows.push(await recordShardPlacement({ vaultId, shardId: shard.id, node: target, role }));
