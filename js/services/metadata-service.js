...
async function indexedRows(section, vaultId) {
  const opened = await loadVaultCatalog().catch(() => null);
...
export async function listFolders(vaultId, parentId = undefined) {
  ...
  return Promise.all(rows.map(async (row) => ({ ...row, name: (await decryptJson(row.encrypted_name)).name })));
}
export async function createFolder(vaultId, name, parentId = null) {
  ...
  const row = { id: crypto.randomUUID(), vault_id: vaultId, parent_id: parentId, encrypted_name, created_at: new Date().toISOString() };
...
export async function createFileRecord({ vaultId, folderId, metadata, sizeBytes }) {
  ...
  const row = { id: crypto.randomUUID(), vault_id: vaultId, folder_id: folderId, encrypted_metadata, size_bytes: sizeBytes, status: "local_pending", current_version: 1, created_at: now, updated_at: now };
