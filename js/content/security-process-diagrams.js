...
export const coldStartProcessDiagram = `<figure class="architecture-process architecture-process--cold-start" aria-labelledby="cold-start-title">
  ...
  <div class="process-step-grid process-step-grid--recovery">
    <article class="process-step process-step--key"><b>01</b><span>Pin trust</span><strong>Patient Zero public key</strong><p>The deployed app contains only Patient Zero's Ed25519 public key. Its private key never leaves that profile.</p></article>
    <article class="process-step process-step--network"><b>02</b><span>Open once</span><strong>Temporary anonymous session</strong><p>The new profile opens a bounded Supabase session and publishes a temporary public node description.</p></article>
    <article class="process-step process-step--manifest"><b>03</b><span>Find one node</span><strong>Exact Patient Zero lookup</strong><p>A targeted database function may return only the device whose public key is pinned in the app—not a general peer list.</p></article>
    ...
    <article class="process-step process-step--crypto"><b>05</b><span>Verify</span><strong>Authenticated mesh peer</strong><p>The client accepts the route only after the WebRTC peer proves the pinned Patient Zero device identity.</p></article>
  ...
  <div class="verification-gate">
    <span><b>Supabase can observe during first contact</b>IP and request metadata, anonymous ID, temporary public node details, SDP/ICE and timing.</span>
  ...
  <figcaption>Patient Zero is the sole exception: it keeps one minimal Supabase listener and a sparse presence heartbeat so completely new profiles can reach it. Existing profiles stay mesh-only.</figcaption>
</figure>`;
...
export const nativeBackgroundProcessDiagram = `<figure class="architecture-process architecture-process--native" aria-labelledby="native-background-title">
  ...
  <div class="process-step-grid process-step-grid--native">
    ...
    <article class="process-step"><b>02</b><span>Narrow bridge</span><strong>Public configuration only</strong><p>The bridge sends Vault ID, the pinned Patient Zero key, temporary first-contact configuration, node label, coarse location and quotas. Secret-looking fields are rejected.</p></article>
