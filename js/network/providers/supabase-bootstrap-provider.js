...
async function publishPresenceOnce({ force = false } = {}) {
  ...
  const entries = (data || []).filter((row) => Number(row.coordination_protocol_version) === 3).map((row) => {
    ...
    return {
      ...
      capabilities: anchorHealthy ? ["anchor-control-v3"] : [],
    };
  });
