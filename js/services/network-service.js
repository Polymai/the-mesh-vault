import { appTable, appConfig, currentAccessToken, supabase, supabaseFetch } from "./supabase.js";
...
async function loadNetworkOverviewFallback({ includeNodes = true } = {}) {
  ...
  let nodesQuery = includeNodes
    ? appTable("nodes").select("id,vault_id,device_public_key,failure_domain_id,device_label,status,reliability_score,survival_mode,lifecycle_state,lease_expires_at,uptime_seconds,wake_lock_active,buddy_node_ids,last_handoff_at,capacity_bytes,used_bytes,country_code,region_code,network_domain_hash,location_source,location_updated_at,last_seen_at").gte("last_seen_at", cutoff).order("last_seen_at", { ascending: false }).limit(MESH_VIEW_NODE_LIMIT)
  ...
  if (includeNodes && nodesResult.error && /country_code|region_code|network_domain_hash|location_source|survival_mode|lifecycle_state|lease_expires_at|uptime_seconds|wake_lock_active|buddy_node_ids|last_handoff_at/i.test(`${nodesResult.error.message || ""} ${nodesResult.error.details || ""}`)) nodesResult = await appTable("nodes").select("id,vault_id,device_public_key,failure_domain_id,device_label,status,reliability_score,capacity_bytes,used_bytes,last_seen_at").gte("last_seen_at", cutoff).order("last_seen_at", { ascending: false }).limit(MESH_VIEW_NODE_LIMIT);
