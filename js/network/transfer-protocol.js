...
export function attachTransferProtocol(channel, handlers = {}) {
  ...
  channel.addEventListener("message", async ({ data }) => {
    ...
    if (message.type === "shard-request") handlers.onRequest?.(message.shardId);
  ...
  return {
    ...
    async sendShard(shardId, bytes, hash, onProgress = () => {}) {
      ...
      for (let index = 0; index < chunks; index++) { while (channel.bufferedAmount > BUFFER_LIMIT) await new Promise((resolve) => setTimeout(resolve, 20)); const packet = new Uint8Array(bytes.slice(index * PACKET_BYTES, (index + 1) * PACKET_BYTES)); send({ type: "shard-chunk", transferId, index, data: btoa(String.fromCharCode(...packet)) }); onProgress((index + 1) / chunks); }
      send({ type: "shard-end", transferId });
      try { await stored; const seconds = Math.max(0.001, (performance.now() - startedAt) / 1000); reportQuality({ successfulTransfers: quality.successfulTransfers + 1, throughputEwmaBps: ewma(quality.throughputEwmaBps, bytes.byteLength / seconds) }); return transferId; }
      catch (error) { reportQuality({ failedTransfers: quality.failedTransfers + 1 }); throw error; }
    },
    requestShard(shardId) { send({ type: "shard-request", shardId }); },
