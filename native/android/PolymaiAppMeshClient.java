...
/** Supabase bootstrap plus the bounded native peer set. */
final class PolymaiAppMeshClient implements Closeable {
    ...
    private static final long DISCOVERY_MS = 60_000L;
    ...
    void run() throws Exception {
        ...
        reporter.update(configuration.anchorEnabled ? "Anchor running" : "Storage node running");
        while (!stopRequested.get() && !Thread.currentThread().isInterrupted()) {
            long now = System.currentTimeMillis();
            try {
                if (accessExpiresAt - now < AUTH_REFRESH_MARGIN_MS) {
                    authenticate();
                    reconnectRealtime();
                }
                ...
                reporter.update("Reconnect pending");
    ...
    boolean authorizeAndDelete(String shardHash, JSONObject authorization) {
        try {
            ...
            JSONArray shards = restArray("GET", "segment_shards?select=id&shard_hash=eq." + shardHash, null, null);
            if (shards.length() == 0) return false;
            List<String> ids = new ArrayList<>();
            for (int index = 0; index < shards.length(); index++) ids.add(shards.getJSONObject(index).getString("id"));
            String in = String.join(",", ids);
            JSONArray placements = restArray(
                "GET",
                "shard_placements?select=id,vault_id,status,deletion_authorization&node_id=eq." + nodeId + "&shard_id=in.(" + in + ")",
                null,
                null
            );
            JSONArray deletingIds = new JSONArray();
            boolean retainBytes = false;
            for (int index = 0; index < placements.length(); index++) {
                JSONObject placement = placements.getJSONObject(index);
                JSONObject saved = placement.optJSONObject("deletion_authorization");
                String status = placement.optString("status", "");
                boolean target = authorization.optString("vaultId").equals(placement.optString("vault_id"))
                    && saved != null
                    && authorization.optString("signature").equals(saved.optString("signature"))
                    && "deleting".equals(status);
                if (target) deletingIds.put(placement.getString("id"));
                else if (!"deleted".equals(status) && !"deleting".equals(status) && !"retiring".equals(status)) retainBytes = true;
            }
            if (deletingIds.length() == 0) return false;
            if (!retainBytes && !store.deleteShard(shardHash)) return false;
            JSONObject values = new JSONObject().put("status", "deleted").put("deleted_at", isoNow());
            restArray("PATCH", "shard_placements?id=in.(" + joinJsonStrings(deletingIds) + ")", values, "return=minimal");
            if (!retainBytes) restArray("DELETE", "fragment_location_cache?shard_hash=eq." + shardHash + "&node_id=eq." + nodeId, null, "return=minimal");
            return true;
        } catch (Exception error) {
            lastError = safeMessage(error);
            return false;
    ...
    void announceFragment(String shardHash) {
        try {
            JSONObject values = new JSONObject().put("shard_hash", shardHash).put("node_id", nodeId).put("announced_at", isoNow());
            restArray("POST", "fragment_location_cache?on_conflict=shard_hash,node_id", values, "resolution=merge-duplicates,return=minimal");
    ...
    private void registerNode() throws Exception {
        ...
        } catch (Exception rejectedExistingIdentity) {
            // A copied app backup can carry a node id but not the original
            // anonymous Supabase session. Rotate the public node row id rather
            // than weakening RLS ownership.
            throw new IllegalStateException("Native node registration was rejected; clear this Android app's storage and start it again.", rejectedExistingIdentity);
        }
    }
    ...
    private JSONObject nodeValues(PolymaiAppMeshStore.Stats stats, boolean includeIdentity) throws JSONException {
        ...
        values.put("lease_expires_at", iso(now + 90_000L));
        ...
        values.put("anchor_control_capacity_bytes", configuration.anchorEnabled ? configuration.controlCapacityBytes : 0);
        values.put("anchor_control_used_bytes", configuration.anchorEnabled ? stats.controlBytes : 0);
        values.put("anchor_connected_nodes", configuration.anchorEnabled ? peerCount() : 0);
    ...
    private void heartbeat() throws Exception {
        ...
        lastError = null;
        reporter.update((configuration.anchorEnabled ? "Anchor" : "Storage node") + " · " + peerCount() + " peers");
    }
    ...
    private void refreshIceServers() {
        ...
        try {
            Request request = authorized(new Request.Builder()
                .url(configuration.functionsUrl + "/app717-meshvault-api")
                .post(RequestBody.create(new JSONObject().put("action", "turn-credentials").put("clientProtocolVersion", 3).toString(), JSON)))
                .build();
            JSONObject response = executeObject(request);
            appendIceServers(parsed, response.optJSONArray("iceServers"));
        } catch (Exception ignored) {}
    ...
    private void discoverAndConnect() throws Exception {
        Object raw = rpc("discover_node_candidates", new JSONObject().put("p_node_id", nodeId).put("p_limit", 24));
    ...
    private void connectRealtime() throws Exception {
        ...
        joined.await(8, TimeUnit.SECONDS);
    }
    ...
    @Override public void close() {
        try {
            ...
            if (accessToken != null) restArray("PATCH", "nodes?id=eq." + nodeId, offline, "return=minimal");
        } catch (Exception ignored) {}
