...
(async () => {
  ...
  try {
    ...
    assert.equal(policy.shouldWriteSupabaseHeartbeat(policy.COORDINATION_MODES.ANCHOR_PRIMARY, false, now - 600000, now, config), false);
    assert.equal(policy.shouldWriteSupabaseHeartbeat(policy.COORDINATION_MODES.ANCHOR_PRIMARY, true, now - 90000, now, config), false);
    ...
    assert.doesNotMatch(repository, /writeSupabaseIndex/);
    assert.match(repository, /markSyncReceipt\(operation\.opId, "anchor"/);
    assert.doesNotMatch(repository, /markSyncReceipt\(operation\.opId, "supabase"/);
    ...
    assert.doesNotMatch(segmentService, /rpc\(/);
    ...
    assert.match(uploadFlow, /placement\.status === "deleting"/);
    ...
    assert.match(deleteFlow, /tombstoneFile/);
    ...
    assert.match(deletion, /if \(nodeId && await hashHasPhysicalPlacementOnNode\(nodeId, shardHash, excludedPlacementIds\)\) return false/);
    assert.match(nodeService, /Start this node before clearing its shared storage/);
    assert.match(nodeService, /transitionShardPlacement\(placement\.vault_id, placement\.id, "unavailable"\)/);
    assert.match(nodeService, /queueShardRepair\(placement\.vault_id, placement\.shard_id\)/);
    ...
    assert.match(profileUpgrade, /CLAIM_RENEW_INTERVAL_MS/);
    assert.match(profileUpgrade, /renewSegmentProfileUpgradeClaim/);
    ...
    console.log("PASS control plane: encrypted signed index, tamper rejection, transactional local persistence, concurrent queue sequencing, idempotent multi-device replay, bounded semantic mesh cache, mesh-only replication receipts, leased reference-complete profile commits, atomic deduplicated deletion, pending-before-transfer placement, serialized shared-hash cleanup, signed repair leases, and deletion acknowledgements.");
  } finally {
