...
async function discoverForCoordination({ force = false, forceBackbone = false } = {}) {
  ...
  const results = await discoverPeers({
    ...
    force,
    requireProvider: backboneDue ? supabaseBootstrapProvider.name : null,
  }).catch(() => []);
  if (results.some((entry) => entry.provider === supabaseBootstrapProvider.name)) lastBackboneDiscoveryAt = Date.now();
  return results;
}
async function publishMeshPresence() {
  if (!meshFirstMode()) return [];
  return publishPresence().catch(() => []);
}
...
export async function activateNode(deviceIdentity, vaultId, capacityBytes, iceServers = [], locationHint = null, { transient = false, transientCacheBytes = 0 } = {}) {
  ...
  configurePeers({
    fromNodeId: data.id, iceServers, onState: (nodeId, status) => { window.dispatchEvent(new CustomEvent("meshvault:peer-state", { detail: { nodeId, status } })); refreshCoordinationMode().then(() => publishMeshPresence()).catch(() => {}); announceTopology().catch(() => {}); announcePresence().catch(() => {}); replicatePendingControlObjects().catch(() => {}); },
  ...
  const discoveries = await discoverForCoordination({ forceBackbone: true }).catch(() => []);
