...
async function publishPresenceOnce() {
  ...
  const { data, error } = await supabase.from("nodes").select("id,device_public_key,device_label,status,last_seen_at,capacity_bytes,used_bytes").eq("status", "online").gte("last_seen_at", cutoff).order("last_seen_at", { ascending: false }).limit(24);
  if (error) throw error;
  const entries = (data || []).map((row) => ({ nodeId: row.id, nodeName: row.device_label, devicePublicKey: row.device_public_key, capacityBytes: Number(row.capacity_bytes), usedBytes: Number(row.used_bytes) }));
...
export const supabaseBootstrapProvider = {
  ...
  async discoverPeers() {
    ...
    return entries.filter((entry) => entry.nodeId !== currentNodeId).map((entry) => ({ peerId: entry.nodeId, nodeName: entry.nodeName, devicePublicKey: entry.devicePublicKey, capacityBytes: entry.capacityBytes, usedBytes: entry.usedBytes, source: "supabase" }));
  },
