...
has(docs, "refreshes peer discovery and asks the service worker to check for an updated release", "Security documentation omits installed-PWA foreground refresh behavior.");
has(docs, "each open window owns its own direct WebRTC connections", "Security documentation conflates same-profile peer discovery with window-local WebRTC links.");
has(docs, "This foreground refresh does not keep a suspended mobile browser alive.", "Security documentation overstates foreground refresh as mobile background execution.");
