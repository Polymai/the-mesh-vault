  ...
  <div class="supabase-disclosure__list">
    ...
    <article>
      ...
      <dl>
        ...
        <div><dt>Who can see it</dt><dd>Supabase and the exact pinned Patient Zero discovery path. The temporary row is deleted best-effort after the peer is verified.</dd></div>
    ...
    <article>
      ...
      <dl>
        ...
        <div><dt>How it is authorised</dt><dd>The local operator flag is not enough: its Ed25519 public device key must exactly match the public key pinned in the deployed app.</dd></div>
...
const architecture = {
  ...
  sections: [
    ...
    },
    {
      ...
      ${nativeBackgroundProcessDiagram}
      <p>The bridge accepts only public runtime values: protocol version, temporary Supabase first-contact configuration, pinned Patient Zero public key, Vault ID, public node label, coarse country or region, quotas and ICE configuration. The Java worker recursively rejects configuration keys that look like a Mesh Key, root seed, private key, file key, segment key, metadata key or deduplication key, and its status contract always reports <code>keysAccepted: false</code>.</p>
    ...
    },
    {
      ...
      body: `<figure class="architecture-figure architecture-figure--network" aria-labelledby="network-flow-title">
        ...
        <figcaption>A new profile uses Supabase only until Patient Zero is verified over WebRTC. Files, manifests, placements, sharing, deletion, repair and Live Mesh then remain in the peer mesh.</figcaption>
      </figure>
    ...
    },
    {
      ...
      body: `<p><strong>For an ordinary browser, PWA or Android profile, Supabase is used during one bounded first-contact phase only.</strong> The new profile creates a temporary anonymous session, publishes a temporary public node row, asks for the exact device whose Ed25519 key is pinned as Patient Zero, and exchanges the SDP/ICE data needed to form that direct WebRTC connection. It does not query a general node directory.</p>
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-grid">
          ...
          <article class="backbone-card backbone-card--fast"><span>One-time introduction</span><strong>Supabase</strong><ul><li>Temporary anonymous Auth session</li><li>Temporary public node row</li><li>Exact pinned Patient Zero lookup</li><li>Recipient-scoped SDP/ICE signaling</li><li>Best-effort row deletion and local logout</li></ul><b>No vault data, file metadata or shard payloads.</b></article>
           <article class="backbone-card backbone-card--fallback"><span>Continuing distributed path</span><strong>Patient Zero and mesh participants</strong><ul><li>The direct peer proves the pinned Patient Zero identity</li><li>Peer context spreads through authenticated mesh connections</li><li>Manifests, placements and repairs stay in the mesh</li><li>The ordinary profile cannot silently reopen Supabase</li></ul><b>All normal operation continues peer-to-peer.</b></article>
        </div>
        <div class="backbone-outage"><span><b>Ordinary profile</b>After the first verified peer, the temporary row, session and Realtime channel are closed permanently for that local node identity.</span><span><b>Patient Zero</b>The sole exception keeps a minimal listener and sparse heartbeat so a completely new installation has somewhere to knock.</span><span><b>If the mesh is later unreachable</b>The profile does not fall back to Supabase. It needs a cached peer, invite, native handoff or a returning mesh participant.</span></div>
    ...
    },
    {
      ...
      body: `<ul>
        ...
        <li>A completely new profile depends on Supabase for its one-time introduction to the pinned Patient Zero node. Patient Zero itself keeps a minimal hosted listener and heartbeat. If that first-contact gate is unavailable, a profile with no cached peer or invite cannot enter the mesh.</li>
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      id: "service",
      title: "2. The service",
      body: `<p>TheMeshVault is a no-account, cryptographically owned peer-to-peer storage protocol and application. It encrypts files locally and distributes recovery shards across participating devices. Every online node can exchange bounded signed or encrypted control records. A completely new local profile uses a temporary Supabase session only to locate the exact, public-key-pinned Patient Zero node and negotiate its first direct WebRTC connection. Once that identity is verified, the ordinary profile closes that hosted path permanently and continues through authenticated mesh peers.</p>
    ...
    },
    {
      ...
      body: `<p>The current build uses Supabase Auth, a narrow Postgres RPC and Realtime only for the bounded first-contact phase. An ordinary profile creates a temporary anonymous technical session, publishes a temporary public node row, looks up only the pinned Patient Zero public key and exchanges recipient-scoped SDP/ICE messages. After a verified direct peer exists, it deletes the row best-effort, clears the local session and closes Supabase. It cannot silently reopen that path for normal discovery, presence, Live Mesh, file handling or repair.</p>
      <p>Patient Zero is the sole hosted exception and keeps a minimal row, listener and sparse heartbeat while enabled. The app also depends on browser and operating-system vendors, internet providers, public STUN services, an optional TURN relay and optional push delivery. Public pages currently load fonts from Google Fonts and the Supabase browser library from jsDelivr. Those services can receive ordinary connection metadata and have their own terms and availability limits.</p>`,
    },
    {
      id: "warranty",
...
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>The native service creates a separate random device identity. Its private identity seed is encrypted locally with an Android Keystore key. A temporary anonymous Supabase token may exist only while the native profile performs its first contact and is erased after a verified mesh peer is established. It stores public configuration such as Vault ID, node label, capacity, coarse location and the pinned Patient Zero identity. It does not accept the Mesh Key, root seed, owner private key, file content or usable derived file keys.</p>
    ...
    },
    {
      ...
      <p>For an ordinary browser, PWA or Android profile, the permitted calls are limited to temporary anonymous Auth, temporary node registration, exact Patient Zero discovery and recipient-scoped WebRTC signaling. Once Patient Zero or another verified mesh peer is connected, the local bootstrap-complete marker prevents normal operation from reopening Supabase. Files, manifests, segment and shard placements, sharing, deletion, repair, audit state, presence and Live Mesh use local or peer-mesh data instead.</p>
    ...
    },
    {
      ...
      body: `<div class="document-table-wrap"><table><thead><tr><th>Purpose</th><th>Typical data</th><th>Expected legal basis</th></tr></thead><tbody>
        <tr><td>Open the first mesh connection</td><td>Temporary anonymous Supabase session, temporary public node details, pinned Patient Zero lookup and transient SDP/ICE</td><td>Performance of the service contract</td></tr>
