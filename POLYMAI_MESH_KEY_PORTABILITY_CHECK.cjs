...
async function main() {
  ...
  try {
    ...
    if (process.argv.includes("--local-only")) {
      ...
      console.log(`PASS: a complete version-2 Mesh Key file opened the same vault locally in a fresh browser without waiting for recovery-envelope lookup. Screenshot: ${path.join(os.tmpdir(), "themeshvault-simple-restore-mobile.png")}. No recovery secret was logged.`);
      return;
    }
