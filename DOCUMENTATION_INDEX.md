...
- automatic bounded control participation by every online node, the one-time Patient Zero cold-start gate and the pseudonymous data Supabase can observe during that bounded phase; and
- the difference between restoring vault authority and recovering file bytes from enough reachable shards.

The process diagrams explicitly show the current v3 primitives and boundaries: browser-profile IndexedDB versus temporary browser memory, HKDF-SHA-256 derivation, AES-256-GCM segment protection, Ed25519 manifest signing, SHA-256/HMAC integrity roles, Reed-Solomon redundancy, peer shard storage, one-time Patient Zero introduction and the permanent local/mesh-only boundary after it succeeds.

The optional Android wrapper adds a keyless native storage/control service with a foreground-service notification, private app shard storage, a Keystore-protected native device identity, screen-off WebRTC participation and boot restart after explicit opt-in. Its native profile uses the same one-time Supabase introduction and erases the temporary credential after its first verified mesh peer. `ANDROID_BACKGROUND_NODE.md` is the source of truth for that plane.
...
The protocol-v3 artifacts are provisioned, Mesh Key portability and selected browser/PWA/peer checks pass, and the first-contact-only source contracts align. The candidate is still an alpha. The new permanent cutoff requires fresh live validation of first contact, row/session retirement, zero ordinary steady-state Supabase traffic, uploader-offline recovery, K/K-1 threshold, restrictive-network/TURN and physical-phone behavior.
