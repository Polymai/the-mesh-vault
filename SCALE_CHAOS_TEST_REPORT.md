...
Date: 2026-08-04  
Result: **PASS — deterministic logical-node simulation**  
...
The cryptographic fixture used a deterministic 32-byte harness root plus deterministic per-segment test keys with the production AES-GCM helper. It did not execute the complete 24-word Mesh Key, deterministic Ed25519 identity, signed descriptor/manifest or production `deriveSegmentKey()` path. Those protocol-v3 properties are covered by the separate portability, browser-unit and documentation-contract checks; this report is evidence for byte processing, coding and lifecycle behavior, not the full v3 authority chain.
...
- Successful v3 provisioning does not change this limitation: the logical nodes in this harness do not create Supabase sessions or rows.
