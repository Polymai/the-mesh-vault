...
import {
  decryptBytes, decryptJson, deriveSegmentKey, hashBytes,
...
export async function downloadFile(fileId, { vaultIdentity } = {}) {
  ...
  try {
    ...
    for (let index = 0; index < manifest.segments.length; index++) {
      ...
      const key = await deriveSegmentKey(keyContext);
      let representation;
      try { representation = await decryptBytes(cipher, key, segmentEncryptionAad(keyContext)); }
      finally { key.fill(0); }
