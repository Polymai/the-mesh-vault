...
import { upsertPeer, touchPeer } from "./peer-registry.js";
...
function bindChannel(peer, channel) {
  ...
  channel.onopen = () => { clearTimeout(peer.connectTimer); context.onState?.(peer.nodeId, "connected"); upsertPeer({ peerId: peer.nodeId, devicePublicKey: peer.devicePublicKey, protocolVersion: 1, availableStorageBytes: 0 }); window.setTimeout(() => peer.protocol?.measureRtt?.().catch(() => {}), 100); };
  ...
  const protocol = attachTransferProtocol(channel, {
    ...
    onRequest: (shardHash) => context.onFragmentRequest?.(peer, shardHash),
  ...
  peer.protocol = {
    ...protocol,
    async sendShard(shardId, bytes, hash, onProgress) {
      ...
      return protocol.sendShard(shardId, bytes, hash, onProgress);
    },
  };
}
...
export function connectedProtocols() { return Array.from(peers.values()).filter((peer) => peer.channel?.readyState === "open").map((peer) => ({ nodeId: peer.nodeId, peerId: peer.nodeId, devicePublicKey: peer.devicePublicKey, protocol: peer.protocol, quality: peer.protocol?.metrics?.() || peer.quality || null })); }
...
export function closePeers() { peers.forEach((peer) => { clearTimeout(peer.connectTimer); peer.pc.close(); }); peers.clear(); unsubscribeSignal?.(); unsubscribeSignal = null; }
