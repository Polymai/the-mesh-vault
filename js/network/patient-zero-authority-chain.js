...
import { withPatientZeroAuthorityRead } from "./supabase-fallback-policy.js";
...
export async function loadPatientZeroAuthorityBundle({ force = false } = {}) {
  ...
  if (!force && lastRemoteRefreshAt && Date.now() - lastRemoteRefreshAt < REMOTE_REFRESH_MS) return verifiedBundle;
  loadingPromise = (async () => {
    ...
    // One remote read per bounded refresh window is enough. Discovery and UI
    // refreshes reuse the already verified signed chain.
    lastRemoteRefreshAt = Date.now();
    const { data, error } = await withPatientZeroAuthorityRead(async () => {
      await ensureAnonymousSession();
      return supabase.schema("app717_meshvault").rpc("get_patient_zero_authority_bundle", {
