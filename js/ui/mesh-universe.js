...
export function renderMeshUniverse(nodes, edges, { selectedNodeId, currentNodeId } = {}) {
  ...
  return `<div class="mesh-universe" data-mesh-universe data-selected-node="${esc(selectedNodeId)}" data-current-node="${esc(currentNodeId)}">
...
export function bindMeshUniverse({ nodes, edges, selectedNodeId, currentNodeId, onSelect }) {
  ...
  function draw(time) {
    ...
    edges.forEach((edge) => {
      ...
      const idlePulse = .78 + Math.sin(time * .0016 + hashNumber(`${edge.source}${edge.target}`) % 1000) * .22;
      const baseAlpha = (highlighted ? .85 : .14) * idlePulse;
