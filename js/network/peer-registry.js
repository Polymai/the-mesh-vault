...
const PROFILE_CHANNEL_NAME = "themeshvault-peer-registry-v3";
...
export function upsertPeer({
  ...
  if (!peerId || protocolVersion !== PEER_PROTOCOL_VERSION) return false;
  ...
  const next = {
    peerId, nodeName: resolvedNodeName, devicePublicKey: devicePublicKey || existing?.devicePublicKey || null, protocolVersion: PEER_PROTOCOL_VERSION, capabilities: capabilities?.length ? capabilities : existing?.capabilities || [], trustStatus,
