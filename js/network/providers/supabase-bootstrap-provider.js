...
function bootstrapAuthorityKeys() {
  return [...new Set((globalThis.window?.__POLYMAI_RENDEZVOUS_CONFIG__?.authorityPublicKeys || [])
    .map((value) => String(value || "").trim())
    .filter((value) => /^[A-Za-z0-9_-]{43}$/.test(value)))].slice(0, 8);
}
...
async function publishPresenceOnce({ force = false } = {}) {
  ...
  const entries = (data || []).filter((row) => authorityPublicKeys.includes(String(row.device_public_key || ""))
    && Number(row.coordination_protocol_version) === 3 && isReachableCandidate(row)).map((row) => {
