...
function connectPresenceEntries(entries, ownNodeId) {
  ...
  for (const entry of [...selected, ...anchorAssignments.reserve]) {
    ...
    // A node that explicitly receives Patient Zero from the bounded hosted
    // first-contact lookup owns this handshake. It creates the offer even when
    // its random node id sorts after Patient Zero's id. Patient Zero remains a
    // passive answerer and can therefore accept many clients independently.
    else if (firstContact || !reserveIds.has(nodeId)) connectPeer(nodeId, { forceOffer: firstContact }).catch(() => {});
  }
