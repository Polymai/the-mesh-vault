...
export function renderNightNodeView(state) {
  ...
  const reminderLabel = survival.wakeAssistance === "ready"
    ? "On"
    : ["available", "permission needed"].includes(survival.wakeAssistance) ? "Off"
      : ["blocked", "brave setup needed"].includes(survival.wakeAssistance) ? "Needs setup"
        : "Unavailable";
  return `<section class="night-node ${active ? "is-running" : ""}" aria-labelledby="night-node-title">
    ...
    <div class="night-node__core">
      ...
      <p class="form-status" data-night-status role="status"></p>
    </div>
    <div class="night-node__telemetry" aria-live="polite">
      ...
      <div><span>Node reminders</span><strong>${reminderLabel}</strong><small>optional wake assistance</small></div>
    </div>
