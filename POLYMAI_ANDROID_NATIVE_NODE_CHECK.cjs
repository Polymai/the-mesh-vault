...
requireCondition(has(worker, "MAX_CONTRIBUTION_BYTES = 2000L * 1024L * 1024L * 1024L"), "native worker must accept the shared 2,000 GB ceiling");
requireCondition(has(bridge, "2000 * 1024 * 1024 * 1024"), "WebView bridge must accept the shared 2,000 GB ceiling");
...
requireCondition(has(client, 'secrets.put("supabase_refresh_token"') && has(client, 'secrets.get("supabase_refresh_token"'), "native first-contact token handling is missing");
requireCondition(has(client, 'secrets.put("mesh_bootstrap_complete_v3", "true")') && has(client, 'secrets.put("supabase_refresh_token", null)'), "native first-contact completion must persist the cutoff and erase its hosted refresh token");
requireCondition(has(client, 'restArray("DELETE", "nodes?id=eq." + nodeId'), "native first-contact registration must be retired after a verified peer connects");
...
requireCondition(has(client, "/realtime/v1/websocket") && has(client, "discover_bootstrap_gateways") && !has(client, "discover_node_candidates"), "native first-contact signaling must discover only pinned Patient Zero gateways");
requireCondition(has(client, "configuration.iceServers") && has(client, "stun:stun.l.google.com:19302"), "native ICE configuration path is missing");
...
requireCondition(has(settings, "Keep helping when closed") && has(settings, "Android storage") && has(settings, "data-native-node-toggle"), "native Settings controls are incomplete or expose the former technical role panel");
