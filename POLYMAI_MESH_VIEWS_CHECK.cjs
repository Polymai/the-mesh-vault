...
async function connectedEndpoints(page) {
  await page.waitForFunction(async () => {
    ...
    return !!currentNode()?.id && connectedProtocols().some((peer) => !!peer.nodeId);
  }, null, { timeout: 30000 });
  return page.evaluate(async () => {
    ...
    return { fromNodeId: currentNode()?.id, toNodeId: connectedProtocols().find((peer) => peer.nodeId)?.nodeId };
  });
}
...
async function main() {
  ...
  try {
    ...
    null, { timeout: 20000 });
    await owner.page.waitForFunction(() =>
      Number(document.querySelector("[data-mesh-connections]")?.dataset.presencePulseTargets || 0) >= 2,
    null, { timeout: 6000 });
    ...
    assert(mobileMetrics.stageHeight >= 500, `The graph is too short on mobile: ${mobileMetrics.stageHeight}px.`);
