...
function Test-WebSocketDependency {
  if (-not (Test-Path -LiteralPath (Join-Path $BackboneRoot 'node_modules\ws\package.json'))) { return $false }
  & node -e "import('ws').then(()=>process.exit(0)).catch(()=>process.exit(1))"
  return $LASTEXITCODE -eq 0
}

if (-not (Test-WebSocketDependency)) {
  Write-Host 'Installing the small WebSocket dependency. This happens only on first start...'
  $Installed = $false
  for ($Attempt = 1; $Attempt -le 2 -and -not $Installed; $Attempt += 1) {
    & npm.cmd install --omit=dev
    $Installed = $LASTEXITCODE -eq 0 -and (Test-WebSocketDependency)
    if (-not $Installed -and $Attempt -lt 2) { Write-Host 'First install attempt did not complete; retrying once...' -ForegroundColor Yellow }
  }
  if (-not $Installed) { Stop-WithMessage 'npm install failed twice. Check the network connection and run this start file again.' }
}
