...
const supabaseDataDisclosure = `<div class="supabase-disclosure" data-supabase-disclosure>
  ...
  <div class="supabase-disclosure__list">
    ...
    <article>
      ...
      <dl>
        <div><dt>Supabase stores</dt><dd>An owner-signed encrypted private vault profile, an owner-signed encrypted vault-index snapshot, immutable signed vault operations, encrypted names and metadata, file sizes, coding parameters, vault-keyed deduplication fingerprints, shard hashes and placements, recovery state, transfer/repair/deletion records, signed or encrypted manifests, recovery envelopes, share-capability records and audit events.</dd></div>
    ...
    },
    {
      ...
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-grid">
          ...
          <article class="backbone-card backbone-card--fast"><span>Hosted backbone</span><strong>Supabase</strong><ul><li>Anonymous Auth session and RLS access</li><li>Vault and node registration</li><li>Bounded first-contact discovery</li><li>Opaque recipient signaling fallback</li><li>File/segment/shard metadata and placements</li><li>Durable signed index and operation fallback</li><li>Transfer, repair, deletion and audit state</li><li>Encrypted manifest and recovery caches</li></ul><b>Operationally important today.</b></article>
...
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>Supabase is currently used to make the service function, not merely for optional analytics: new file ingestion, durable segment and placement writes, cold peer discovery, phrase-only recovery lookup, signaling fallback and the atomic repair queue normally depend on it. Signed catalog operations, encrypted index snapshots, repair notices and signed deletion orders can also travel through connected Anchors and later reconcile with the backbone. The application does not use Supabase Storage as a central bucket for complete file contents.</p>
