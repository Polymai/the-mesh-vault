...
## 2026-07-31 five-phase control-plane addendum

This addendum covers the source candidate that implements the five ordered control-plane phases: encrypted signed vault index, local signed operation queue, common control repository, distributed repair/deletion coordination, and reduced Supabase backbone load. The new SQL/RLS/RPC artifacts are source-verified but **not live-deployed by this run**; Polymai provisioning is required before hosted index and operation-cache paths can be treated as production-active.

| Test | Result | Observation |
|---|---:|---|
| Encrypted signed vault index | PASS | A real browser generated, signed, encrypted, persisted and reopened an index; modified ciphertext and invalid signatures were rejected. |
| Local signed operation queue | PASS | Twelve concurrent appends received unique sequence numbers; independently signed device operations with the same sequence replayed deterministically and duplicate operation IDs were no-ops. |
| Common control repository | PASS | Catalog loading follows Local → Anchors → Supabase fallback, validates every envelope, rebases valid revision conflicts and records separate Anchor/Supabase receipts. |
| Distributed repair coordination | PASS | Signed requests, short Anchor-replicated deterministic leases and completion markers passed source/browser contracts; the existing Supabase repair queue remains the atomic mutation authority when reachable. |
| Distributed deletion | PASS | Owner-signed erase orders and device-signed acknowledgements are replicated through Anchors; returning nodes verify authority and shard placement before erasing. |
| Reduced hosted heartbeat load | PASS | The 10,000-node/200-Anchor model estimates 18.6 Supabase heartbeat writes/s with 10-minute ordinary Anchor-primary heartbeats and 90-second Anchor heartbeats. This is a deterministic model, not a provider load test. |
| Storage and recovery regression | PASS | Automatic/manual profiles, exact K-shard recovery, retry/cancellation, offline grace, rejoin surplus retirement and file-wide failure-domain spread passed. |
| Browser/Anchor regression | PASS | Optional reminders, Brave failure guidance, persisted Anchor visibility, Night Node lease renewal and reload behavior passed in Chromium. |
| Static and documentation contracts | PASS | 121 JavaScript/CJS files and 4 JSON files parse; registry, schema/RLS/RPC, public pages, documentation truth and app contracts pass. |

The release decision remains conditional. This run did not provision the revised backend, perform a physical Android/iPhone suspension test, test carrier/TURN-only traversal, or run a real Supabase provider load test.
