...
export function renderSettingsView(state) {
  ...
  const nativeSection = nativeNode.supported ? `<section class="panel settings-section native-node-panel"><div class="settings-section-head"><div><h2>Android background node</h2><span class="settings-section-kicker">${nativeNode.running ? "Running with screen off" : nativeNode.enabled ? "Restart pending" : "Off"}</span></div><span class="status-pill status-pill--${nativeNode.running ? "safe" : "neutral"}">${nativeNode.running ? "Running" : "Stopped"}</span></div><div class="native-node-stats"><span><small>Stored</small><strong>${compactBytes(nativeNode.usedBytes)}</strong></span><span><small>Pieces</small><strong>${Number(nativeNode.shardCount || 0)}</strong></span><span><small>Peers</small><strong>${Number(nativeNode.peerCount || 0)}</strong></span><span><small>Role</small><strong>${nativeNode.anchor ? "Anchor" : "Node"}</strong></span></div><p class="native-node-boundary">Encrypted pieces only. Your Mesh Key and file keys stay in the vault UI.</p>${nativeNode.lastError ? `<p class="form-status form-status--error">${esc(nativeNode.lastError)}</p>` : ""}<div class="native-node-actions"><button class="button" type="button" data-native-node-toggle="${nativeNode.enabled || nativeNode.running ? "off" : "on"}">${nativeNode.enabled || nativeNode.running ? "Stop background node" : "Run with screen off"}</button><button class="button button--ghost" type="button" data-native-power ${nativeNode.enabled || nativeNode.running ? "" : "disabled"}>Battery settings</button><button class="button button--ghost button--danger" type="button" data-native-clear ${nativeNode.running ? "disabled" : ""}>Remove Android pieces</button></div><p class="form-status" data-native-node-status role="status"></p></section>` : "";
  ...
  <div class="settings-grid">
    <section class="panel settings-section"><div class="settings-section-head"><h2>Storage you share</h2></div><form data-capacity-form><label class="range-label"><span><strong>Maximum on this ${nativeNode.running ? "Android app" : "browser"}</strong><output>${gb(contributionLimitBytes)} GB</output></span><input type="range" name="capacity" min="0" max="${maxContributionGb}" step="0.25" value="${gb(contributionLimitBytes)}"><small>Up to ${maxContributionGb} GB. Device storage limits still apply.</small></label><button class="button" type="submit">Save limit</button><p class="form-status" role="status"></p></form></section>
    ${nativeSection}
