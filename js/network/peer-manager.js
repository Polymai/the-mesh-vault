...
function scheduleReconnect(nodeId) {
  ...
  const attempt = Math.min(6, Number(reconnectAttempts.get(nodeId) || 0));
...
function create(nodeId, polite) {
  ...
  const peer = { nodeId, peerId: nodeId, pc, makingOffer: false, ignoreOffer: false, isSettingRemoteAnswerPending: false, polite, pendingIce: [], channel: null, protocol: null, devicePublicKey: null, createdAt: Date.now(), connectTimer: null, manualClose: false };
...
export function closePeers() { reconnectTimers.forEach((timer) => clearTimeout(timer)); reconnectTimers.clear(); reconnectAttempts.clear(); seenSignals.clear(); peers.forEach((peer) => { clearTimeout(peer.connectTimer); peer.manualClose = true; peer.pc.close(); }); peers.clear(); externalPeers.clear(); unsubscribeSignal?.(); unsubscribeSignal = null; }
