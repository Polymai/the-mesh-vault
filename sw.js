const APP_BUILD = 68;
const CACHE_NAME = `themeshvault-shell-v${APP_BUILD}`;
...
const SHELL = ["./", "./index.html", "./manifest.webmanifest", "./assets/pwa-icon-192.png", "./assets/pwa-icon-512.png", "./assets/apple-touch-icon.png", "./data/runtime-config.js", "./data/rendezvousConfig.js", "./css/tokens.css", "./css/base.css", "./css/shell.css", "./css/components.css", "./css/views.css", "./css/mesh-view.css", "./css/data-flow.css", "./css/mesh-connections.css", "./css/landing-refinement.css", "./css/public-documents.css", "./css/security-architecture.css", "./css/security-process-diagrams.css", "./css/night-node.css", "./js/vendor/nacl-fast.min.js", "./js/vendor/cytoscape.esm.min.js", "./js/network/supabase-fallback-policy.js", "./js/platform/patient-zero-operator.js", "./js/ui/mesh-connections.js", "./js/ui/data-flow-panel.js", "./js/observability/data-flow-log.js", "./js/content/security-process-diagrams.js", "./js/main.js"];

self.addEventListener("install", (event) => {
  event.waitUntil(caches.open(CACHE_NAME)
    .then((cache) => Promise.allSettled(SHELL.map((asset) => cacheFreshShellAsset(cache, asset))))
    .then(() => self.skipWaiting()));
});
async function cacheFreshShellAsset(cache, asset) {
  const canonicalUrl = new URL(asset, self.registration.scope);
  const freshUrl = new URL(canonicalUrl);
  freshUrl.searchParams.set("app-release", String(APP_BUILD));
  const response = await fetch(freshUrl, { cache: "reload" });
  if (!response.ok) throw new Error(`Unable to cache ${asset}: ${response.status}`);
  await cache.put(canonicalUrl.href, response);
}
async function activateAppShell() {
  const keys = await caches.keys();
  const replacedShell = keys.some((key) => key.startsWith("themeshvault-shell-v") && key !== CACHE_NAME);
  await Promise.all([
    Promise.all(keys.filter((key) => ![CACHE_NAME, SHARE_CACHE_NAME].includes(key)).map((key) => caches.delete(key))),
    caches.open(SHARE_CACHE_NAME).then((cache) => cleanupShareInbox(cache)),
  ]);
  await self.clients.claim();
  if (!replacedShell) return;
  const windows = await self.clients.matchAll({ type: "window", includeUncontrolled: true });
  await Promise.all(windows.map(async (client) => {
    try {
      if (typeof client.navigate === "function") await client.navigate(client.url);
      else client.postMessage({ type: "meshvault:app-shell-updated" });
    } catch {
      client.postMessage({ type: "meshvault:app-shell-updated" });
    }
  }));
}
...
self.addEventListener("message", (event) => {
  if (event.data?.type === "meshvault:skip-waiting") self.skipWaiting();
});
...
self.addEventListener("fetch", (event) => {
  ...
  const mustBeFresh = event.request.mode === "navigate"
    ...
    || url.pathname.endsWith("/data/runtime-config.js");
  const networkRequest = mustBeFresh
