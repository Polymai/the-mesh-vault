...
(async () => {
  ...
  const estimatedHeartbeatWritesPerSecond = 200 / 60 + 9800 / 120;
  assert.ok(estimatedHeartbeatWritesPerSecond < 100, "10,000-node steady-state heartbeat target stays below 100 writes/s");
  ...
  console.log(`PASS coordination scale: 10,000 nodes, 200 Anchors, avg ${average.toFixed(1)} primary assignments, max ${Math.max(...loads)}, estimated ${estimatedHeartbeatWritesPerSecond.toFixed(1)} heartbeat writes/s`);
})().catch((error) => {
