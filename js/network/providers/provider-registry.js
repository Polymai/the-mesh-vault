...
import { supabaseProviderAllowed } from "../supabase-fallback-policy.js";
...
function routedProviders(kind) {
  ...
  return providersFor(kind).slice().sort((left, right) => {
    ...
    // Supabase is not an implicit tail provider. Outside the short, explicit
    // fallback window even a failed mesh send must stop locally instead of
    // leaking into hosted discovery/signaling/manifest lookups.
    .filter((provider) => provider.name !== "supabase" || supabaseProviderAllowed());
}
...
async function fanOut(kind, method, args, { stopOnFirstSuccess = false, excludeProviders = [] } = {}) {
  ...
  for (const provider of routedProviders(kind)) {
    ...
    try {
      ...
      // A browser Anchor relay can accept a message without proving delivery
      // to the final recipient. Continue through the fallback chain for that
      // provisional result; verified WSS/Supabase delivery remains final.
      if (stopOnFirstSuccess && result && result?.conclusive !== false) return results;
