...
const recoveryCoordinatorSource = fs.readFileSync("js/network/recovery-coordinator.js", "utf8");
const deletionServiceSource = fs.readFileSync("js/services/deletion-service.js", "utf8");

const reservationIndex = recoveryCoordinatorSource.indexOf('status: "pending"');
const repairSendIndex = recoveryCoordinatorSource.indexOf("protocol.sendShard", reservationIndex);
const repairVerifyIndex = recoveryCoordinatorSource.indexOf('transitionShardPlacement(node.vault_id, reservation.id, "verified")', repairSendIndex);
assert(reservationIndex >= 0 && repairSendIndex > reservationIndex && repairVerifyIndex > repairSendIndex,
  "Repair must reserve a pending placement before transfer and verify it only after acknowledgement.");
const lifecycleDeleteIndex = recoveryCoordinatorSource.indexOf("acknowledgeLifecycleShardDeletion");
const sharedLookupIndex = deletionServiceSource.indexOf("hashHasPhysicalPlacementOnNode");
const guardedDeleteIndex = deletionServiceSource.indexOf("deleteShardIfUnreferenced", sharedLookupIndex);
const physicalDeleteIndex = deletionServiceSource.indexOf("await deleteShard(shardHash)", guardedDeleteIndex);
const deletedTransitionIndex = deletionServiceSource.indexOf('transitionShardPlacement(segment.vault_id, placement.id, "deleted")', physicalDeleteIndex);
assert(lifecycleDeleteIndex >= 0 && sharedLookupIndex >= 0 && guardedDeleteIndex > sharedLookupIndex
  && physicalDeleteIndex > guardedDeleteIndex && deletedTransitionIndex > physicalDeleteIndex,
"Surplus cleanup must use reference-aware physical deletion and mark a placement deleted only after acknowledgement.");
