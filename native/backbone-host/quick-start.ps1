...
$Config = [ordered]@{
  ...
  # A loopback-only host is intentionally disconnected from Supabase. It may be
  # managed and tested locally, but it cannot advertise internet storage until
  # an operator gives it a public pinned WSS endpoint.
  supabase = [ordered]@{ url = ''; publishableKey = ''; heartbeatMs = 600000 }
