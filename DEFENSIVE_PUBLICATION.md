...
Readable file content, the 256-bit root, owner private key and root-level symmetric keys remain inside authorised browser or app-WebView profiles during operation. Storage peers hold encrypted shard payloads without decryption keys. The optional Android foreground service is a keyless storage peer with a separate native device identity and private opaque-shard store; its bridge rejects vault-secret fields. Mesh control caches may hold public keys plus signed or encrypted coordination objects, including encrypted metadata, signed manifests, shard hashes and placement records. Supabase receives only temporary first-contact node data and signaling, never those continuing control records, file-shard payloads or root-level vault secrets. An explicitly created share capability contains only the segment-key grant for one named file version, and the recipient secret remains in the URL fragment.
...
The current implementation is browser-first and uses WebRTC for peer transport, browser-local OPFS/IndexedDB for shard storage, participating peers for continuing coordination and Supabase only as a bounded first-contact gate to Patient Zero. Alternative implementations can use native mobile wrappers, desktop clients, servers, removable storage or opaque third-party storage adapters while preserving the same cryptographic and recovery model.
...
| Mesh Key | A 24-word BIP39 encoding of the vault's 256-bit root seed, including its checksum. It deterministically reconstructs the same v3 identity and key hierarchy locally. |
...
| Capability link | A signed encrypted grant limited to one file version and its named segment keys, plus a recipient secret kept in the URL fragment. |
...
flowchart LR
    ...
    U -->|public keys, signed/encrypted\ncoordination objects only| C[Coordination backbone]
    U <-->|signed/encrypted control records| A[Anchor peers]

    C -. never receives .-> X[256-bit root, Mesh Key,\nowner private key, usable segment keys,\nfile-shard payloads, readable file]
    A -. never receives in control role .-> X
    N1 -. cannot read .-> X
    N2 -. cannot read .-> X
    N3 -. cannot read .-> X
```
...
The locally held root deterministically establishes the Ed25519 owner key and separate symmetric key domains. A hosted user account is not the root of ownership. Operations that affect ownership or trusted state can be signed and verified at clients.
...
The mesh coordination plane provides discovery after first contact, signaling, metadata indexing, placement status, encrypted caches, repair scheduling and deletion progress. Its records remain subject to signature, hash and client-side decryption checks. Participating control caches carry coordination objects rather than encrypted shard payload bytes; Supabase is outside this continuing plane.
...
An authorised client asks the browser CSPRNG for one 32-byte root seed using `crypto.getRandomValues()`. BIP39 appends its checksum and encodes those 256 random bits as 24 English words. The same root deterministically produces:

- a domain-separated 32-byte Ed25519 owner signing seed;
- an Ed25519 owner public key and self-certifying Vault ID;
- a private discovery identifier;
- an AES-256 key for private metadata;
- an HMAC-SHA-256 key for vault-private deduplication; and
- an HKDF root from which context-bound AES-256 segment keys are derived.

A separate device signing identity belongs to one browser profile and is not derived as the vault owner.
...
This makes the identifier deterministic and self-certifying: a verifier can confirm that an operation's raw Ed25519 public owner key corresponds to the stated Vault ID before checking its signature. Protocol v3 admits Ed25519 vault authority only.
...
## 6. Root seed and Mesh Key recovery

The present Mesh Key is a standard 24-word BIP39 mnemonic for exactly 256 bits of entropy plus the BIP39 checksum. Input is normalised and validated against the pinned English word list before any identity is restored. It is a direct portable representation of the root secret, not a password for a hosted key record.

The root seed, AES-GCM nonces, capability identifiers and share secrets are obtained from `crypto.getRandomValues()`, not `Math.random()`. This prevents predictable application-level pseudo-randomness, but it still depends on the browser and operating system correctly implementing and protecting their cryptographically secure random-number generator.

The fixed v3 key hierarchy uses HKDF-SHA-256 with a fixed hierarchy salt and explicit domain labels. In simplified form:

```text
root_seed = CSPRNG(32 bytes)
mesh_key = BIP39-English(root_seed)                    # 24 words
owner_seed = HKDF-SHA-256(root_seed, "owner-ed25519-seed")
owner_keypair = Ed25519-from-seed(owner_seed)
vault_id = SHA-256(raw_owner_public_key)
metadata_key = HKDF-SHA-256(root_seed, "metadata-aes-256")
dedup_key = HKDF-SHA-256(root_seed, "dedup-hmac-sha256")
segment_root = HKDF-SHA-256(root_seed, "segment-hkdf-root")
```

```mermaid
flowchart TB
    R[256-bit root from crypto.getRandomValues] --> M[24-word BIP39 Mesh Key]
    R --> O[Domain: Ed25519 owner seed]
    R --> D[Domain: private discovery ID]
    R --> E[Domain: metadata AES-256]
    R --> H[Domain: deduplication HMAC-SHA-256]
    R --> S[Domain: segment HKDF root]
    O --> V[Deterministic owner public key and Vault ID]
    S --> K[Context-bound per-segment AES-256 keys]
```

Two current input forms carry the same secret:

- **24-word phrase:** the user enters the BIP39 words directly; and
- **Mesh Key file:** a local JSON bearer file contains the 24 words, public Vault ID and format metadata.

Either form deterministically reconstructs the same cryptographic identity and symmetric hierarchy inside the new browser without fetching secret key material. That local identity restoration must not be confused with remote vault discovery or file recovery. A disconnected local profile may use one bounded Supabase window to reach the exact pinned Patient Zero node; it then receives current peer context through the authenticated mesh and cannot reopen Supabase for normal operation during that running lifecycle.

### 6.1 Local key-storage boundary

For reload persistence, the current web client stores the encoded 256-bit root and separate device identity material in application-scoped IndexedDB. It reconstructs the owner signing key and symmetric hierarchy locally. These values rely on the browser profile, operating system and device access controls for local-at-rest protection. The current implementation does not claim hardware-backed keys or protection against code already executing with the application's origin privileges.

Consequently, same-origin XSS, a malicious dependency, a privileged browser extension, an unlocked endpoint, operating-system compromise or direct access to the browser profile can expose the root and derived secrets. Network cryptography does not mitigate that local execution threat. A copied phrase or Mesh Key file is likewise a bearer credential.

Alternative embodiments may add secret sharing of recovery material, hardware-backed roots, passkeys, social recovery, QR transfer or threshold authorisation while retaining explicit domain separation.
...
- maximum selectable device contribution: 2,000 GiB, subject to the storage quota actually granted by the browser, operating system and available disk; and
...
- a deterministic 256-bit segment key derived from the vault's segment HKDF root and canonical segment context;
...
The derivation descriptor binds protocol v3, Vault ID, segment ID, key-derivation version, encryption generation, storage-format version and vault-private deduplication token. SHA-256 of the canonical descriptor supplies the HKDF salt and the fixed info label identifies segment AES-256-GCM use. The encryption AAD additionally binds original size, stored size and compression method. The key is recomputed locally from this authenticated context and is neither stored in the manifest nor transported through coordination.

Segment private metadata is encrypted with the separate domain-derived metadata key and its own explicit AAD. The 96-bit segment nonce is generated by `crypto.getRandomValues()` and prefixed to the cipher representation. The implementation relies on cryptographically random nonces and does not maintain a global nonce registry.
...
Profiles may be upgraded through a new immutable coding generation when more independent nodes become available. Old and new coding generations may coexist during one migration, but the current reader accepts protocol v3 records only. Manifest and storage-format fields make that strict boundary explicit.
...
- uses bounded parallel shard requests and starts delayed hedge requests when an earlier candidate is slow, instead of waiting serially for one desktop-like peer.
...
Each file version has a signed, content-addressed protocol v3 manifest. The present manifest format and storage protocol are both 3; the current verifier rejects any other protocol value.

The manifest includes:

- manifest and storage protocol versions;
- Vault ID and file-version ID;
- raw Ed25519 owner public key and signature algorithm;
- creation time;
...
- for every segment:
  ...
  - compression method;
  - storage-format, key-derivation and encryption-generation fields;
  - vault-private deduplication token;
  - coding generation;
  - K, parity count, R and N;
  - cipher hash;
  - owner-signed segment descriptor hash and signature;
  - encrypted private metadata containing the plaintext hash and authenticated context;
  - ordered shard hashes; and
  - observed durable placements with shard index, node, failure domain and role.

The manifest core is canonicalised, hashed and signed. A manifest identifier is derived from the complete signed object. A client checks the content address, owner/Vault ID relationship, signature, strict protocol v3 fields and internal structure before using it.

Readable filenames and sensitive private segment metadata remain encrypted in their signed protocol objects as appropriate.

---

## 12. Vault-scoped deduplication and version reuse

Before encryption, each plaintext segment is fingerprinted with HMAC-SHA-256 under the root-derived deduplication key and a protocol-specific domain label:

```text
dedup_key = HKDF-SHA-256(root_seed, "dedup-hmac-sha256")
fingerprint = HMAC-SHA-256(dedup_key,
                           "themeshvault/v3/dedup-token" || NUL || plaintext_segment)
```
...
2. derives the segment key locally from the segment HKDF root and verified canonical descriptor;
...
1. generates a random 24-byte capability identifier;
2. generates a random 256-bit fragment secret;
3. deterministically derives the relevant segment keys locally from their verified descriptors;
4. constructs a bounded grant containing only those segment IDs, derivation generations, segment keys and plaintext verification hashes needed for one named file version;
5. derives an AES-256 capability key from the fragment secret, capability identifier and optional password material;
6. encrypts the bounded grant with explicit capability AAD;
7. signs an Ed25519 core containing Vault ID, file-version ID, manifest ID, expiry, optional download limit and single-use flag;
8. stores the signed encrypted capability record in coordination; and
9. places the recipient fragment secret only in the URL fragment.

Browsers do not normally send URL fragments in HTTP requests. The recipient presents the capability identifier to coordination, verifies the self-certifying owner signature, file/version scope and expiry, uses the fragment secret to derive the capability key, opens the limited segment-key grant, retrieves shards and verifies exact recovery. The grant conveys neither the vault root nor owner private key and cannot derive keys for segments outside its encrypted list.
...
## 17. Supabase first-contact gate and trust boundary

The current release uses Supabase for one bounded bootstrap phase per ordinary local node profile. It is not the continuing metadata, placement, repair, catalog, sharing or network-overview database. Public rendezvous relays are disabled in this deployment.

### 17.1 Ordinary first contact

A completely new browser, PWA or Android profile creates a temporary anonymous session, publishes a temporary public node row, calls the narrow `discover_bootstrap_gateways` RPC for the deployment-pinned Patient Zero key, and exchanges recipient-scoped WebRTC offer, answer and ICE messages with that exact device. The route is accepted only after the remote peer proves the pinned Ed25519 identity.

After the first verified peer exists, the ordinary profile records bootstrap completion locally, deletes its temporary row best-effort, clears the anonymous session and closes Realtime. It is not permitted to reopen Supabase for discovery, presence, signaling, Live Mesh, files or repair. If every cached and connected mesh route is later unavailable, it waits for a cached peer, invite, native handoff or returning participant rather than silently falling back to Supabase.

Patient Zero is the sole intentional continuing exception. While explicitly enabled by the deployment operator, it retains one minimal public node row, a signaling listener and a sparse heartbeat so a completely new profile has somewhere to make first contact. It does not receive vault authority or readable file data.

### 17.2 Supabase data boundary

During first contact Supabase can receive a temporary anonymous Auth identifier, temporary public node details, the exact pinned Patient Zero lookup request, recipient-scoped SDP/ICE signaling, and normal service metadata such as IP address, user agent, timing and payload sizes.

Supabase does not receive as intended application data:

- the 256-bit root seed or its 24-word Mesh Key representation;
- owner or device private keys;
- usable metadata, deduplication or segment keys;
- readable file content or encrypted shard payload bytes;
- vault catalogs, manifests, segment or shard placements;
- continuing presence, repair, deletion, sharing or Live Mesh state; or
- a central file backup in Supabase Storage.

The ordinary row is deleted best-effort, but infrastructure logs and backups follow the provider's own lifecycle. The boundary is therefore **first contact only**, not anonymity from the bootstrap provider. Supabase is never trusted for identity claims: the client verifies Patient Zero's pinned key and verifies all later signatures, hashes, content addresses, recovery thresholds and authenticated encryption.

### 17.3 Recovery and outage boundary

Entering the 24-word Mesh Key reconstructs the same identity and symmetric hierarchy locally. No secret key material is fetched from Supabase or Patient Zero. Network discovery is separate: a disconnected app lifecycle may use one bounded first-contact gate, then uses only its cached or connected mesh routes after a peer is verified. A later cold start with no reachable route may briefly use a new bounded gate.

If Supabase is unavailable, established participants continue over existing peer paths. A completely new profile with no cached peer or invite cannot perform its first introduction until Patient Zero's minimal gate is reachable again. Patient Zero's private key remains only on that device; source code and Supabase contain only its public trust key.

File ingestion, catalogs, manifests, placements, repair and deletion do not use Supabase in this release. The Android native worker follows the same bounded per-lifecycle first-contact rule as the browser client.

The architecture therefore distinguishes the bounded cold-start dependency from continuing mesh operation. Public rendezvous relays and a continuous Supabase compatibility fallback are disabled.

---

## 18. Automatic control mesh and Backbones

Every online protocol-v3 node automatically participates in bounded replication of signed or encrypted control records. A node that opts into a larger cache and a stable availability role is called a **Backbone**. A dedicated Node.js Backbone can expose a verified WSS endpoint and store real opaque shards. The control layer may store:

- signed manifests;
- signed operation-log entries;
- owner-signed encrypted vault-index snapshots;
- short-lived repair requests and signed repair leases;
- owner-signed deletion orders and device-signed deletion acknowledgements;
...
- 32 MiB automatic participant cache and a 100 MiB default Backbone cache;
- selectable range approximately 25 MiB to 1 GiB;
- three primary Backbone assignments plus two deterministic reserves where enough independent healthy Backbones exist;
- replication target of three independent failure domains;
- 60-second Anchor control-replication cycle;
- 30-second peer/Anchor heartbeat while active;
- no ordinary Supabase heartbeat after first-contact completion for the remainder of that lifecycle;
- 30-day manifest and operation-log retention;
- 120-second peer presence; and
- 5-minute fragment-location announcements.

A control cache receives public keys and bounded signed or encrypted coordination objects. It does not receive the root, Mesh Key, private or segment keys, readable file, or shard payload bytes, and it gains no vault authority. A browser, Android node or dedicated server can separately hold encrypted shards under its storage quota.

```mermaid
flowchart TB
    B[Browser mesh] <-->|bounded control| P[Ordinary participants]
    B <-->|pinned WSS and replicas| A1[Backbone A]
    B <-->|pinned WSS and replicas| A2[Backbone B]
    S[Supabase first-contact gate] -. new profile only .-> B
...
Backbone selection uses deterministic weighted rendezvous ranking with lease health, failure-domain diversity, reliability, bounded cache headroom and reported connection load. Ordinary participants provide the continuing distributed path. A configured pinned Backbone authenticates server messages with Ed25519, but this release does not require an ordinary user to publish a WSS endpoint. Supabase introduces a brand-new local profile only to the exact pinned Patient Zero identity.

The control repository maintains an encrypted owner-signed catalog snapshot and append-only signed local operation queue. It resolves reads from local state and connected participants. Operation IDs make replay idempotent, and mesh receipts are tracked without a continuing hosted application-data mirror.

Repair needs are signed, gossiped and cached as expiring control objects. Candidate repairers publish short signed leases and deterministically elect the earliest valid lease observed for a request. Placements and acknowledgements are verified and propagated through the mesh; Supabase is not the repair queue or placement database in this release.

For deletion, the owner publishes node-bound signed orders to Anchors before hiding the file. A returning holder verifies the owner signature, erases the addressed shard when it is no longer referenced, and publishes a device-signed acknowledgement. The owner checks that acknowledgement against the registered device public key, placement, shard hash and original deletion signature before marking the persistent placement deleted.

Alternative embodiments can extend this control plane with a CRDT, quorum register, DHT or Byzantine-fault-aware consensus. The essential rule is that replicated coordination state remains subordinate to cryptographic vault authority and content verification.

---

## 19. Night Nodes, mobile participation and continuity

A browser, old phone, native Android container, desktop or server can act as a longer-lived node. The present Night Node concept adds:
...
Backbone eligibility is determined by the renewable lease and current node status, not by notification permission. Reminders are optional. A selected buddy can ask the hosted Edge Function to send a rate-limited notification; the recipient must tap it to reopen Night Node. It does not run WebRTC in the service worker.

The foreground client also reconciles stale application state without claiming continuous background execution. On launch or return to visibility it resumes node presence, refreshes discovery and checks for a newer service worker. While the Live Mesh surface is visible, peer discovery is refreshed at a 15-second cadence and the wider overview at a 60-second cadence. Same-origin windows in one browser profile exchange bounded discovered-peer snapshots through `BroadcastChannel`; direct WebRTC channels remain owned by the individual window and are not represented as shared. Topology events are emitted only for semantic changes so routine heartbeat telemetry does not force the mesh visualisation or an open details element to rebuild.
...
The current Android wrapper adds a user-started foreground-service node beside the hosted protocol-v3 WebView. The WebView retains vault authority and performs root-derived encryption, decryption and owner signing. A versioned native bridge sends only public configuration: protocol version, hosted endpoint and publishable key, Vault ID, public node label, coarse location, storage/control quotas, Anchor role and ICE configuration. The Java worker recursively rejects secret-looking configuration fields and never receives the Mesh Key, root seed, owner private key, metadata/deduplication roots or segment keys.

The native service has a separate random Ed25519 device identity, a SQLite index and private app files for opaque encrypted shards. It verifies shard hashes before an atomic local commit and bounds manifest/control records before caching them. The device seed is encrypted at rest with an app-specific Android Keystore AES-GCM key. A temporary anonymous Supabase credential may exist only during first contact and is erased after a verified mesh peer is established. Native WebRTC uses the same bounded shard, proof, manifest and control messages as browser nodes. A two-minute partial CPU Wake Lock is acquired only during an active shard transfer; the service does not keep an indefinite Wake Lock.

After explicit user start, Android presents a persistent foreground-service notification and may restart the enabled node after boot. The service can therefore remain available when the WebView is hidden or the screen is off. It cannot override force-stop, Doze, network loss, OEM battery policy or operating-system process termination and is not described as permanently alive. Native repair currently serves and relocates already-created opaque shards under owner-authorized protocol messages; root-key-dependent erasure reconstruction and owner repair publication remain in the authorised vault client.

The Android app and an ordinary mobile browser intentionally have separate storage sandboxes. Restoring the same Mesh Key in the app recreates the same vault authority, but does not copy a browser profile's local shards or IndexedDB state. The new native node obtains its own node identity and placements. The earlier browser node should be retired or cleared only after the mesh reports adequate verified replacement placement.

The hosted delivery model lets a normal web release update both browsers and the Android app's WebView. Any change to the Java service, bridge, Android permissions or packaged dependencies requires a newly signed APK/AAB release.
...
for each stable segment S of F:
    whole_hash.update(S)
    fingerprint = HMAC(dedup_key, "themeshvault/v3/dedup-token" || NUL || S)
    ...
    descriptor = canonical(v3, vault, segment, derivation, encryption generation,
                           storage format, fingerprint)
    key = HKDF-SHA-256(segment_root, SHA-256(descriptor),
                      "themeshvault/v3/segment-aes-256-gcm")
    cipher = AEAD_encrypt(key, fresh_random_96_bit_nonce, R0, bound_context)
    ...
    discard derived key bytes after use
    retain private hash metadata encrypted under metadata_key
...
for each segment:
    ...
    verify cipher hash
    derive segment key from verified canonical descriptor
...
- Mesh, Backbone and Supabase control caches receive no root, private keys or encrypted shard payload bytes through their coordination roles.
...
- The 24-word phrase restores cryptographic identity locally. A completely new local profile still depends on the bounded Supabase introduction to the pinned Patient Zero device unless it already has a cached peer or invite. Placements and repair are mesh state, not Supabase transactions.
- Same-origin XSS, malicious dependencies, privileged extensions, operating-system compromise and access to the local browser profile are outside the network cryptography's protection and may expose the root or plaintext.
...
| Vault identity, BIP39 Mesh Key and deterministic root hierarchy | `js/identity/vault-identity.js` |
...
| Signed encrypted vault index | `js/control/vault-index.js` |
| Local operation queue and control routing | `js/oplog/oplog-engine.js`, `js/control/control-repository.js` |
| Distributed repair leases | `js/network/distributed-repair.js`, `js/network/recovery-coordinator.js` |
...
| Android bridge contract | `js/platform/native-node.js`, `native/android/background-worker.json` |
| Android foreground storage/Anchor worker | `native/android/PolymaiAppBackgroundWorker.java`, `native/android/PolymaiAppMeshClient.java`, `native/android/PolymaiAppMeshPeer.java` |
| Android private shard store and device identity | `native/android/PolymaiAppMeshStore.java`, `native/android/PolymaiAppMeshCrypto.java` |
...
The protocol-v3 Supabase schema, RLS, Realtime and Edge artifacts were successfully provisioned on 5 August 2026. Provisioning proves that the current database/function artifacts were accepted by the target platform; it does not prove a peer transfer, exact file recovery, TURN traversal or mobile background survival.

Current v3 source and deterministic test evidence includes:

- checksum-valid 24-word Mesh Key generation from 256 browser-CSPRNG bits;
- deterministic phrase/file restore of the same Ed25519 owner identity and self-certifying Vault ID;
- domain-separated metadata, deduplication and segment-key derivation with no hosted recovery envelope or wrapped segment key;
- strict v3 segment-descriptor, manifest, capability and peer admission checks;
- source/schema/RLS/Edge alignment through schema plan 11;
- full browser phrase/file portability against the configured backend without Mesh Key request or console leakage;
- an installed-PWA share-target import and camera fixture completing through the production upload path;
- four isolated browser profiles establishing real peer connections while rendering one observed mesh; 
- atomic generation upgrades and deduplicated deletion, pending-before-transfer placement and fail-closed content-addressed cleanup contracts through C32;
- deterministic K/R/N profile, repair, return and surplus-reconciliation checks; and
- a 1,000-logical-node chaos model that used real compression, AES-GCM, SHA-256 and Reed-Solomon operations, while modeling rather than opening the browser, WebRTC, Anchor and Supabase layers at that scale.

Earlier browser runs recorded five-profile WebRTC transfer, exact 100,000,000-byte recovery, uploader-offline recovery, threshold behavior, repair, sharing, deletion and warm Supabase-outage behavior for the superseded pre-v3 storage design. Those observations are retained as engineering history only. They do not certify v3 because that design used hosted recovery lookup and key-wrapping records that v3 deliberately removed.

The still-open v3 evidence gates are a fresh uploader-offline golden recovery, live threshold and deletion/repair journeys, hosted-data inspection, TURN-only traversal, physical Android/iPhone behavior, browser eviction resistance, days-long churn and unrestricted production load. Those limits do not alter the disclosed architecture, but they constrain present product claims.

See `UAT_TEST_LOG.md`, `GOLDEN_RECOVERY_TEST_REPORT.md` and `RELEASE_TEST_REPORT.md` for the current execution boundary.
...
TMV-DP-001, version 1.5, 5 August 2026, [public URL], [SHA-256].
```
...
2. a 256-bit browser-generated root encoded as a 24-word BIP39 Mesh Key, with deterministic Ed25519 identity and domain-separated metadata, deduplication and segment-key derivations;
...
13. capability sharing whose encrypted, file-version-limited segment-key grant is opened by a recipient secret in the URL fragment;
