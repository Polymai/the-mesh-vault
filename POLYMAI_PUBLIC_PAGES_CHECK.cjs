  ...
  try {
    ...
    for (const route of ["security", "terms", "privacy"]) {
      ...
      if (route === "security") {
        ...
        assert(backbone.includes("No hosted secret-key lookup occurs."), "Security does not disclose the protocol-v3 Mesh Key boundary.");
        ...
        assert.equal(await page.locator(".architecture-process").count(), 3, "Security must render the key, upload and recovery process diagrams.");
    ...
    assert.equal(await mobile.locator(".architecture-process").count(), 3, "Mobile Security lost one or more process diagrams.");
    for (const kind of ["keys", "upload", "recovery"]) {
