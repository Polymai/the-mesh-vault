...
(async () => {
  ...
  assert.match(anchorView, /Open setup guide/);
  assert.doesNotMatch(anchorView, /github\.com\/Polymai\/the-mesh-vault\/tree\/main\/native\/backbone-host/);
  assert.match(anchorView, /Total space to share/);
  assert.match(anchorView, /Several virtual nodes on one computer still count as one independent device/);
  assert.match(serverGuide, /themeshvault-backbone-host-v3\.1\.0\.zip/);
  assert.match(serverGuide, /npm run check/);
