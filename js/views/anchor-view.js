...
function patientZeroOperatorMode() {
  try {
    return new URLSearchParams(globalThis.location?.search || "").get("patient-zero-setup") === "1";
  } catch {
    return false;
  }
}
...
export function renderAnchorView(state) {
  ...
  const operatorSetup = patientZeroOperatorMode();
  const rendezvous = operatorSetup ? state.anchor?.distributedRendezvous || window.__TMV_RENDEZVOUS_STATUS__ || null : null;
  const rendezvousMarkup = operatorSetup ? `<section class="panel settings-section patient-zero-panel">
    <div class="settings-section-head"><div><span class="eyebrow">Operator only</span><h2>Patient Zero setup</h2><p>Run this once in the browser profile that will remain Patient Zero. Its private signing key stays on this computer.</p></div><span class="status-pill status-pill--${rendezvous?.trustedAuthorities ? "safe" : "neutral"}">${rendezvous?.authority ? "Patient Zero" : rendezvous?.trustedAuthorities ? "App configured" : "Not published"}</span></div>
    <ol class="patient-zero-steps">
      ...
      <li><span>2</span><div><strong>Copy the app configuration</strong><small>Paste it into Polymai to replace <code>data/rendezvousConfig.js</code>, then publish the app.</small></div></li>
    ...
    <div class="patient-zero-actions"><button class="button" type="button" data-download-patient-zero>Download public setup</button><button class="button button--ghost" type="button" data-copy-patient-zero-config>Copy app configuration</button></div>
    <details class="settings-technical"><summary>Connection status</summary><div class="anchor-ledger"><span>Relays<strong>${Number(rendezvous?.relays?.connected || 0)} / ${Number(rendezvous?.relays?.configured || 0)} connected</strong></span><span>Directory<strong>${Number(rendezvous?.peers || 0)} recent peers</strong></span><span>Export<strong>Public keys only</strong></span></div></details>
