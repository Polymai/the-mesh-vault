  ...
  sections: [
    ...
    },
    {
      ...
      body: `<ul>
        ...
        <li>A completely new profile depends on Supabase for its one-time introduction to the current Patient Zero responder. It verifies the public authority chain from the genesis key already in the app. Patient Zero itself keeps a minimal hosted listener and heartbeat. If that first-contact gate is unavailable, a profile with no cached peer or invite cannot enter the mesh.</li>
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: `<p>The current build uses Supabase Auth, narrow Postgres RPCs and Realtime for the bounded first-contact phase. An ordinary profile creates a temporary anonymous technical session, verifies the small public Patient Zero authority chain from the genesis key shipped in the app, publishes a temporary public node row, looks up only the chain's current responder and exchanges recipient-scoped SDP/ICE messages. After a verified direct peer exists, it deletes the row best-effort, clears the local session and closes Supabase. It cannot silently reopen that path for normal discovery, presence, Live Mesh, file handling or repair.</p>
