...
const LEGACY_DB_NAMES = [`${prefix}anchor-control-v1`, `${prefix}anchor-control-v2`];
const DB_NAME = `${prefix}anchor-control-v3`;
const STORE = "objects";
...
const ALLOWED_KINDS = new Set(Object.keys(GROUP_LIMITS));
...
function startLegacyCleanup() {
  ...
  for (const name of LEGACY_DB_NAMES) try { indexedDB.deleteDatabase(name); } catch {}
}
...
export async function buildControlObject(signer, kind, lookupKey, payload, { ttlMs = 5 * 60 * 1000 } = {}) {
  ...
  if (!ALLOWED_KINDS.has(String(kind || ""))) throw new Error("Invalid control-object kind.");
  ...
  const core = {
    ...
    createdAt: Date.now(), expiresAt: Date.now() + boundedTtl, protocolVersion: 3,
  };
...
export async function verifyControlObject(object, { now = Date.now() } = {}) {
  if (!object || typeof object !== "object" || object.protocolVersion !== 3) return { valid: false, reason: "malformed" };
  if (!ALLOWED_KINDS.has(String(object.kind || "")) || !String(object.lookupKey || "")) return { valid: false, reason: "malformed" };
