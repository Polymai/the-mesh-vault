...
import {
  ...
  acceptManifestReplica, receiveRelayedSignal, flushRelayedSignalsForPeer, publishControlObject, replicatePendingControlObjects, anchorStatus,
...
let peerDirectoryReplayTimer = null;
...
const PEER_DIRECTORY_REPLAY_DEBOUNCE_MS = 150;
...
async function handleGossipMessage(_peer, message) {
  if (message.kind === GOSSIP_KINDS.PEER_ANNOUNCE) {
    const previousAnnouncement = latestPeerAnnouncements.get(message.payload.nodeId);
    const announcementChanged = !previousAnnouncement
      || previousAnnouncement.signature !== message.signature
      || previousAnnouncement.timestamp !== message.timestamp;
    ...
    // Every recipient must create local handshake state. connectPeer owns the
    // deterministic offer rule; the larger node sends a hello so it can still
    // receive the smaller node's relayed offer when announcements arrive in a
    ...
    if (announcementChanged) {
      replayPeerDirectoryToAll();
      schedulePeerDirectoryReplay();
    }
...
function replayPeerDirectoryToAll() {
  for (const peer of connectedProtocols()) replayPeerDirectory(peer.nodeId);
}
function schedulePeerDirectoryReplay() {
  if (!activeNode || peerDirectoryReplayTimer) return;
  peerDirectoryReplayTimer = window.setTimeout(() => {
    peerDirectoryReplayTimer = null;
    replayPeerDirectoryToAll();
  }, PEER_DIRECTORY_REPLAY_DEBOUNCE_MS);
}
...
export async function activateNode(deviceIdentity, vaultId, capacityBytes, iceServers = [], locationHint = null, { transient = false, transientCacheBytes = 0, fallbackIceLoader = null } = {}) {
  ...
  configurePeers({
    ...
    onProtocolConnected: async (peer) => {
      ...
      flushRelayedSignalsForPeer(peer.nodeId);
...
export async function pauseNode() {
  ...
  if (peerDirectoryReplayTimer) clearTimeout(peerDirectoryReplayTimer); peerDirectoryReplayTimer = null;
