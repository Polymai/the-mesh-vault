...
import {
  COORDINATION_MODES, evaluateCoordination, heartbeatIntervalMs, isHealthyAnchor,
...
async function refreshCoordinationMode() {
  ...
  const needsRealtime = anchor.enabled || [COORDINATION_MODES.SUPABASE_PRIMARY, COORDINATION_MODES.HYBRID].includes(next.mode);
...
async function handleGossipMessage(_peer, message) {
  if (message.kind === GOSSIP_KINDS.PEER_ANNOUNCE) {
    ...
    if (message.payload.protocolVersion === 3 && message.payload.capabilities?.includes("anchor-control-v3")) await publishControlObject("peer", "active", { ...message.payload, devicePublicKey: message.senderPublicKey, observedAt: message.timestamp, expiresAt: message.expiresAt }, Math.max(10000, message.expiresAt - Date.now())).catch(() => {});
  }
...
async function announcePresence() {
  ...
  broadcastGossip(await buildGossipMessage(deviceSigner, GOSSIP_KINDS.PEER_ANNOUNCE, {
    ...
    capabilities: anchorCapabilities(), capacityBytes: Number(activeNode.capacity_bytes) || 0,
...
function connectPresenceEntries(entries, ownNodeId) {
  ...
  for (const entry of [...selected, ...anchorAssignments.reserve]) {
    ...
    if (!reserveIds.has(nodeId)) connectPeer(nodeId).catch(() => {});
  }
...
export async function activateNode(deviceIdentity, vaultId, capacityBytes, iceServers = [], locationHint = null, { transient = false, transientCacheBytes = 0 } = {}) {
  ...
  configurePeers({
    fromNodeId: data.id, iceServers, onState: (nodeId, status) => { window.dispatchEvent(new CustomEvent("meshvault:peer-state", { detail: { nodeId, status } })); refreshCoordinationMode().catch(() => {}); announceTopology().catch(() => {}); announcePresence().catch(() => {}); replicatePendingControlObjects().catch(() => {}); },
    protocolHandlers: {
...
export async function heartbeatNode() {
  ...
  if (!isProviderUp(supabaseBootstrapProvider.name)) connectSupabaseInBackground(activeNode.id);
  await refreshCoordinationMode();
  const estimate = await storageEstimate();
  ...
  if (anchor.enabled) coreValues.lease_expires_at = new Date(now.getTime() + (window.__DATA__?.survival?.leaseMs || 90000)).toISOString();
  ...
  const writeBackbone = shouldWriteSupabaseHeartbeat(
  ...
  if (writeBackbone) {
    ...
    try {
      let result = await appTable("nodes").update({ ...coreValues, ...coordinationValues }).eq("id", activeNode.id);
  ...
  await Promise.all([publishPresence().catch(() => {}), measureConnectedPeers().catch(() => []), replicatePendingControlObjects().catch(() => 0)]);
...
export async function updateNodeSurvivalTelemetry(telemetry = {}) {
  ...
  try {
    ...
    // Keep the node reachable if optional telemetry fields are temporarily
    // unavailable: the minimal v3 heartbeat still refreshes its live status.
