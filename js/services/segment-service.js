import {
  loadVaultCatalog, mutateCatalogRow, removeCatalogRow, upsertCatalogRow,
...
export async function mutateContentSegment(vaultId, segmentId, mutate) {
  return mutateCatalogRow("segments", segmentId, (segment) => {
    if (segment.vault_id !== vaultId) return null;
    const next = mutate(clone(segment));
    if (!next || typeof next?.then === "function") throw new Error("Segment catalog mutations must be synchronous.");
    return clone(next);
  });
}
...
export async function recordShardPlacement({ vaultId, shardId, node, role = "durable", replacementForPlacementId = null, status = "verified" }) {
  ...
  let stored = null;
  await mutateCatalogRow("segments", segment.id, (current) => {
    if (current.vault_id !== vaultId) return null;
    const shards = (current.segment_shards || []).map((shard) => {
    ...
    return { ...current, segment_shards: shards };
  });
  if (!stored) throw new Error("The shard placement could not be recorded atomically.");
  return clone(stored);
}
...
export async function transitionShardPlacement(vaultId, placementId, status) {
  ...
  await mutateCatalogRow("segments", segment.id, (current) => {
    if (current.vault_id !== vaultId) return null;
    const shards = current.segment_shards.map((shard) => ({ ...shard, shard_placements: (shard.shard_placements || []).map((placement) => {
      if (placement.id !== placementId) return placement;
    ...
    return stored ? { ...current, segment_shards: shards } : null;
  });
  if (!stored) throw new Error(`Shard placement could not transition to ${status}.`);
  return clone(stored);
}
