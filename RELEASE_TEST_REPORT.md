...
## 2026-07-31 five-phase source-candidate addendum

The current source adds a self-verifying encrypted vault catalog, a transaction-safe signed local operation queue, one Local → Anchor → Supabase control repository, signed distributed repair/deletion coordination, and lower hosted heartbeat cadence in stable Anchor-primary mode. Focused browser, cryptographic, shard-lifecycle, 10,000-node model, schema, documentation and app-contract checks pass.

Supabase remains the operational backbone for cold first contact, anonymous coordination sessions, initial file ingestion, durable segment/placement rows and atomic repair mutation. Anchors can now retain and exchange validated catalog/control state and reduce steady-state hosted traffic, but they do not make a brand-new isolated browser independent of every bootstrap service.

This addendum is a source-candidate result, not a production deployment result. The revised schema, RLS and `publish_vault_index` RPC require Polymai provisioning. Physical-phone suspension, carrier/TURN traversal and real provider capacity remain release gates, so the existing conditional pilot decision remains unchanged.

## 2026-07-28 source-candidate addendum

The current source now selects three primary plus two reserve Anchors, prefers the Anchor path after a measured stability window, falls back to Supabase automatically, uses bounded peer discovery and recipient-scoped signaling, and carries signed repair advisories through the mesh. A deterministic 10,000-client/200-Anchor simulation, the 1,000-node shard-chaos model and focused static/browser contracts pass.

Identity startup is now serialized, node labels survive an activation race, and a complete version-2 Mesh Key file opens the same vault locally before hosted coordination finishes. This is not a new production deployment result. The revised schema, policies and RPCs still require Polymai provisioning; fresh phrase-only recovery and a live multi-browser pass were blocked by the project's anonymous-signup rate window after repeated UAT. No unrestricted-release claim is added. The decision and physical-phone gate below remain authoritative.
