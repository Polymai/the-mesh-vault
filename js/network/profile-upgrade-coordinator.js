...
import { createPlacementContext, rankPlacementCandidates, recordPlacementChoice } from "./placement-engine.js";
...
async function placeGeneration({ identity, node, peers, candidates, segment, generation, profile, cipher }) {
  ...
  let physicalBytes = 0;
  for (let index = 0; index < shardRows.length; index += 1) {
    const shard = shardRows[index]; const payload = payloads[index]; let target = null; let role = "cache"; let stored = false;
    for (const candidate of rankPlacementCandidates(candidates, { context: placementContext, shardBytes: payload.bytes.byteLength })) {
      try {
        if (candidate.isLocal) await storeLocalVerified(shard.shard_hash, payload.bytes);
        else await candidate.peer.protocol.sendShard(shard.shard_hash, payload.bytes, payload.hash);
        target = candidate; stored = true; role = "durable"; recordPlacementChoice(placementContext, candidate); break;
      } catch {}
    }
    if (!stored) {
      ...
      if (role === "durable") recordPlacementChoice(placementContext, node);
...
export async function upgradeOneCodingProfile() {
  ...
  try {
    ...
    placed = await placeGeneration({ identity, node, peers, candidates, segment, generation, profile, cipher });
