...
async function configureContractSurvival(page) {
  await page.evaluate(async () => {
    const { configureSurvivalService } = await import("/js/network/survival-service.js");
    await configureSurvivalService({
      nodeId: "night-node-browser-check",
      getPeers: () => [],
      peerInfo: () => null,
      accessToken: async () => "browser-contract-token",
      renewLease: async () => {},
    });
  });
}

async function waitForDeviceStartup(page) {
  await page.waitForFunction(async () => {
    const { getState } = await import("/js/state/store.js");
    const state = getState();
    return state.survival?.wakeAssistance !== "unknown"
      || /could not join the mesh/i.test(state.notice?.message || "");
...
async function main() {
  ...
  try {
    ...
    assert.equal(await page.locator("[name='capacity']").getAttribute("type"), "number", "Shared storage must use a manually editable number field.");
    assert.equal(await page.locator("[name='capacity']").getAttribute("max"), "2000", "Settings does not expose the configured 2,000 GB contribution ceiling.");
    await page.locator("[name='capacity']").fill("800");
    await page.locator("[data-capacity-form] button[type='submit']").click();
    const storedMaximum = await page.evaluate(async () => (await import("/js/services/vault-service.js")).getContributionLimit());
    assert.equal(storedMaximum, 800 * 1024 ** 3, "An 800 GB Patient Zero contribution does not persist.");
    // Keep this browser contract independent of hosted anonymous-auth rate
    // windows. The UI and push flow are exercised against the intercepted Edge
    // boundary below, while live authentication is covered by the Supabase
    // contract and release UAT suites.
    await waitForDeviceStartup(page);
    await configureContractSurvival(page);
    ...
    await reminderPanel.getByRole("button", { name: "Enable", exact: true }).click({ force: true });
    ...
    // The Settings view refreshes when reminder state changes. Invoke the
    // current controls synchronously so a legitimate rerender cannot detach a
    // button between Playwright's actionability check and the click.
    await page.evaluate(() => document.querySelector('[data-wake-assistance-action="enable"]')?.click());
    await page.waitForFunction(async () => (await import("/js/state/store.js")).getState().survival?.wakeAssistance === "ready");
    await page.evaluate(() => document.querySelector('[data-wake-assistance-action="disable"]')?.click());
    await page.waitForFunction(async () => (await import("/js/state/store.js")).getState().survival?.wakeAssistance === "available");
    const expectedAppUrl = await page.evaluate(() => {
      const url = new URL(window.__POLYMAI_SUPABASE_CONFIG__.siteUrl);
      url.hash = "/home";
      return url.href;
    });
    ...
    await page.evaluate(async () => {
      window.__notificationPermission = "granted";
      const { disableWakeAssistance } = await import("/js/network/survival-service.js");
      await disableWakeAssistance();
      window.__notificationPermission = "denied";
    });
    await page.locator(".wake-assistance-panel").getByRole("button", { name: "Enable", exact: true }).click({ force: true });
    ...
    await page.getByRole("heading", { name: "Backbone", exact: true }).waitFor();
    await page.locator(".backbone-section-stack").waitFor();
    await page.setViewportSize({ width: 390, height: 844 });
    ...
    await page.setViewportSize({ width: 1280, height: 800 });
    await page.locator(".anchor-device-settings > summary").click();
    ...
    await page.waitForFunction(() => document.querySelector("[data-night-lease]")?.textContent !== "expired", null, { timeout: 15000 });
    await page.goto("http://127.0.0.1:3007/#/settings");
    assert.equal(await page.locator(".anchor-role-badge").count(), 0, "The normal shell still exposes the internal Backbone role.");
    ...
    await page.locator(".wake-assistance-panel").getByRole("button", { name: "Disable", exact: true }).click({ force: true });
    await page.locator(".wake-assistance-panel").getByRole("button", { name: "Enable", exact: true }).waitFor();
    const anchorAfterReminderDisable = await page.evaluate(async () => {
      const [{ getAnchorSettings }, { getState }] = await Promise.all([
        import("/js/services/vault-service.js"),
        import("/js/state/store.js"),
      ]);
      return {
        persisted: await getAnchorSettings(),
        rendered: getState().anchor,
      };
    });
    assert(anchorAfterReminderDisable.persisted.enabled && anchorAfterReminderDisable.rendered.enabled,
      `Disabling optional reminders stopped the active Anchor: ${JSON.stringify(anchorAfterReminderDisable)}`);
    await page.evaluate(async () => { (await import("/js/router.js")).renderApp(true); });
    assert.equal(await page.locator(".anchor-role-badge").count(), 0,
      `The normal shell exposed the internal Backbone role after disabling reminders: ${JSON.stringify(anchorAfterReminderDisable)}`);
    ...
    await page.evaluate(async () => { (await import("/js/router.js")).renderApp(true); });
    assert.equal(await page.locator(".anchor-role-badge").count(), 0, "The normal shell exposed the internal Backbone role after reminder loss.");
