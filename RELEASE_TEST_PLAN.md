# TheMeshVault v0.1 Release Test Plan

## Objective

Establish whether the provisioned TheMeshVault application is safe and functional enough for a first public release. Tests must use real app code and real Supabase coordination. Stubs are allowed only for deterministic unit checks and must never be reported as end-to-end proof.
...
- Provisioned Supabase project configured by `data/supabaseConfig.js`.
...
| B08 | P1 | Trusted-device view | Current trusted-device state renders without granting unverified devices authority. |
...
| C05 | P0 | Manifest integrity | Manifest v2 signature, hash, segment metadata, coding generation, nonces, shard hashes, and placement records verify. |
...
| D09 | P0 | Current profiles | Five nodes select 3+2, ten select 7+3, sixteen standard select 12+4, and sixteen high-resilience select 10+6. |
...
| G02 | P1 | Authenticated navigation | Dashboard, Drive, Mesh, Recovery, Settings, Trusted devices, Account, and Share routes render without console/page errors. |
