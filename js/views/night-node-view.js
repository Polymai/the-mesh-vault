...
export function renderNightNodeView(state) {
  ...
  return `<section class="night-node ${active ? "is-running" : ""}" aria-labelledby="night-node-title">
    ...
    <div class="night-node__core">
      <span class="eyebrow">${active ? "Anchor lease active" : "Ready when you are"}</span>
      <h1 id="night-node-title">${active ? "Keeping this node useful." : "Turn this device into an anchor."}</h1>
      <p>${active ? "The screen stays deliberately dark while the browser keeps storage, discovery and control data available." : "Best for an old phone left plugged in. The browser still controls background suspension, so TheMeshVault measures the lease instead of promising permanent uptime."}</p>
      <button class="night-node__power" type="button" data-night-toggle data-enabled="${active}">
        ...
        <small>${active ? "Release Wake Lock and expire the lease" : "Request fullscreen, Wake Lock and persistent storage"}</small>
      </button>
    </div>
