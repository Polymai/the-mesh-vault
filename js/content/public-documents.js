...
export const LEGAL_VERSIONS = Object.freeze({
  terms: "2026-08-05.1-draft",
  privacy: "2026-08-05.1-draft",
});
...
const architecture = {
  ...
  sections: [
    ...
    },
    {
      ...
      title: "Android background node",
      body: `<p>The optional Android build uses the same hosted vault interface, but adds a separate native foreground service for storage and Anchor duties. The WebView remains the authorised vault: it creates or restores the Mesh Key, encrypts readable files, derives segment keys, decrypts downloads and signs owner operations. The native worker is deliberately less trusted and does not receive those secrets.</p>
      ${nativeBackgroundProcessDiagram}
    ...
    },
    {
      ...
      body: `<figure class="architecture-figure architecture-figure--network" aria-labelledby="network-flow-title">
        ...
        <div class="network-diagram">
          ...
          <article class="network-node network-node--coordination"><span>Control mesh + fallback</span><strong>Participants, Backbones and Supabase</strong><small>Every online node can relay bounded signed or encrypted control data. Pinned WSS Backbones provide stable first contact; Supabase remains the durable metadata and repair fallback.</small></article>
        ...
        <figcaption>Discovery is mesh-first. A verified configured WSS Backbone can bootstrap a browser without Supabase, but durable file/placement records, anonymous-session compatibility and atomic repair still use Supabase in this release.</figcaption>
      </figure>
    ...
    },
    {
      ...
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-grid">
          ...
          <article class="backbone-card backbone-card--fallback"><span>Distributed active path</span><strong>Mesh participants and Backbones</strong><ul><li>Every online node retains a small bounded control cache</li><li>Stable opt-in Backbones retain and serve more records</li><li>Pinned WSS Backbones can provide verified cold first contact</li><li>Signals, manifests, repair and deletion records travel peer-to-peer</li></ul><b>Preferred first. Backbones do not become vault authorities.</b></article>
        </div>
        <div class="backbone-outage"><span><b>Normal path</b>Known peers, mesh participants, verified Backbones and invites are tried before Supabase. Two participants or one Backbone are enough to keep the control path mesh-first.</span><span><b>Cold start</b>A browser with a configured, reachable and public-key-pinned WSS Backbone can discover control context without Supabase. The current Android native worker still uses Supabase for its own first contact and signaling.</span><span><b>Current hard limits</b>New file ingestion, durable segment/placement records and the atomic repair job remain Supabase-backed. A standalone Backbone stores real opaque shards, but placement receipts still require sparse Supabase registration.</span></div>
      ...
      <p><strong>Cold-start boundary:</strong> entering the 24 words reconstructs the same identity and key hierarchy locally; no secret-key lookup occurs. Network discovery is separate. A reachable public-key-pinned WSS Backbone can bootstrap browser discovery, otherwise the delayed Supabase fallback is used. The configured seed list is deployment data and may initially be empty.</p>`,
    },
    {
      id: "identity",
    ...
    },
    {
      ...
      body: `<ul>
        ...
        <li>Verified WSS Backbones can bootstrap browser control discovery, but this release still depends on Supabase for durable metadata, placement transactions, atomic repair and Android native first contact. Supabase independence is therefore partial, not complete.</li>
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      id: "service",
      title: "2. The service",
      body: `<p>TheMeshVault is a no-account, cryptographically owned peer-to-peer storage protocol and application. It encrypts files locally and distributes recovery shards across participating devices. Every online node helps exchange bounded control records; stable opt-in Backbones can provide verified first contact and larger caches. Supabase remains the durable fallback for pseudonymous sessions, metadata, placement, repair and deletion transactions. None of these control roles are intended to hold readable files, encrypted shard payloads or secret vault keys.</p>
      <p>It is not a guaranteed backup service. Recoverability depends on verified independent placements, reachable devices and possession of the Mesh Key. A public-key-pinned WSS Backbone can bootstrap a browser without Supabase, but durable storage transactions and the current Android native first-contact path still depend on Supabase.</p>`,
    },
    {
      id: "eligibility",
      title: "3. Eligibility",
...
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>Supabase is currently used to make the service function, not for analytics: anonymous-session compatibility, durable segment and placement writes, delayed discovery/signaling fallback and the atomic repair queue depend on it. Browser discovery first tries local context, connected participants, public-key-pinned WSS Backbones and invites. Signed catalog operations, encrypted indexes, repair notices and deletion orders can travel through that mesh and later reconcile with Supabase. A configured reachable Backbone can cold-start a browser control path; the current Android native worker still uses Supabase first contact. Backbones retain a sparse Supabase compatibility lease at an approximately ten-minute cadence. Neither Supabase nor mesh control caches receive shard payloads, the root or derived private keys.</p>
