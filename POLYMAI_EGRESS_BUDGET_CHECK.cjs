...
assert.match(runtime, /liveMeshRefreshMs:\s*120000/);
...
assert.match(provider, /PRESENCE_CACHE_MS\s*=\s*60000/);
...
assert.ok(hybridReduction > 0.90, "Anchor-assisted control requests should fall by more than 90% in the timer model");
...
console.log(`PASS egress budget: local-only check; modeled Anchor-assisted requests -${(hybridReduction * 100).toFixed(1)}%, cold-start requests -${((1 - newColdStartRequestsPerNodeHour / oldRequestsPerNodeHour) * 100).toFixed(1)}%, initial operation payload -${(operationInitialPayloadReduction * 100).toFixed(1)}%. No network requests executed.`);
