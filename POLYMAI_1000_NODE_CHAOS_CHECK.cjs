...
// Scale evidence is an in-memory model. It must never spend hosted quota.
global.fetch = async () => { throw new Error("External network access is forbidden in the 1,000-node chaos check."); };
global.WebSocket = class BlockedWebSocket { constructor() { throw new Error("External network access is forbidden in the 1,000-node chaos check."); } };
