...
async function announceRepairNeed(event) {
  ...
  await publishControlObject("repair-request", "open", payload, 1800000).catch(() => {});
...
async function announcePresence() {
  ...
  broadcastGossip(await buildGossipMessage(deviceSigner, GOSSIP_KINDS.PEER_ANNOUNCE, {
    nodeId: activeNode.id, nodeName: activeNode.device_label, protocolVersion: 1,
    ...
    anchorProtocolVersion: anchor.enabled ? Number(window.__DATA__?.anchor?.protocolVersion || 2) : 0,
...
export async function activateNode(deviceIdentity, vaultId, capacityBytes, iceServers = [], locationHint = null, { transient = false, transientCacheBytes = 0 } = {}) {
  ...
  const protocolValues = { coordination_protocol_version: 2 };
  ...
  if (error && /coordination_protocol_version/i.test(`${error.message || ""} ${error.details || ""}`)) ({ data, error } = await persist({ ...baseValues, ...locationValues }).catch((persistError) => ({ data: null, error: persistError })));
...
export async function heartbeatNode() {
  ...
  const coordinationValues = {
    ...
    anchor_protocol_version: anchor.enabled ? Number(window.__DATA__?.anchor?.protocolVersion || 1) : 0,
