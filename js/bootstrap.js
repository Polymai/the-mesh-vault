...
import { listFiles, listPendingFileDeletions, listFolders, createFolder, moveFileRecord } from "./services/metadata-service.js";
...
import { appConfig, currentAccessToken, ensureAnonymousSession, supabase, supabaseFetch } from "./services/supabase.js";
...
import { activateNode, setNodePaused, updateNodeCapacity, updateNodeLabel, heartbeatNode, refreshNodePeers, updateNodeSurvivalTelemetry, clearLocalNodeStorage, pauseNode, exportPatientZeroPublicSetup } from "./network/node-service.js";
...
const NETWORK_OVERVIEW_REFRESH_MS = Number(coordinationConfig().networkOverviewRefreshMs || 300000);
...
function observedMeshOverview(state = getState()) {
  ...
  const local = state.node?.id ? {
    ...
    capacity_bytes: Number(state.node.capacityBytes || 0),
    used_bytes: Number(state.node.usedBytes || 0),
...
async function refreshLiveMesh({ includeOverview = false } = {}) {
  ...
  liveMeshRefreshPromise = (async () => {
    ...
    if (connectedProtocols().length) {
      const network = observedMeshOverview();
      setState((state) => ({ nodes: network.nodes, network: { ...state.network, ...network, connectedPeerCount: connectedProtocols().length, bootstrapUp: providerStatuses().supabase?.up !== false, status: computeNetworkStatus({ connectedPeerCount: connectedProtocols().length, bootstrapUp: providerStatuses().supabase?.up !== false, peerDiscoveryUp: true }) } }));
      return;
    }
    ...
    const network = await loadNetworkOverview({ includeNodes: true }).catch(() => null);
    if (!network) return;
...
async function refreshPrivateData({ includeOverview = false } = {}) {
  ...
  const meshRouteAvailable = connectedProtocols().length > 0;
  const shouldLoadOverview = !meshRouteAvailable && (includeOverview || state.route === "mesh");
  const [files, pendingDeletions, folders, network, log] = await Promise.all([
    ...
    meshRouteAvailable ? observedMeshOverview(state) : shouldLoadOverview ? loadNetworkOverview({ includeNodes: state.route === "mesh" }).catch(() => emptyNetwork) : emptyNetwork,
...
async function syncVaultCoordination(identity, { force = false } = {}) {
  ...
  if (!force && Date.now() - lastCoordinationSyncAt < 60000) return false;
  coordinationSyncPromise = (async () => {
    // Give real WebRTC/Backbone routes the configured cold-start window.
    // A local vault renders immediately while discovery runs.
    const delay = Math.max(0, Number(coordinationConfig().supabaseColdStartDelayMs || 5000));
    if (!force && delay) await new Promise((resolve) => window.setTimeout(resolve, delay));
    const meshRouteAvailable = connectedProtocols().length > 0;
    if (!meshRouteAvailable) {
      await ensureAnonymousSession();
      await ensureVaultRegistration(identity);
    }
    const [catalog, _operations, remoteProfile] = await Promise.all([
      loadVaultCatalog({ includeSupabase: !meshRouteAvailable }).catch(() => null),
      loadReplicatedOperations({ force, includeSupabase: !meshRouteAvailable }).catch(() => null),
      meshRouteAvailable ? getVaultProfile(identity.vaultId).catch(() => null) : getVaultProfile(identity.vaultId, identity).catch(() => null),
    ]);
    if (!catalog) {
      await publishVaultCatalog({ folders: [], files: [], tombstones: [], repairState: [] }, { forceSupabase: !meshRouteAvailable }).catch(() => {});
    }
    await flushControlPlane().catch(() => {});
    if (remoteProfile && getState().identity?.vaultId === identity.vaultId) setState({ vaultProfile: remoteProfile });
    lastCoordinationSyncAt = Date.now();
    await refreshPrivateData({ includeOverview: !meshRouteAvailable && ["dashboard", "mesh"].includes(getState().route) });
    return true;
...
async function startDevice(deviceIdentity, vaultId) {
  ...
  try {
    // Start with public STUN and the encrypted mesh rendezvous. TURN credentials
    // are loaded only if that mesh-first attempt fails and Supabase fallback is
    // actually opened.
    ...
    startRecoveryCoordinator({
      ...
      runWhenReady: connectedProtocols().length > 0,
    });
...
const actions = {
  ...
  settings: {
    ...
    async copyPatientZeroConfig() {
      const setup = await exportPatientZeroPublicSetup();
      const relayUrls = [...new Set((setup.relayUrls || []).map(String).filter((value) => value.startsWith("wss://")))];
      const relaySource = JSON.stringify(relayUrls, null, 2).replace(/^/gm, "    ").trimStart();
      const source = `(function () {\n  // Public Patient Zero trust and relay addresses only. No private key is included.\n  window.__POLYMAI_RENDEZVOUS_CONFIG__ = Object.freeze({\n    enabled: true,\n    relayUrls: Object.freeze(${relaySource}),\n    authorityPublicKeys: Object.freeze([\n      ${JSON.stringify(setup.authorityPublicKey)}\n    ]),\n    staticResponders: Object.freeze([\n      Object.freeze({ authorityPublicKey: ${JSON.stringify(setup.authorityPublicKey)}, transportPublicKey: ${JSON.stringify(setup.transportPublicKey)} })\n    ])\n  });\n})();\n`;
    ...
    async anchor({ enabled, nodeName, cacheLimitBytes }) {
      ...
      await heartbeatNode({ forceSupabase: true }).catch(() => {});
...
export async function bootstrap() {
  ...
  window.addEventListener("meshvault:rendezvous-status", (event) => updateNested("anchor", { distributedRendezvous: event.detail }));
