...
function validPeerPayload(payload) {
  return payload && typeof payload.nodeId === "string" && payload.nodeId && Number(payload.expiresAt || 0) > Date.now();
}
...
export const anchorPeerBootstrapProvider = {
  ...
  async discoverPeers() {
    ...
    return rows.map((row) => row.payload).filter(validPeerPayload).map((peer) => ({
      peerId: peer.nodeId, nodeName: peer.nodeName || null, devicePublicKey: peer.devicePublicKey || null, protocolVersion: peer.protocolVersion || 1,
  ...
  async sendSignal(signal) { return relaySignalThroughAnchors(signal) > 0; },
