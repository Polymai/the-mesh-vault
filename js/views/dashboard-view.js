...
export function renderDashboardView(state) {
  ...
  const capacitySnapshots = state.network.snapshots || [];
  ...
  return `<section class="view-head view-head--compact dashboard-head"><div><h1>${state.files.length ? "Vault overview" : "Your vault is ready."}</h1></div><a class="button" href="#/drive">Upload files</a></section>
  <div class="metric-grid">
    <article class="metric-card metric-card--accent"><span>Your files</span><strong>${bytes(totalFileBytes)}</strong><small>${healthy}/${state.files.length} recoverable</small></article>
    <article class="metric-card"><span>Mesh storage</span><strong>${bytes(storage.physicalBytes || 0)}</strong><small>${storage.physicalShardCopies || 0} shard copies</small></article>
    <article class="metric-card"><span>Live nodes</span><strong>${liveNodeCount}</strong><small>${bytes(meshUsed)} used of ${bytes(meshCapacity)}</small></article>
  ...
  <section class="panel mesh-capacity-panel">
    <div class="panel-head mesh-capacity-head"><h2>Mesh capacity</h2><span class="live-label"><i></i>${liveNodeCount} live ${liveNodeCount === 1 ? "node" : "nodes"}</span></div>
    <div class="mesh-capacity-total"><strong>${bytes(meshCapacity)}</strong><span>${meshCapacity ? "available now" : "No capacity reported"}</span></div>
    ...
    ${capacitySnapshots.length ? `<div class="mesh-capacity-timeline"><div><strong>Past 24 hours</strong></div>${capacityChart(capacitySnapshots, { compact: true })}</div>` : ""}
  </section>
  ${state.files.length ? `<section class="panel overview-storage"><div class="panel-head"><h2>Storage details</h2><a href="#/drive">Open files</a></div>
  ...
  ${activity ? `<section class="panel"><div class="panel-head"><h2>Activity</h2><a href="#/mesh">Open mesh</a></div>${activity}</section>` : ""}
