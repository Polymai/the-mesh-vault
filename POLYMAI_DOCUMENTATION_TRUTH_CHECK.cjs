...
has(controlRepository, 'queryAnchors("vault-index"', "The documented local/mesh index path is missing.");
has(controlRepository, 'markSyncReceipt(operation.opId, "anchor"', "The documented Anchor operation receipt is missing.");
lacks(controlRepository, 'markSyncReceipt(operation.opId, "supabase"', "The control repository still records a continuing Supabase operation receipt.");
...
has(docs, "Encrypted vault indexes, operations, names, file sizes, coding parameters, deduplication tokens, shard hashes and placements, recovery/repair/deletion state, manifests, capabilities and audit objects stay local or move through authenticated mesh peers.", "Privacy page does not state where private vault coordination data now lives.");
...
has(docs, "IP address, user-agent, timing and security-log data may also be processed", "Supabase infrastructure-log disclosure is missing.");
has(docs, "The temporary row is deleted best-effort", "Supabase temporary-row lifecycle is not disclosed.");
has(docs, "Random IDs and public keys do not encode a name", "Stable identifiers are not distinguished from direct identity.");
...
has(docs, "exact device whose Ed25519 key is pinned as Patient Zero", "Security omits the verified Patient Zero cold-start path.");
has(docs, "Files, shards, manifests and placements are never written to Supabase", "Security overstates Supabase independence for storage transactions.");
has(docs, "Encrypted vault indexes", "Security and Privacy omit the current mesh-held vault-index data.");
has(docs, "Files, manifests, placements, sharing, deletion, repair and Live Mesh then remain in the peer mesh.", "Security overstates post-bootstrap hosted dependence.");
has(docs, "sparse heartbeat", "Security does not disclose the Patient Zero Supabase lease cadence.");
...
has(docs, "the notification cannot run WebRTC by itself", "Security overstates what an optional reminder can do.");
...
has(docs, "resumes its local heartbeat and reuses recently discovered peers", "Security documentation omits installed-PWA foreground refresh behavior.");
