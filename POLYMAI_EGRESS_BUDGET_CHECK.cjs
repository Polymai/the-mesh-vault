...
assert.doesNotMatch(nodeService, /scheduleOrdinaryBootstrapRetry|bootstrapReconnect|bootstrapRetryCount/);
assert.match(nodeService, /const writeBackbone = activeNode\._supabaseRegistered[\s\S]{0,160}&& patientZeroListener/);
assert.doesNotMatch(nodeService, /const writeBackbone[\s\S]{0,400}supabaseFallbackOpened/);
...
assert.match(bootstrap, /let deviceStartPromise = null/);
assert.match(bootstrap, /if \(deviceStartPromise && deviceStartKey === key\) return deviceStartPromise/);
...
assert.match(fallbackPolicy, /fallbackAttemptedThisLifecycle/);
assert.match(fallbackPolicy, /if \(!force && fallbackAttemptedThisLifecycle\) return false/);
assert.match(fallbackPolicy, /withPatientZeroAuthorityRead/);
assert.match(authorityChain, /withPatientZeroAuthorityRead\(async \(\) =>/);
assert.match(authorityChain, /await ensureAnonymousSession\(\)/);
assert.match(nodeService, /const needsRealtime = supabaseFallbackOpened &&/);
assert.doesNotMatch(nodeService, /const needsRealtime = supabaseBootstrapState\(\)\.patientZeroListener\s*\|\|/);
