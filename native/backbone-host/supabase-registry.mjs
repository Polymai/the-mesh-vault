...
export class SupabaseNodeRegistry {
  ...
  rows(status = "online") {
    ...
    const remotelyReachable = /^wss:\/\//i.test(publicUrl);
    return this.layout.instances.map((instance) => {
      const instanceOnline = status === "online" && this.store?.instanceState?.(instance.id)?.enabled !== false;
      return ({
      id: instance.id,
      supabase_auth_id: this.session.user.id,
      vault_id: null,
      device_public_key: this.identity.publicKey,
      device_label: instance.label || "Backbone",
      status: instanceOnline ? "online" : "offline",
      capacity_bytes: remotelyReachable && instanceOnline ? instance.quotaBytes : 0,
      // A host without a public WSS endpoint is a local control cache, not a
      // remotely usable storage target. Keep capacity and usage aligned so it
      // cannot inflate mesh capacity or violate used_bytes <= capacity_bytes.
      used_bytes: remotelyReachable && instanceOnline
        ? Math.min(instance.quotaBytes, Number(this.store?.usedBytes?.(instance) || instance.usedBytes || 0))
        : 0,
      last_seen_at: iso(now),
      country_code: null,
      region_code: null,
      network_domain_hash: null,
      failure_domain_id: instance.failureDomainId,
      survival_mode: true,
      ...
      backbone_url: remotelyReachable ? publicUrl : null,
      backbone_public_key: remotelyReachable ? this.identity.publicKey : null,
      });
    });
  }
  ...
  async stop() {
    ...
    this.timer = null;
    if (this.enabled) await this.upsert("offline").catch(() => {});
  }
}
