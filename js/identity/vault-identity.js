...
export async function loadOrCreateVaultIdentity() {
  ...
  let created = false;
  if (!record) { record = await createRecord(); created = true; }
...
export function exportMeshKeyFile() {
  ...
  return new Blob([JSON.stringify(cached._record.recoveryEnvelope, null, 2)], { type: "application/json" });
}
...
async function restoreFromEnvelope(phrase, envelope) {
  ...
  const record = {
    ...
    ownerPublicKeyRaw: bytesToBase64Url(await exportPublicKeyRaw(await importPublicKeyJwk(decoded.ownerPublicKeyJwk, decoded.algorithm))),
...
export async function restoreVaultFromFile(fileJson, phrase) {
  const envelope = typeof fileJson === "string" ? JSON.parse(fileJson) : fileJson;
