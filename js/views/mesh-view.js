...
import { renderDataFlowPanel, bindDataFlowPanel } from "../ui/data-flow-panel.js";
...
function signatureNode(node = {}) {
  return [
    ...
    Number(node.anchor_protocol_version ?? node.anchorProtocolVersion) || 0,
  ];
}
...
export function renderMeshView(state) {
  ...
  const allNodes = scopedNodes.map((node) => ({
    ...
    is_anchor: node.id === currentRecord?.id
      ...
      : !!node.capabilities?.includes("anchor-control-v3"),
  }));
...
export function bindMeshView(actions) {
  ...
  unbindDataFlow?.(); unbindDataFlow = bindDataFlowPanel();
...
export function disposeMeshView() {
  ...
  unbindUniverse?.(); unbindUniverse = null;
  unbindGlobe?.(); unbindGlobe = null;
  unbindDataFlow?.(); unbindDataFlow = null;
  destroyDataGlobe();
