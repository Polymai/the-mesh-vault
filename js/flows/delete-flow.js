...
export async function deleteFile(fileId, { vaultIdentity } = {}) {
  ...
  try {
    ...
    // Owner-signed orders are copied to connected Anchors before the file is
    // hidden. Supabase remains the durable fallback, while an offline node can
    // receive and acknowledge the same order through the Anchor control path.
