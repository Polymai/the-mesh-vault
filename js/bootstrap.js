...
import {
  openSupabaseFallbackWindow, supabaseAutomaticFallbackEnabled,
  supabaseFallbackWindowOpen,
} from "./network/supabase-fallback-policy.js";
...
function meshBootstrapAvailable() {
  ...
  return hasVerifiedMeshRoute()
    || Number(status?.distributedRendezvous?.relays?.connected || 0) > 0
    || Number(status?.distributedRendezvous?.responders || 0) > 0;
}
...
async function syncVaultCoordination(identity, { force = false } = {}) {
  ...
  coordinationSyncPromise = (async () => {
    ...
    const delay = supabaseAutomaticFallbackEnabled()
      ? Math.max(0, Number(coordinationConfig().supabaseColdStartDelayMs || 60000))
      : 0;
    if (!force && delay) await new Promise((resolve) => window.setTimeout(resolve, delay));
    const meshRouteAvailable = hasVerifiedMeshRoute();
    const useSupabaseFallback = !meshRouteAvailable
      && (supabaseFallbackWindowOpen() || openSupabaseFallbackWindow({ force }));
    ...
    const [catalog, _operations, remoteProfile] = await Promise.all([
      loadVaultCatalog({ includeSupabase: useSupabaseFallback, includeAnchors: meshRouteAvailable }).catch(() => null),
      loadReplicatedOperations({ force, includeSupabase: useSupabaseFallback }).catch(() => null),
      useSupabaseFallback ? getVaultProfile(identity.vaultId, identity).catch(() => null) : getVaultProfile(identity.vaultId).catch(() => null),
    ]);
    if (!catalog) {
      await publishVaultCatalog({ folders: [], files: [], tombstones: [], repairState: [] }, { forceSupabase: useSupabaseFallback }).catch(() => {});
    }
  ...
  try {
    ...
    startRecoveryCoordinator({
      shouldRun: (reason) => {
        ...
        const eventDriven = ["file-changed", "deletion-changed", "profile-changed", "repair-needed", "repair-advisory"].includes(reason);
        return document.visibilityState !== "hidden"
          && !!state.identity?.vaultId
          && hasVaultWork
          && connectedProtocols().length > 0
          && (eventDriven || supabaseFallbackWindowOpen());
      },
