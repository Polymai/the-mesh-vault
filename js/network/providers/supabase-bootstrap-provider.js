...
import { normalizeSignalEnvelope } from "./signal-envelope.js";
...
function deliverBroadcast(payload) {
  const row = normalizeSignalEnvelope(payload);
  if (!row || row.toNodeId !== currentNodeId || !rememberSignal(row.signalId || row.id)) return;
  const signal = { signalId: row.signalId || row.id || null, fromNodeId: row.fromNodeId, toNodeId: row.toNodeId, type: row.type, payload: row.payload, sentAt: Number(row.sentAt) || Date.now() };
  ...
  // Patient Zero opens its addressed first-contact channel before the WebRTC
  // manager is configured. Keep that short startup gap lossless: otherwise an
  // offer can be acknowledged by Supabase and disappear before any listener
  // exists, leaving every client visible but permanently disconnected.
  if (!signalListeners.size) {
    pendingSignals.push(signal);
    if (pendingSignals.length > MAX_PENDING_SIGNALS) pendingSignals.splice(0, pendingSignals.length - MAX_PENDING_SIGNALS);
    return;
  }
...
export const supabaseBootstrapProvider = {
  ...
  async disconnectNode() {
    stopFallbacks(); currentNodeId = null; signalTopic = null; cachedPresenceEntries = []; cachedPresenceAt = 0; presencePromise = null; realtimeSignalObserved = false; seenSignalIds.clear(); pendingSignals.length = 0;
    await closeRealtimeChannel();
  },
  ...
  onSignal(handler) {
    ...
    // There is one peer-manager subscriber. Drain once, in arrival order, so
    // offers, ICE and answers received during startup retain their ordering.
