...
export async function buildDeletionAuthorization({ vaultId, fileId, manifestId = null, shardHash, nodeId }, signer) {
  ...
  const core = {
    protocolVersion: 3, vaultId, fileId, manifestId: manifestId || null,
...
export async function verifyDeletionAuthorization(authorization, { nodeId, shardHash } = {}) {
  if (!authorization || authorization.protocolVersion !== 3 || !authorization.vaultId || !authorization.fileId || !authorization.ownerPublicKey || !authorization.signature) return false;
