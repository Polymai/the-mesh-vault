...
export function isHealthyAnchor(candidate, now = Date.now()) {
  ...
  const advertised = capabilities.includes("anchor-control-v1") || candidate?.survivalMode === true || candidate?.survival_mode === true;
  if (!advertised || !candidateId(candidate)) return false;
...
export function evaluateCoordination(previous = {}, observation = {}, options = {}) {
  ...
  const enoughForPrimary = anchorCount >= config.targetAnchors;
  ...
  let mode;
  if (!supabaseUp) {
    mode = anchorCount >= config.minimumMeshAnchors ? COORDINATION_MODES.MESH_DEGRADED : COORDINATION_MODES.ISOLATED;
  } else if (enoughForPrimary && stableFor >= config.anchorStableMs) {
    mode = COORDINATION_MODES.ANCHOR_PRIMARY;
  } else if (anchorCount >= config.minimumMeshAnchors) {
...
export function providerOrder(mode, kind) {
  const anchorFirst = ["anchor-peer", "local-peer", "supabase", "invite-link"];
  const supabaseFirst = ["supabase", "anchor-peer", "local-peer", "invite-link"];
  const meshOnly = ["anchor-peer", "local-peer", "invite-link"];
  if (mode === COORDINATION_MODES.ANCHOR_PRIMARY || mode === COORDINATION_MODES.HYBRID) return anchorFirst;
