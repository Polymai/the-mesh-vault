...
public final class PolymaiAppBackgroundWorker implements PolymaiBackgroundWorker {
    ...
    static final long MAX_CONTRIBUTION_BYTES = 2000L * 1024L * 1024L * 1024L;
    ...
    static final class Configuration {
        ...
        final JSONArray bootstrapAuthorityKeys;
        final String label;
        ...
        Configuration(
            String supabaseUrl, String publishableKey, JSONArray bootstrapAuthorityKeys,
            ...
            this.bootstrapAuthorityKeys = bootstrapAuthorityKeys;
            this.label = label;
        ...
        static Configuration parse(JSONObject input) throws Exception {
            ...
            if (capacity > MAX_CONTRIBUTION_BYTES) throw new IllegalArgumentException("Native storage is limited to 2,000 GB.");
            ...
            JSONArray authorityKeys = input.optJSONArray("bootstrapAuthorityKeys");
            if (authorityKeys == null || authorityKeys.length() == 0 || authorityKeys.length() > 8) throw new IllegalArgumentException("A Patient Zero genesis or verified responder public key is required.");
            JSONArray validatedAuthorityKeys = new JSONArray();
            for (int index = 0; index < authorityKeys.length(); index++) {
                String key = authorityKeys.optString(index, "").trim();
                if (!key.matches("^[A-Za-z0-9_-]{43}$")) throw new IllegalArgumentException("A Patient Zero public key is malformed.");
                validatedAuthorityKeys.put(key);
            }
            return new Configuration(
                supabaseUrl, publishableKey, validatedAuthorityKeys, label, vaultId,
        ...
        JSONObject toJson() {
            ...
            putUnchecked(result, "bootstrapAuthorityKeys", bootstrapAuthorityKeys);
