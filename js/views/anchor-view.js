...
import { coordinationStatus, currentNode } from "../network/node-service.js";
import { patientZeroAuthorityState } from "../network/patient-zero-authority-chain.js";
...
const BACKBONE_PORT_SCAN_RADIUS = 4;
const backboneHostCache = new Map();

class LocalBackboneError extends Error {
  constructor(message, code = "unreachable", cause = null) {
    super(message, cause ? { cause } : undefined);
    this.name = "LocalBackboneError";
    this.code = code;
  }
}
...
async function loopbackPermissionState() {
  ...
  // Chrome 145 split Local Network Access into loopback-network and
  // local-network. The former alias remains compatible with older releases.
...
async function localBackboneRequest(port, path = "/status", options = {}, timeoutMs = 1400) {
  ...
  let timer = null;
  try {
    const request = fetch(`http://127.0.0.1:${port}${BACKBONE_MANAGEMENT_PATH}${path}`, {
      cache: "no-store",
      credentials: "omit",
      mode: "cors",
      // This annotation deliberately triggers Chromium's loopback-network
      // permission and mixed-content exception for the local manager.
      targetAddressSpace: "loopback",
      ...options,
      signal: controller.signal,
    }).catch(async (error) => {
      const permission = await loopbackPermissionState();
      if (permission === "denied") throw new LocalBackboneError("Local server access is blocked by the browser.", "permission-denied", error);
      throw new LocalBackboneError("The browser could not reach the local Backbone process.", permission === "prompt" ? "permission-prompt" : "unreachable", error);
    });
    const deadline = new Promise((_, reject) => {
      timer = setTimeout(() => {
        controller.abort();
        reject(new LocalBackboneError("The local Backbone connection timed out.", "permission-prompt"));
      }, timeoutMs);
    });
...
async function scanLocalBackbones({ force = false } = {}) {
  if (backbonePortScanPromise) return backbonePortScanPromise;
  if (!force && Date.now() - backbonePortScanAt < BACKBONE_PORT_SCAN_COOLDOWN_MS) return new Map();
  const ports = [...new Set(backboneLaunches().flatMap((launch) => {
    const hint = Math.max(BACKBONE_PORT_MIN, Math.min(BACKBONE_PORT_MAX, Number(launch.port) || BACKBONE_PORT_MIN));
    return Array.from({ length: BACKBONE_PORT_SCAN_RADIUS * 2 + 1 }, (_, index) => hint - BACKBONE_PORT_SCAN_RADIUS + index)
      .filter((port) => port >= BACKBONE_PORT_MIN && port <= BACKBONE_PORT_MAX);
...
async function locateLocalBackbone(launch, { interactive = false } = {}) {
  let directError = null;
  try {
    const status = await localBackboneRequest(launch.port, "/status", {}, interactive ? 5000 : 1800);
    if (!status.launcherHostId || status.launcherHostId === launch.hostId) return { launch, status };
  } catch (error) {
    directError = error;
    if (error?.code === "permission-denied" || error?.code === "permission-prompt") throw error;
  }
  ...
  const match = (await scanLocalBackbones({ force: interactive })).get(launch.hostId);
  if (match) {
    const corrected = { ...launch, port: match.port };
    saveBackboneLaunch(corrected);
    return { launch: corrected, status: match.status };
  }
  throw directError || new LocalBackboneError("Local Backbone host not found.");
}
...
function hostMarkup({ launch, status, checking = false, error = null }) {
  if (status?.ok) {
    const remote = !!status.remotelyReachable;
    return `<section class="backbone-host-status"><header><div><strong>${esc(status.label)}</strong><small>${status.instances.length} storage lane${status.instances.length === 1 ? "" : "s"} · one physical device</small></div><span class="status-pill status-pill--${remote ? "safe" : "neutral"}">${remote ? "Mesh reachable" : "Local host"}</span></header>${remote ? "" : `<p class="backbone-host-note">Running on this computer only. It joins the internet mesh after a public WSS address is configured.</p>`}<div>${status.instances.map((instance) => instanceMarkup(instance, launch.port)).join("")}</div><button class="text-button text-button--danger" type="button" data-backbone-remove="${esc(launch.hostId)}" data-port="${launch.port}">Stop and remove</button></section>`;
  }
  if (checking) return `<section class="backbone-host-status"><header><div><strong>${esc(launch.displayName)}</strong><small>Finding the local server process…</small></div><span class="status-pill status-pill--neutral">Checking</span></header><button class="text-button text-button--danger" type="button" data-backbone-remove="${esc(launch.hostId)}" data-port="${launch.port}">Remove from list</button></section>`;
  const permissionBlocked = error?.code === "permission-denied" || error?.code === "permission-prompt";
  const explanation = permissionBlocked
    ? "Allow TheMeshVault to connect to this computer, then try again."
    : `The browser did not find the local host near port ${launch.port}.`;
  return `<section class="backbone-host-status backbone-host-status--offline"><header><div><strong>${esc(launch.displayName)}</strong><small>${explanation}</small></div><span class="status-pill status-pill--neutral">${permissionBlocked ? "Permission needed" : "Not connected"}</span></header><div class="backbone-instance-actions"><button class="button button--ghost" type="button" data-backbone-connect="${esc(launch.hostId)}">${permissionBlocked ? "Allow local access" : "Try again"}</button><button class="button button--ghost" type="button" data-backbone-redownload="${esc(launch.hostId)}">Download start file</button><button class="text-button text-button--danger" type="button" data-backbone-remove="${esc(launch.hostId)}">Remove</button></div></section>`;
}
...
export function renderAnchorView(state) {
  ...
  const statusLabel = !anchor.enabled ? "Mesh participant" : nativeActive ? "Android Backbone" : anchor.status === "paused" ? "Paused" : "Browser Backbone";
  ...
  const publicConfigurationPublished = !!runtimeNode?.device_public_key
    && patientZeroOperatorEnabled(runtimeNode.device_public_key);
  const operatorSetup = patientZeroOperatorEnabled() || publicConfigurationPublished;
  const firstContact = coordinationStatus().supabaseRealtime || {};
  const firstContactOnline = publicConfigurationPublished && !!firstContact.connected;
  const publicationLabel = publicConfigurationPublished ? "Published" : "Not published";
  const listenerLabel = firstContactOnline
    ? "Online"
    : firstContact.registrationError ? "Needs attention" : "Connecting";
  const patientZeroLabel = firstContactOnline
    ? "Patient Zero online"
    : publicConfigurationPublished ? "Configuration published" : "Not published";
  const authority = patientZeroAuthorityState();
  const rendezvousMarkup = operatorSetup ? `<section class="panel settings-section patient-zero-panel">
    <div class="settings-section-head"><div><span class="eyebrow">Operator only</span><h2>Patient Zero setup</h2><p>This browser profile owns the private Patient Zero signing key.</p></div><span class="status-pill status-pill--${firstContactOnline || publicConfigurationPublished ? "safe" : "neutral"}">${patientZeroLabel}</span></div>
    <ol class="patient-zero-steps">
      <li><span>1</span><div><strong>Download the public setup</strong><small>Keep this public-only JSON with the deployment records.</small></div></li>
      <li><span>2</span><div><strong>${publicConfigurationPublished ? "Authority verified" : "Publish the app configuration"}</strong><small>${publicConfigurationPublished ? "The signed public authority chain currently names this profile as Patient Zero." : "Copy the immutable genesis configuration into Polymai, then publish the app."}</small></div></li>
      <li><span>3</span><div><strong>Keep this browser profile</strong><small>It holds the private Patient Zero signing key. The exported files do not.</small></div></li>
    </ol>
    <div class="patient-zero-actions"><button class="button" type="button" data-download-patient-zero>Download public setup</button><button class="button button--ghost" type="button" data-copy-patient-zero-config>${publicConfigurationPublished ? "Copy configuration again" : "Copy app configuration"}</button><button class="text-button" type="button" data-disable-patient-zero>Hide operator tools on this device</button></div>
    <details class="settings-technical"><summary>Authority and recovery</summary><div class="anchor-ledger"><span>Authority chain<strong>v${Number(authority.sequence || 0)}</strong></span><span>Offline recovery<strong>${authority.recoveryPublicKey ? "Ready" : "Not created"}</strong></span><span>Supabase stores<strong>Public chain only</strong></span></div><div class="patient-zero-actions"><button class="button button--ghost" type="button" data-rotate-patient-zero>Rotate public key</button><button class="button button--ghost" type="button" data-create-patient-zero-recovery>Create recovery file</button><label class="button button--ghost">Recover Mainframe<input hidden type="file" accept="application/json,.json" data-patient-zero-recovery-file></label></div><p>Rotation is signed by the current private authority and the new private authority. Neither private key is uploaded.</p></details>
    <details class="settings-technical"><summary>Connection status</summary><div class="anchor-ledger"><span>App configuration<strong>${publicationLabel}</strong></span><span>First-contact listener<strong>${listenerLabel}</strong></span><span>Export<strong>Public keys only</strong></span></div>${firstContact.registrationError ? `<p class="form-status form-status--error">${esc(firstContact.registrationError)}</p>` : ""}</details>
    <p class="form-status" data-patient-zero-status role="status"></p>
  </section>` : "";
  ...
  <div class="backbone-section-stack">
...
export function bindAnchorView(actions) {
  ...
  document.querySelector("[data-download-patient-zero]")?.addEventListener("click", async (event) => {
    const status = document.querySelector("[data-patient-zero-status]");
    event.currentTarget.disabled = true;
    if (status) status.textContent = "Preparing public setup…";
    try { await actions.downloadPatientZeroSetup(); if (status) status.textContent = "Public setup downloaded. No private key is included."; }
    catch (error) { if (status) status.textContent = error.message; }
    finally { if (event.currentTarget.isConnected) event.currentTarget.disabled = false; }
  });
  document.querySelector("[data-copy-patient-zero-config]")?.addEventListener("click", async (event) => {
    const status = document.querySelector("[data-patient-zero-status]");
    event.currentTarget.disabled = true;
    if (status) status.textContent = "Preparing app configuration...";
    try { await actions.copyPatientZeroConfig(); if (status) status.textContent = "App configuration copied. Paste it into Polymai and publish."; }
    catch (error) { if (status) status.textContent = error.message; }
    finally { if (event.currentTarget.isConnected) event.currentTarget.disabled = false; }
  });
  document.querySelector("[data-disable-patient-zero]")?.addEventListener("click", () => actions.disablePatientZeroOperator());
  document.querySelector("[data-rotate-patient-zero]")?.addEventListener("click", async (event) => {
    const status = document.querySelector("[data-patient-zero-status]"); event.currentTarget.disabled = true;
    try { await actions.rotatePatientZeroAuthority(); if (status) status.textContent = "Public authority rotated. Existing clients verify the signed chain automatically."; }
    catch (error) { if (status) status.textContent = error.message; }
    finally { if (event.currentTarget.isConnected) event.currentTarget.disabled = false; }
  });
  document.querySelector("[data-create-patient-zero-recovery]")?.addEventListener("click", async (event) => {
    const status = document.querySelector("[data-patient-zero-status]"); event.currentTarget.disabled = true;
    try { await actions.createPatientZeroRecovery(); if (status) status.textContent = "Offline recovery file downloaded. Store it away from this computer."; }
    catch (error) { if (status) status.textContent = error.message; }
    finally { if (event.currentTarget.isConnected) event.currentTarget.disabled = false; }
  });
  document.querySelector("[data-patient-zero-recovery-file]")?.addEventListener("change", async (event) => {
    const status = document.querySelector("[data-patient-zero-status]");
    try { await actions.recoverPatientZeroAuthority(event.currentTarget.files?.[0]); if (status) status.textContent = "Mainframe authority recovered and rotated to this profile."; }
    catch (error) { if (status) status.textContent = error.message; }
  ...
  document.querySelector("[data-backbone-host-list]")?.addEventListener("click", async (event) => {
    ...
    const connect = event.target.closest("[data-backbone-connect]");
    const redownload = event.target.closest("[data-backbone-redownload]");
    const remove = event.target.closest("[data-backbone-remove]");
    if (connect) {
      const launch = backboneLaunches().find((item) => item.hostId === connect.dataset.backboneConnect);
      if (!launch) return;
      connect.disabled = true;
      connect.textContent = "Connecting…";
      backbonePortScanAt = 0;
      backboneHostCache.delete(launch.hostId);
      try {
        const host = await locateLocalBackbone(launch, { interactive: true });
        backboneHostCache.set(launch.hostId, host);
      } catch (error) {
        backboneHostCache.set(launch.hostId, { launch, error });
      }
      const target = document.querySelector("[data-backbone-host-list]");
      if (target) target.innerHTML = cachedHostMarkup(backboneLaunches());
      return;
    }
    if (remove) {
      const launch = backboneLaunches().find((item) => item.hostId === remove.dataset.backboneRemove);
      const cached = launch ? backboneHostCache.get(launch.hostId)?.status : null;
      removeBackboneLaunch(remove.dataset.backboneRemove);
      const target = document.querySelector("[data-backbone-host-list]");
      const remaining = backboneLaunches();
      if (target) target.innerHTML = remaining.length ? cachedHostMarkup(remaining) : `<p class="backbone-manager-empty">No dedicated servers started from this browser yet.</p>`;
      if (launch && remove.dataset.port) {
        const port = Number(remove.dataset.port);
        localBackboneRequest(port, "/shutdown", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }, 900).catch(() => Promise.all((cached?.instances || []).map((instance) => localBackboneRequest(port, `/instances/${encodeURIComponent(instance.id)}/stop`, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }, 650).catch(() => {}))));
      }
      return;
    }
