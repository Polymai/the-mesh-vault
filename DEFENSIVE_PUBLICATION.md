...
Either form deterministically reconstructs the same cryptographic identity and symmetric hierarchy inside the new browser without fetching secret key material. That local identity restoration must not be confused with remote vault discovery or file recovery. A completely new local profile uses Supabase only to reach the exact pinned Patient Zero node; it then receives current peer context through the authenticated mesh and cannot reopen Supabase for normal operation.
...
Entering the 24-word Mesh Key reconstructs the same identity and symmetric hierarchy locally. No secret key material is fetched from Supabase or Patient Zero. Network discovery is separate: a new local profile may use the bounded first-contact gate, while a profile that has already completed bootstrap uses only its cached or connected mesh routes.
...
File ingestion, catalogs, manifests, placements, repair and deletion do not use Supabase in this release. The Android native worker follows the same one-time first-contact rule as the browser client.

The architecture therefore distinguishes the one-time bootstrap dependency from continuing mesh operation. Public rendezvous relays and a permanent Supabase compatibility fallback are disabled.
...
- no ordinary Supabase heartbeat after first-contact completion;
