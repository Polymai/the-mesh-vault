...
export async function loadVaultCatalog({ includeSupabase = true } = {}) {
  const owner = configuredIdentity();
  const candidates = [];
  const local = await getLocalVaultIndex(owner.vaultId);
  if (local) candidates.push(local);

  const anchorRows = await queryAnchors("vault-index", owner.vaultId, 10).catch(() => []);
...
export async function publishVaultCatalog(payload, { forceSupabase = false } = {}) {
  ...
  const current = await loadVaultCatalog({ includeSupabase: false }).catch(() => null);
...
async function mutateCatalog(mutator) {
  const run = async () => {
    const current = await loadVaultCatalog().catch(() => null);
...
export async function loadReplicatedOperations() {
  ...
  if (isProviderUp("supabase")) {
    ...
    incoming.push(...(data || []).map((row) => row.operation).filter(Boolean));
  }
  return ingestOperations(owner.vaultId, incoming, { ownerPublicKey: owner.ownerPublicKey });
}
