...
- automatic bounded control participation by every online node, stable opt-in Backbones and pseudonymous Supabase fallback data; and
- the difference between restoring vault authority and recovering file bytes from enough reachable shards.

The process diagrams explicitly show the current v3 primitives and boundaries: browser-profile IndexedDB versus temporary browser memory, HKDF-SHA-256 derivation, AES-256-GCM segment protection, Ed25519 manifest signing, SHA-256/HMAC integrity roles, Reed-Solomon redundancy, peer shard storage and the narrower mesh/Backbone/Supabase control plane.
