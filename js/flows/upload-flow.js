...
import { chooseErasureProfile, independentCandidateCount, loadPlacementCandidates, findReusableSegment, getContentSegment, createContentSegment, removeContentSegment, updateSegmentPhysicalBytes, markSegmentReady, registerSegmentShards, recordShardPlacement, transitionShardPlacement, linkVersionSegment, queueShardRepair, summarizeVersionSegments } from "../services/segment-service.js";
...
async function retainFailedSegmentCleanup(vaultId, segmentId, node, peers) {
  const segment = await getContentSegment(vaultId, segmentId);
  if (!segment) return;
  ...
  await removeContentSegment(vaultId, segmentId);
}
...
async function responsiveUploadPeers(peers, signal) {
  const timeoutMs = Math.max(500, Number(window.__DATA__?.transport?.uploadPeerProbeTimeoutMs) || 2500);
  const checked = await Promise.all((peers || []).map(async (peer) => {
    if (!peer?.protocol?.sendShard) return null;
    if (typeof peer.protocol.measureRtt !== "function") return peer;
    try {
      const rtt = await peer.protocol.measureRtt(timeoutMs);
      if (signal?.aborted) throw signal.reason || new DOMException("Operation cancelled", "AbortError");
      return rtt == null ? null : peer;
    } catch (error) {
...
export async function uploadFile(file, { vaultIdentity, folderId = null, resilienceClass = window.__DATA__?.resilience?.defaultErasureClass || "auto" } = {}) {
  ...
  const task = createTask(`Securing ${file.name}`, 6 * 60 * 60 * 1000, { kind: "upload", fileName: file.name, fileSize: file.size, phase: "preparing", segmentIndex: 1, totalSegments, preventUnload: true }); let record = null; let buildingSegmentId = null; let cleanupPeers = [];
  try {
    ...
    if (!node || node.status !== "online") throw new Error("Enable this device before uploading a file.");
    const peers = connectedProtocols();
    cleanupPeers = peers;
    task.phase("preparing", peers.length ? "Checking connected storage nodes" : "Preparing local encrypted storage", { progress: .03 });
    const responsivePeers = await responsiveUploadPeers(peers, task.signal);
    const candidates = await loadPlacementCandidates(node, responsivePeers);
    const profile = chooseErasureProfile(independentCandidateCount(candidates), resilienceClass, { segmentCount: totalSegments, candidates });
    ...
    for await (const input of streamFileSegments(file, segmentTarget, reportRead)) {
      ...
      if (segment) {
        ...
        const maximumRemoteCandidates = Math.max(1, Number(transferConfig.uploadCandidateAttempts) || 2);
        ...
        await runConcurrent(shardRows, uploadConcurrency, async (shard, index, runSignal) => {
          ...
          while (!stored) {
            ...
            if (!candidate) break;
            if (!candidate.isLocal && remoteAttempts >= maximumRemoteCandidates) break;
            attemptedCandidateIds.add(candidate.id);
            if (!candidate.isLocal) remoteAttempts += 1;
            ...
            try {
              ...
              await withRetry(async (_attempt, attemptSignal) => {
                ...
                retries: Math.max(0, Number(transferConfig.uploadRetryAttempts ?? 0) || 0),
                baseDelayMs: Number(transferConfig.retryBaseDelayMs) || 500,
                timeoutMs: Number(transferConfig.uploadAttemptTimeoutMs) || 30000,
                signal: runSignal,
              });
              ...
              reportPlacementProgress(index, .03, "A storage node did not confirm; trying another verified location");
            }
          }
          if (!stored) {
            reportPlacementProgress(index, .05, "Saving an encrypted piece locally; mesh distribution will continue later");
      ...
      processed += plain.byteLength; task.progress(Math.min(.94, processed / Math.max(1, file.size) * .94), `Segment ${segmentNumber} verified`);
    }
    ...
    task.done(resilience.recoveryStatus === "resilient" ? "File resilient" : "File stored; waiting for independent devices");
