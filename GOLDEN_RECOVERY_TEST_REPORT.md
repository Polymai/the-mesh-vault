...
Date: 2026-08-05  
Current result: **V3 BACKEND PROVISIONED; LIVE GOLDEN RECOVERY NOT YET RUN**
...
Protocol v3 intentionally removes recovery envelopes and wrapped segment keys. A 24-word Mesh Key now derives the owner identity and all vault key roots locally. Existing pre-v3 app data was deliberately removed by the one-time pre-release `local-root-key-v3` schema reset, so the new proof must start with a new v3 vault and new v3 file.
...
1. Confirm the deployed origin reports protocol v3 and the provisioned schema contains no legacy recovery/key-envelope objects.
...
| Evidence | V3 status |
|---|---:|
| Protocol-v3 Mesh Key portability | PASS in the full configured-backend browser run for phrase and file restore |
| Live v3 Supabase deployment/reset | PASS; operator-confirmed provisioning on 2026-08-05 |
