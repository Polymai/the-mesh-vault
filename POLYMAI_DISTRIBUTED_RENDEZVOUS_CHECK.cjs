...
(async () => {
  ...
  expect(/patient-zero-setup/.test(anchorView) && /const rendezvousMarkup = operatorSetup \?/.test(anchorView), "Patient Zero setup controls must remain hidden outside explicit operator mode.");
  ...
  expect(/themeshvault-shell-v38/.test(worker) && /rendezvousConfig\.js/.test(worker) && /distributed-rendezvous-provider\.js/.test(worker), "The installable app shell must cache the distributed rendezvous runtime.");
