...
export function readSegmentErasureProfile(segment = {}) {
  ...
  const dataShards = Number(explicit.dataShards ?? segment.dataShards ?? segment.requiredShards ?? 4);
  const statedTotal = Number(explicit.totalShards ?? segment.totalShards);
  const parityShards = Number(explicit.parityShards ?? segment.parityShards ?? (Number.isInteger(statedTotal) ? statedTotal - dataShards : 2));
  const requiredShards = Number(explicit.requiredShards ?? segment.requiredShards ?? dataShards);
  const totalShards = Number.isInteger(statedTotal) ? statedTotal : dataShards + parityShards;
  const repairThreshold = Number(segment.repairThreshold ?? explicit.repairThreshold ?? Math.min(totalShards, requiredShards + Math.ceil(parityShards / 2)));
  return {
    algorithm: explicit.algorithm || "reed-solomon",
    profileVersion: Number(explicit.profileVersion ?? segment.codingProfileVersion ?? 1),
    dataShards,
    parityShards,
    requiredShards,
    repairThreshold,
...
export async function verifyManifestV2(envelope, opts = {}) {
  ...
  for (const segment of core.segments) {
    ...
    if (!Array.isArray(segment.shardHashes) || segment.shardHashes.length !== profile.totalShards) return { valid: false, reason: "invalid_shard_hashes" };
    if (
    ...
    ) return { valid: false, reason: "invalid_erasure_profile" };
  }
