...
export async function buildGossipMessage(signer, kind, payload, { ttlHops = DEFAULT_TTL_HOPS, expiresInMs = DEFAULT_EXPIRY_MS } = {}) {
  ...
  // ttlHops is the signed maximum. Forwarders add a separate hopsRemaining
  // transport field, leaving direct messages compatible with older peers.
...
export async function acceptGossipMessage(message, peerKey) {
  ...
  const hopsRemaining = message.hopsRemaining ?? message.ttlHops;
  if (!Number.isInteger(hopsRemaining) || hopsRemaining < 0 || hopsRemaining > message.ttlHops) return { accepted: false, reason: "invalid_hop_count" };
  ...
  try {
    ...
    const { signature, hopsRemaining: _hopsRemaining, ...core } = message;
...
export function isForwardable(message) { return (message.hopsRemaining ?? message.ttlHops) > 1 && Date.now() < message.expiresAt; }
export function withDecrementedHop(message) { return { ...message, hopsRemaining: (message.hopsRemaining ?? message.ttlHops) - 1 }; }
