...
The current release uses Supabase Auth, Postgres, Realtime and an Edge Function as its hosted first-contact, persistent coordination and immediate fallback backbone. After three healthy Anchors have remained directly connected for two minutes, current clients prefer the Anchor path for discovery, signaling relay and bounded control replication. Ordinary Anchor-primary clients then reduce hosted node-presence writes to a ten-minute cadence, while signed operations and the encrypted vault index are periodically backed up. This reduces steady-state fan-out and writes but does not remove the persistent Supabase responsibilities listed below.
...
- bounded first-contact discovery and transient WebRTC signaling fallback through app-owned Realtime Broadcast recipient topics protected by random 256-bit tokens;
...
An already-open vault may use cached records and already-established peer or Anchor connections for a bounded subset of control and transfer operations during a Supabase interruption. Local catalog edits can queue and later replay idempotently. This is neither general offline operation nor a guarantee that recovery, discovery or repair can finish during the outage.

Entering the 24-word Mesh Key reconstructs identity and keys locally; it does not query Supabase for secret material. A completely new browser still needs the hosted backbone or existing reachable network context to discover current manifests, peers and placements. Connected Anchors do not yet form a complete cold-start bootstrap. New file ingestion, durable segment/placement writes and the atomic repair-job mutation remain Supabase-backed.
...
- approximately 90-second Supabase reporting for Anchors and ten-minute Supabase reporting for ordinary Anchor-primary clients, with jitter;
- 30-day manifest and operation-log retention;
- 120-second peer presence; and
- 5-minute fragment-location announcements.

An Anchor control cache receives public keys and bounded signed or encrypted coordination objects. It does not receive the root, Mesh Key, private or segment keys, readable file, or encrypted shard payload bytes, and it gains no authority over another vault. A browser that separately opts into the storage role may hold encrypted shards under that separate role and quota; the Anchor control cache itself does not.
...
Anchor selection uses deterministic weighted rendezvous ranking with lease health, failure-domain diversity, reliability, bounded cache headroom and reported connection load. Three stable connected Anchors place an already-connected client in Anchor-primary mode; two use hybrid mode; fewer than two immediately restore Supabase-primary discovery and opaque recipient signaling. If Supabase is unavailable, connected Anchors may preserve a degraded path for clients that already know and can reach them, but they do not bootstrap a wholly new browser from the Mesh Key alone.

The current control repository maintains an encrypted owner-signed catalog snapshot and an append-only signed local operation queue. It resolves reads in local, connected-Anchor and Supabase-fallback order. Operation IDs make replay idempotent, and Anchor and Supabase receipts are tracked separately so temporary loss of either path does not silently discard a catalog operation.
...
- Supabase and Anchor control caches receive no root, private keys or encrypted shard payload bytes through their coordination roles.
...
- The 24-word phrase restores cryptographic identity locally, but a wholly new browser is not guaranteed to discover remote vault state through Anchors alone; Supabase remains the current bootstrap, backbone and immediate fallback.
