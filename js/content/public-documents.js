...
const architecture = {
  ...
  sections: [
    ...
    },
    {
      ...
      <details class="architecture-timers"><summary>How the installed app refreshes mesh state</summary><p>When an installed PWA or ordinary tab becomes visible, the current client resumes its node heartbeat and reuses recently discovered peers. While Live Mesh remains visible, discovery is refreshed at most every two minutes and the bounded network overview every five minutes. Concurrent requests share one result, and an available Anchor is tried before the hosted backbone. Windows from the same browser profile share discovered-peer metadata, but each open window owns its own direct WebRTC connections and may therefore show a different direct-link count. This foreground refresh does not keep a suspended mobile browser alive.</p></details>`,
    },
    {
      id: "data-locations",
    ...
    },
    {
      ...
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-outage"><span><b>Three stable Anchors</b>An already-connected client can prefer Anchor relay and control replication. Ordinary clients write node presence to Supabase at a reduced ten-minute cadence while signed operations are periodically backed up.</span><span><b>One or two Anchors</b>The client uses the available Anchor first and keeps Supabase as the live fallback for discovery and opaque recipient signaling. With three Anchors it measures stability before reducing backbone traffic further.</span><span><b>Current hard limits</b>The 24 words reconstruct identity locally, but a completely new browser still needs the hosted backbone or existing reachable mesh context to discover manifests, peers and placements. New file ingestion, durable segment/placement records and the atomic repair job remain Supabase-backed.</span></div>
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: `<p>If you run a storage, Night or Anchor node, you offer capacity directly to the peer network and authorise the protocol to use up to the contribution limit you select for encrypted shards and bounded control data. You can pause the node or clear its local storage. The optional Android app can continue this role through a user-started foreground service with a persistent notification and may restart it after boot; Android can still stop or restrict the service. Availability, battery use, data use and local storage behaviour depend on your device and network.</p>
...
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>This data may remain until you delete it, clear site data, remove the browser profile or the browser/operating system evicts it. Normal storage and coordination do not send the root, 24-word Mesh Key, readable file contents, owner private key or usable derived symmetric keys to Supabase, Anchors or storage nodes. Protocol v3 has no hosted recovery envelope or wrapped segment-key store. If you deliberately create a share link, Supabase stores a one-file key grant encrypted for the link recipient; only the bearer secret in the URL fragment can open it. A phrase or complete v3 Mesh Key file reconstructs the identity locally and sends no secret-key lookup; network coordination is still needed to discover remote metadata and enough valid shards.</p>`,
    },
    {
      ...
      <p>The native service creates a separate random device identity. Its private identity seed and anonymous Supabase refresh token are encrypted locally with an Android Keystore key. The service also stores public configuration such as Vault ID, public node label, capacity limit, coarse location, Anchor role and Supabase endpoint. It does not accept the Mesh Key, root seed, owner private key, file content or usable derived file keys.</p>
