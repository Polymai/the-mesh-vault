...
export async function moveFileRecord(vaultId, fileId, folderId = null) {
  const { data, error } = await appTable("files").update({ folder_id: folderId, updated_at: new Date().toISOString() }).eq("vault_id", vaultId).eq("id", fileId).select().single();
  if (error) throw error;
  const file = await cachedFile(fileId);
  if (file) await cacheFile({ ...file, folder_id: folderId, updated_at: data.updated_at });
  return data;
}
