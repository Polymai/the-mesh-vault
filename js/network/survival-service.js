...
async function edgeAction(action, payload = {}) {
  ...
  const response = await supabaseFetch(`${window.__DATA__.functionsBaseUrl}/app717-meshvault-api`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ action, clientProtocolVersion: 3, ...payload }),
  });
...
async function probeWakeAssistance(registration) {
  if (!registration || !("PushManager" in window) || !("Notification" in window)) return setWakeAssistance("unsupported");
  try {
    const config = await edgeAction("push-config");
    if (!config.publicKey) return setWakeAssistance("setup needed");
    if (Notification.permission === "denied") return setWakeAssistance("blocked");
    const existing = await registration.pushManager.getSubscription();
    if (!existing) return setWakeAssistance("available");
    await edgeAction("register-push", { nodeId: runtime.nodeId, pushEndpoint: existing.toJSON() });
    return setWakeAssistance("ready");
  } catch (error) {
    return reportWakeFailure(error, "push-config");
  }
}
export async function configureWakeAssistance() {
  ...
  if (!("PushManager" in window) || !("Notification" in window)) return setWakeAssistance("unsupported");
  let config;
  try { config = await edgeAction("push-config"); }
  catch (error) { return reportWakeFailure(error, "push-config"); }
  if (!config.publicKey) return setWakeAssistance("setup needed", { reason: "network-setup", message: "The network has not configured Node reminders yet." });
  const permission = Notification.permission === "default" ? await Notification.requestPermission() : Notification.permission;
  if (permission !== "granted") return setWakeAssistance(permission === "denied" ? "blocked" : "permission needed");
  let pushEndpoint;
  try {
    pushEndpoint = await registration.pushManager.subscribe({
      userVisibleOnly: true,
      applicationServerKey: urlBase64ToUint8Array(config.publicKey),
    });
  } catch (error) {
    return reportWakeFailure(error, "push-subscribe");
  }
  try {
    await edgeAction("register-push", { nodeId: runtime.nodeId, pushEndpoint: pushEndpoint.toJSON() });
    return setWakeAssistance("ready");
  } catch (error) {
    return reportWakeFailure(error, "backend-register");
  }
}
export async function disableWakeAssistance() {
  ...
  if (runtime?.nodeId) await edgeAction("unregister-push", { nodeId: runtime.nodeId }).catch(() => {});
