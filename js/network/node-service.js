...
async function reclaimSupabaseNodeRegistration(nodeId, authUserId) {
  ...
  const { data, error } = await supabase.functions.invoke("app717-meshvault-api", {
    body: { action: "reclaim-node-registration", statement, signature, clientProtocolVersion: 3 },
  });
  if (error || !data?.reclaimed) {
    throw error || new Error("The existing node registration could not be reclaimed.");
  }
}
