...
const legacy = readSegmentErasureProfile({ shardHashes: Array.from({ length: 6 }, (_, index) => `legacy-${index}`) });
assert.deepEqual(
  (({ dataShards, parityShards, requiredShards, totalShards, profileVersion }) => ({ dataShards, parityShards, requiredShards, totalShards, profileVersion }))(legacy),
  { dataShards: 4, parityShards: 2, requiredShards: 4, totalShards: 6, profileVersion: 1 },
);
...
console.log("PASS legacy manifests default to 4+2 without changing stored data");
