...
import { bindPwaInstallPrompt, renderPwaInstallPrompt } from "./pwa-install-prompt.js";

const navItems = [
  ["dashboard", "Overview"], ["drive", "My Drive"], ["mesh", "Live Mesh"], ["recovery", "Mesh Key"], ["settings", "Settings"],
];
const routeLabels = Object.freeze({ "backbone-server": "Server setup" });
let stopPublicHeaderSync = () => {};
const esc = (value) => String(value ?? "").replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[character]));
const releaseLabel = () => esc(globalThis.window?.__DATA__?.release?.label || "development");
const renderReleaseBadge = (modifier = "") => `<span class="app-version${modifier ? ` app-version--${modifier}` : ""}" title="Running TheMeshVault ${releaseLabel()}">${releaseLabel()}</span>`;
...
function vaultLabel(state) {
  if (state.identity) return esc(state.vaultProfile?.name || `Vault ${state.identity.ownerPublicKey.slice(0, 10)}…`);
...
const downloadPhaseLabels = { finding: "Finding recovery data", choosing: "Choosing destination", retrieving: "Downloading shards", reconstructing: "Rebuilding segment", decrypting: "Decrypting locally", verifying: "Verifying original", saving: "Saving file", complete: "Recovery verified", cancelled: "Download cancelled", failed: "Recovery stopped" };
const downloadStages = [
  [["finding", "choosing"], "Find"],
  [["retrieving"], "Fetch"],
  [["reconstructing"], "Rebuild"],
  [["decrypting"], "Decrypt"],
...
function renderDownloadTask(state) {
  const downloads = Object.values(state.activeTasks).filter((task) => task.kind === "download");
  if (!downloads.length) return "";
  const task = downloads.at(-1);
  const percent = Math.round(Number(task.progress || 0) * 100);
  const currentStage = downloadStages.findIndex(([phases]) => phases.includes(task.phase));
  const finalizing = task.phase === "complete";
  const stages = downloadStages.map(([phases, label], index) => `<li class="${phases.includes(task.phase) ? "is-active" : (currentStage > index || finalizing ? "is-done" : "")}"><i aria-hidden="true"></i><span>${label}</span></li>`).join("");
  const segment = task.totalSegments ? `Segment ${Math.min(task.segmentIndex || 1, task.totalSegments)} of ${task.totalSegments}` : "Preparing recovery";
  const actions = task.status === "running"
    ? `<button type="button" data-cancel-task="${task.id}" data-task-kind="download">Cancel download</button>`
    : `<button type="button" data-dismiss-task="${task.id}">Close</button>`;
  const footer = task.status === "running" ? "Keep this tab open while the file is recovered and checked." : escapeTaskText(task.message);
  return `<aside class="upload-status download-status upload-status--${task.status}" role="status" aria-live="polite" aria-label="Download progress" data-download-status>
    <div class="upload-status__head"><div><span>${escapeTaskText(downloadPhaseLabels[task.phase] || "Recovering file")}</span><strong>${escapeTaskText(task.fileName || task.label)}</strong></div><b>${percent}%</b></div>
    <progress max="1" value="${Number(task.progress || 0)}">${percent}%</progress>
    <div class="upload-status__meta"><span>${escapeTaskText(segment)}</span><span>${escapeTaskText(task.message || "Starting recovery")}</span></div>
    <ol class="upload-stages download-stages">${stages}</ol>
    <div class="upload-status__foot"><span>${footer}</span>${actions}</div>
  </aside>`;
}
function renderFileTasks(state) {
  const tasks = `${renderUploadTask(state)}${renderDownloadTask(state)}`;
  return tasks ? `<div class="file-task-stack">${tasks}</div>` : "";
}
export function renderShell(state, content) {
  ...
  if (publicRoute) {
    ...
    const howLink = state.route === "home" ? 'href="#/home" data-scroll-target="mesh-in-one-look"' : 'href="#/security"';
    const publicLinks = `<a class="nav-link${state.route === "security" ? " is-active" : ""}" ${howLink}>How it works</a><a class="nav-link${state.route === "terms" ? " is-active" : ""}" href="#/terms">Terms</a><a class="nav-link${state.route === "privacy" ? " is-active" : ""}" href="#/privacy">Privacy</a>`;
    return `<div class="site-shell"><header class="public-header" data-public-header><nav class="public-nav" aria-label="Public navigation"><a class="button button--ghost button--small" ${howLink}>How it works</a></nav><div class="brand-scoop" aria-hidden="true"></div>${renderBrandWordmark()}<button type="button" class="public-menu-toggle" data-public-menu-toggle aria-label="Open navigation" aria-expanded="false" aria-controls="public-sidebar">Menu</button><div class="public-actions">${action}</div></header><div class="drawer-backdrop" data-public-menu-backdrop></div><aside id="public-sidebar" class="public-sidebar" data-public-menu aria-hidden="true"><div class="sidebar-head">${renderBrandWordmark()}<button type="button" class="drawer-close" data-public-menu-close aria-label="Close navigation">Close</button></div><nav class="side-nav" aria-label="Public navigation">${publicLinks}</nav><div class="public-sidebar-foot">${action}</div></aside>${notice}<main id="route-view">${content}</main><footer class="public-footer"><span>${BRAND_NAME}</span><nav aria-label="Legal and technical information"><a href="#/security">Security & Architecture</a><a href="#/terms">Terms</a><a href="#/privacy">Privacy</a></nav><span>No account required. Your Mesh Key controls access.</span></footer>${renderPwaInstallPrompt(state)}</div>`;
  }
  if (state.route === "night-node") return `<main id="route-view">${notice}${content}</main>`;
  const items = state.identity ? navItems : navItems.filter(([id]) => id === "settings");
  const banner = shouldShowMeshKeyBanner(state) ? renderMeshKeyBanner() : "";
  const currentRouteLabel = navItems.find(([id]) => id === state.route)?.[1] || routeLabels[state.route] || BRAND_NAME;
  return `<div class="app-shell${state.mobileNavOpen ? " nav-open" : ""}"><header class="mobile-topbar"><button class="menu-toggle" type="button" aria-label="Open navigation" aria-expanded="${state.mobileNavOpen}" aria-controls="app-sidebar" data-toggle-nav>Menu</button>${renderBrandWordmark({ href: "#/dashboard" })}<div class="mobile-node-state">${renderReleaseBadge("mobile")}<span class="node-dot node-dot--${state.node.status}" title="Node ${state.node.status}"></span></div></header><div class="drawer-backdrop" data-close-nav></div><aside id="app-sidebar" class="sidebar"><div class="sidebar-head">${renderBrandWordmark({ href: "#/dashboard" })}<button type="button" class="drawer-close" data-close-nav aria-label="Close navigation">Close</button></div><div class="node-summary"><span class="node-dot node-dot--${state.node.status}"></span><div><strong>${esc(state.node.label || "This device")}</strong><span>${state.node.status === "online" ? "Helping the mesh" : state.node.status}</span></div></div><nav class="side-nav" aria-label="Main navigation">${items.map(([id, label]) => routeLink(id, label, state.route)).join("")}</nav><div class="sidebar-foot"><span class="account-email">${vaultLabel(state)}</span>${state.identity ? `<span class="mesh-key-indicator">${state.identity.meshKeySaved ? "Mesh Key saved" : "Mesh Key not saved"}</span>` : ""}<nav class="sidebar-legal" aria-label="Legal and security"><a href="#/security">Security</a><a href="#/terms">Terms</a><a href="#/privacy">Privacy</a></nav></div></aside><main class="app-main"><div class="app-topline"><div><span class="eyebrow">Private workspace</span><strong>${currentRouteLabel}</strong></div><div class="app-topline-status">${renderReleaseBadge()}<span class="key-state">${state.identity ? "No-account vault" : "Storage node only"}</span></div></div>${notice}${banner}<div id="route-view" class="route-view">${content}</div></main>${renderFileTasks(state)}${renderPwaInstallPrompt(state)}</div>`;
}
export function bindShell({ onToggleNav, onCloseNav, onDismiss, onScrollTarget, onInstallApp }) {
  bindPublicHeaderScroll();
  bindPublicMenu();
  bindMeshKeyBanner();
  bindPwaInstallPrompt(onInstallApp);
  ...
  document.querySelector("[data-cancel-task]")?.addEventListener("click", (event) => {
    if (event.currentTarget.dataset.taskKind === "download") cancelTask(event.currentTarget.dataset.cancelTask);
    else if (window.confirm("Cancel this upload? The incomplete encrypted data will be cleaned up.")) cancelTask(event.currentTarget.dataset.cancelTask);
  });
