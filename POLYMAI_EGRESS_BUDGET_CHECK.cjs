...
// window. Once a verified mesh route exists, its persisted completion marker
// makes scheduled Supabase traffic permanently zero for that profile.
const oldRequestsPerNodeHour = 240 + (60 * 4) + 120 + 40;
const verifiedMeshIdleRequestsPerNodeHour = 0;
const automaticColdStartWindowsPerProfile = 1;
...
assert.equal(automaticColdStartWindowsPerProfile, 1, "an ordinary profile gets exactly one bounded first-contact phase");
...
console.log(`PASS egress budget: local-only check; verified-mesh idle Supabase requests ${verifiedMeshIdleRequestsPerNodeHour}/hour, first-contact windows ${automaticColdStartWindowsPerProfile}/profile, former timer model reduced ${(hybridReduction * 100).toFixed(1)}%, initial operation payload -${(operationInitialPayloadReduction * 100).toFixed(1)}%. No network requests executed.`);
