...
The current Android native service follows the same one-time bootstrap contract as the browser client. A completely new native profile creates a temporary anonymous Supabase session and node row, discovers only the deployment-pinned Patient Zero identity and uses recipient-scoped Realtime signaling for the first WebRTC connection. After the peer proves that pinned identity, the worker marks bootstrap complete, deletes the temporary row best-effort, erases the temporary credential and closes the hosted path. Direct WebRTC and authenticated mesh peers carry subsequent shard and control traffic.

The browser/WebView path uses the same permanent boundary. A new local profile can use Supabase for its bounded introduction to Patient Zero; a completed profile uses cached peers, connected participants and invites and cannot reopen Supabase for discovery, files, placements, repair or Live Mesh. Patient Zero is the sole intentional hosted exception and keeps only the minimal listener and sparse heartbeat required for future first contacts.
