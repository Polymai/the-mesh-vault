...
export function renderMeshUniverse(nodes, edges, { selectedNodeId, currentNodeId } = {}) {
  ...
  return `<div class="mesh-universe" data-mesh-universe data-transfer-animation="real-shard-pulses-only" data-selected-node="${esc(selectedNodeId)}" data-current-node="${esc(currentNodeId)}">
    <canvas class="mesh-universe__canvas" tabindex="0" role="application" aria-label="3D mesh map. Drag to orbit, pinch or scroll to zoom, and click a node to fly to it."></canvas>
    ...
    <div class="mesh-universe__hint">Drag to orbit <span>&bull;</span> Pinch or scroll to zoom <span>&bull;</span> Click a node to fly</div>
  </div>`;
}
...
export function bindMeshUniverse({ nodes, edges, selectedNodeId, currentNodeId, onSelect }) {
  ...
  function setZoom(value) {
    camera.zoom = Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, Number(value) || 1));
    root.dataset.cameraZoom = camera.zoom.toFixed(3);
  }
  ...
  root.dataset.touchGesture = "drag-and-pinch";
  setZoom(camera.zoom);
  ...
  function draw(time) {
    ...
    if (!pointers.size && !reducedMotion) camera.yaw += elapsed * .000025;
  ...
  canvas.addEventListener("pointerdown", (event) => {
    ...
    try { canvas.setPointerCapture(event.pointerId); } catch { /* Synthetic tests and older WebViews may not expose capture. */ }
    if (pointers.size === 1) {
      drag = { pointerId: event.pointerId, ...point, yaw: camera.yaw, pitch: camera.pitch };
      pinch = null;
      gestureMoved = false;
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    const dx = next.x - drag.x, dy = next.y - drag.y;
    if (Math.abs(dx) + Math.abs(dy) > 4) gestureMoved = true;
    camera.yaw = drag.yaw + dx * .008;
    camera.pitch = Math.max(-1.2, Math.min(1.2, drag.pitch + dy * .008));
  }, { passive: false });
  function finishPointer(event, cancelled = false) {
    if (!pointers.has(event.pointerId)) return;
    const point = pointerPosition(event);
    const wasPinching = !!pinch || pointers.size > 1;
    pointers.delete(event.pointerId);
    try { if (canvas.hasPointerCapture(event.pointerId)) canvas.releasePointerCapture(event.pointerId); } catch { /* Capture is optional. */ }
    if (pointers.size === 1) {
      const [pointerId, remaining] = Array.from(pointers.entries())[0];
      drag = { pointerId, ...remaining, yaw: camera.yaw, pitch: camera.pitch };
      pinch = null;
      gestureMoved = true;
      return;
    }
    drag = null;
    pinch = null;
    if (cancelled || wasPinching || gestureMoved) return;
  ...
  canvas.addEventListener("wheel", (event) => { event.preventDefault(); setZoom(camera.zoom * (event.deltaY > 0 ? .9 : 1.1)); }, { passive: false });
  ...
  root.querySelector("[data-universe-zoom='in']")?.addEventListener("click", () => setZoom(camera.zoom * 1.18));
  root.querySelector("[data-universe-zoom='out']")?.addEventListener("click", () => setZoom(camera.zoom / 1.18));
  root.querySelector("[data-universe-reset]")?.addEventListener("click", () => { selected = currentNodeId; targetFocus = positions.get(currentNodeId) || { x: 0, y: 0, z: 0 }; camera.yaw = -.38; camera.pitch = .22; setZoom(1.15); if (currentNodeId) onSelect(currentNodeId); });
  frame = requestAnimationFrame(draw);
