...
(async () => {
  ...
  const { normalizeSignalEnvelope } = await import(pathToFileURL(path.join(root, "js/network/providers/signal-envelope.js")).href);
  ...
  assert.match(node, /excludeProviders:\s*supabaseDiscoveryPending\s*\?\s*\[\]\s*:\s*\[supabaseBootstrapProvider\.name\]/);
  assert.match(node, /if \(!patientZero && !supabaseAutomaticFallbackEnabled\(\)\) return;/);
  assert.match(node, /markSupabaseBootstrapComplete\(\)/);
  assert.match(node, /clearAnonymousSessionLocally\(\)/);
  ...
  assert.match(schema, /count\(distinct failure_domain_id\)/, "recovery accounting must count physical failure domains, not logical server processes");

  const directSignal = {
    signalId: "signal-direct",
    fromNodeId: "mainframe",
    toNodeId: "phone-a",
    type: "offer",
    payload: { type: "offer", sdp: "v=0" },
  };
  assert.equal(normalizeSignalEnvelope(directSignal), directSignal,
    "a direct Realtime signal must retain its addressed envelope instead of being reduced to SDP");
  const wrappedSignal = { payload: {
    signalId: "signal-wrapped",
    fromNodeId: "phone-b",
    toNodeId: "mainframe",
    type: "answer",
    payload: { type: "answer", sdp: "v=0" },
  } };
  assert.equal(normalizeSignalEnvelope(wrappedSignal), wrappedSignal.payload,
    "a runtime-wrapped Realtime signal must be unwrapped exactly once");
  assert.equal(normalizeSignalEnvelope({ type: "offer", sdp: "v=0" }), null,
    "bare SDP is not an addressed mesh signal");
  ...
  assert.equal(warm.mode, policy.COORDINATION_MODES.ANCHOR_PRIMARY);
  ...
  console.log("PASS Backbone mesh: automatic control participation, pinned WSS identity, one-time Supabase first contact, honest host/disk domains and keyless Android participation. No network requests executed.");
})().catch((error) => {
