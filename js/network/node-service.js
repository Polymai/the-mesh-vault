...
async function handleGossipMessage(_peer, message) {
  if (message.kind === GOSSIP_KINDS.PEER_ANNOUNCE) {
    ...
    // Both recipients learn about each other at effectively the same instant.
    // Let only the lexicographically smaller node create the WebRTC offer so
    // simultaneous offers cannot replace or strand each other's connection.
    const shouldInitiate = String(activeNode?.id || "").localeCompare(String(announcedNodeId || "")) < 0;
    ...
    if (announcementChanged) schedulePeerDirectoryReplay();
