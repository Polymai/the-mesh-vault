...
import { createPatientZeroRecoveryFile, recoverPatientZeroAuthority as recoverPatientZeroAuthorityWithFile, rotatePatientZeroAuthority as rotatePatientZeroAuthorityKey } from "./platform/patient-zero-authority.js";
import {
...
async function startDeviceOnce(deviceIdentity, vaultId) {
  ...
  try {
    ...
    // receives one bounded Supabase first-contact window per disconnected app lifecycle;
...
function startDevice(deviceIdentity, vaultId) {
  const key = `${deviceIdentity?.deviceId || ""}|${deviceIdentity?.devicePublicKey || ""}|${vaultId || "storage-only"}`;
  // PWA/native lifecycle events and route re-entry can overlap during startup.
  // Reuse the same activation instead of registering the same node twice and
  // reopening a second Supabase first-contact window.
  if (deviceStartPromise && deviceStartKey === key) return deviceStartPromise;
  deviceStartKey = key;
  deviceStartPromise = startDeviceOnce(deviceIdentity, vaultId);
  return deviceStartPromise;
}
...
const actions = {
  ...
  settings: {
    ...
    async rotatePatientZeroAuthority() {
      const bundle = await rotatePatientZeroAuthorityKey();
      announce("Patient Zero public authority rotated.", "success", 7000);
      renderApp(true);
      return bundle;
    },
    async createPatientZeroRecovery() {
      const recovery = await createPatientZeroRecoveryFile();
      const url = URL.createObjectURL(new Blob([`${JSON.stringify(recovery, null, 2)}\n`], { type: "application/json" }));
      const link = document.createElement("a"); link.href = url; link.download = "themeshvault-patient-zero-offline-recovery.json"; link.click(); URL.revokeObjectURL(url);
      announce("Patient Zero recovery file downloaded. It contains a private recovery key; keep it offline.", "warning", 10000);
      renderApp(true);
      return recovery;
    },
    async recoverPatientZeroAuthority(file) {
      if (!file) throw new Error("Choose the registered Patient Zero recovery file.");
      const recovery = JSON.parse(await file.text());
      const bundle = await recoverPatientZeroAuthorityWithFile(recovery);
      announce("Patient Zero authority recovered to this browser profile.", "success", 8000);
      renderApp(true);
      return bundle;
    },
    async installApp() {
