...
function routePositions(nodes, currentNodeId, edges, compact) {
  ...
  for (const [level, levelNodes] of levels) {
    if (compact) {
      const y = 95 + (level / maxLevel) * 500;
      const columns = Math.min(7, levelNodes.length);
      levelNodes.forEach((node, index) => {
        const row = Math.floor(index / columns);
        const col = index % columns;
        const rowCount = Math.min(columns, levelNodes.length - row * columns);
        positions.set(node.id, {
          x: 24 + ((col + 1) / (rowCount + 1)) * 312,
          y: y + row * 34,
        });
      });
    } else {
...
function graphStyle(compact = false, mode = "radial") {
  const size = compact
    ? { node: 8, neighbor: 9, current: 12, selected: 13, font: 7.4, edge: .7, direct: 1.15, focused: 1.55 }
  ...
  return [
    {
      ...
      style: {
        ...
        "text-opacity": 0,
        "text-background-color": "#f4fafb",
        "text-background-opacity": .88,
        "text-background-padding": compact ? 1 : 2,
    ...
    { selector: "node.mesh-device.role-visible", style: { "background-color": "#e7f0f2", "border-color": "#8eabb3" } },
    { selector: "node.mesh-device.role-files", style: { "background-color": "#fff3c9", "border-color": "#d19a0c" } },
    { selector: "node.mesh-device.current", style: { width: size.current, height: size.current, "background-color": "#07111f", "border-color": "#35d9e6", "border-width": compact ? 1.8 : 2.3, color: "#07111f" } },
    ...
    { selector: "node.mesh-device.ambient", style: { opacity: 0.5 } },
    { selector: "node.mesh-device.offline", style: { opacity: 0.32 } },
    {
...
export function renderMeshConnections(nodes = [], edges = [], { mode = "radial" } = {}) {
  ...
  return `<div class="mesh-connections mesh-connections--${mode}" data-mesh-connections data-mesh-graph-mode="${mode}" data-link-geometry="${mode === "routes" ? "curved" : mode}" data-transfer-animation="real-shard-pulses-only" aria-busy="true">
    ...
    <div class="mesh-connections__controls" aria-label="${view.label} view controls">
      <button type="button" data-connections-organise aria-label="Arrange ${view.label.toLowerCase()} view" title="Arrange">${organiseIcon}</button>
      <button type="button" data-connections-fit aria-label="Fit all nodes" title="Fit all">${fitIcon}</button>
    </div>
