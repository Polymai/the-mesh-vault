...
- maximum selectable browser contribution: 250 GiB, subject to the storage quota actually granted by the browser; and
...
An already-open vault may use cached records, ordinary control participants and Backbones for a bounded subset of control and transfer operations during a Supabase interruption. Local catalog edits can queue and later replay idempotently. A new browser with a configured, reachable and public-key-pinned WSS Backbone can obtain control-plane first contact without Supabase. This is not general offline operation or proof that every recovery or repair can finish during an outage.

Entering the 24-word Mesh Key reconstructs identity and keys locally; it does not query Supabase for secret material. A completely new browser still needs reachable coordination context. A configured pinned WSS Backbone can provide it; otherwise Supabase is the fallback. New file ingestion, durable segment/placement writes and the atomic repair-job mutation remain Supabase-backed. The current Android native worker also still uses Supabase for its own first contact and signaling.
