...
export function renderAnchorView(state) {
  ...
  const statusLabel = !anchor.enabled ? "Off" : nativeActive ? "Android background" : anchor.status === "paused" ? "Paused in background" : "Running";
  ...
  const assurance = !anchor.enabled
    ...
    : anchor.status === "paused" && !nativeActive
      ? `<strong>Paused in the background.</strong> Return to this tab to resume.`
    : nativeActive
      ? `<strong>Running in the Android app.</strong> The persistent notification keeps this user-enabled node available when the screen is off.`
    : connectedAnchors
  ...
  <section class="panel settings-section anchor-panel">
    <form data-anchor-form>
      ...
      <label class="anchor-switch"><input type="checkbox" name="enabled" ${anchor.enabled ? "checked" : ""}><span><strong>Run as an Anchor</strong><small>${nativeActive ? "Handled by the Android background node." : remindersReady ? "Node reminders on. Keep this tab open." : "Keep this tab open. Node reminders are optional."}</small></span></label>
