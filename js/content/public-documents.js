...
const supabaseDataDisclosure = `<div class="supabase-disclosure" data-supabase-disclosure>
  ...
  <div class="supabase-disclosure__list">
    ...
    <article>
      ...
      <dl>
        <div><dt>Supabase stores</dt><dd>An owner-signed encrypted private vault profile, encrypted names and metadata, file sizes, coding parameters, vault-keyed deduplication fingerprints, shard hashes and placements, recovery state, transfer/repair/deletion records, signed or encrypted manifests, recovery envelopes, share-capability records and audit events.</dd></div>
    ...
    <article>
      ...
      <dl>
        <div><dt>Supabase processes</dt><dd>Sender and recipient node IDs, offers, answers and ICE candidates when the hosted signaling fallback is used. Current clients receive them through a recipient topic protected by a random 256-bit token kept in an app-owned RLS table; short-lived database rows remain only for protocol-version-1 compatibility.</dd></div>
        ...
        <div><dt>What it reveals</dt><dd>ICE data may contain network-address information. Signal rows expire, but normal infrastructure logs can follow separate retention rules.</dd></div>
      </dl>
    </article>
    <article>
    ...
    <article class="is-boundary">
      ...
      <dl>
        <div><dt>Remains on authorised browsers</dt><dd>The Mesh Key phrase, owner private key, unwrapped segment keys and readable file bytes.</dd></div>
        <div><dt>Stored on peer devices instead</dt><dd>Encrypted shard payloads. The current app does not use Supabase Storage for file or shard content.</dd></div>
  ...
  sections: [
    ...
    },
    {
      ...
      body: `<ol>
        ...
        <li><strong>Independent encryption.</strong> Every newly created representation is encrypted with AES-GCM using a new random 256-bit segment key, a unique nonce and authenticated context. The segment key is wrapped by the vault key; private segment metadata is encrypted separately. Reused vault-local segments keep their existing verified ciphertext and key wrapper.</li>
    ...
    },
    {
      ...
      <div class="document-table-wrap"><table><thead><tr><th>Location</th><th>What can be there</th><th>What is not intended to be there</th></tr></thead><tbody>
        <tr><td>Your browser</td><td>Vault and device keys, the Mesh Key phrase, encrypted metadata, your private vault name, manifests, operation log, settings, and any encrypted shards this browser stores. Shards use OPFS when available with IndexedDB metadata/fallback.</td><td>The application does not send the Mesh Key phrase, private owner key, unwrapped segment keys or readable file bytes to Supabase or storage nodes.</td></tr>
        <tr><td>Other browser nodes</td><td>Content-addressed encrypted shards, public node labels, signed deletion orders, and bounded signed control objects when Anchor mode is enabled.</td><td>They should not receive the private vault name, original plaintext file or the key needed to decrypt it.</td></tr>
        <tr><td>Supabase backbone</td><td>Pseudonymous anonymous-session identifiers, vault and device public keys, an owner-signed encrypted private vault profile, public node labels, encrypted file/folder metadata, file sizes, vault-scoped keyed deduplication fingerprints, coding records, shard hashes and placements, node capacity/status/location, short-lived signaling, transfer/repair/deletion state, signed/encrypted manifests, encrypted recovery envelopes, capability records and audit events.</td><td>The private vault name remains ciphertext. The Mesh Key phrase, private owner key, unwrapped segment keys and full readable file are not stored there. This app does not use Supabase Storage for file content.</td></tr>
    ...
    },
    {
      ...
      <figure class="architecture-figure architecture-figure--backbone" aria-labelledby="backbone-title">
        ...
        <div class="backbone-grid">
          ...
          <article class="backbone-card backbone-card--fast"><span>Hosted backbone</span><strong>Supabase</strong><ul><li>Anonymous Auth session and RLS access</li><li>Vault and node registration</li><li>Bounded first-contact discovery</li><li>Opaque recipient signaling fallback</li><li>File/segment/shard metadata and placements</li><li>Transfer, repair, deletion and audit state</li><li>Encrypted manifest and recovery caches</li></ul><b>Operationally important today.</b></article>
          <article class="backbone-card backbone-card--fallback"><span>Distributed active path</span><strong>Connected Anchors and peers</strong><ul><li>Select three primary and two reserve Anchors</li><li>Replicate bounded signed control records</li><li>Relay signals within the known mesh</li><li>Serve cached manifests and locations</li><li>Carry signed repair advisories</li></ul><b>Not yet a complete cold-start replacement. Preferred after three Anchors are stable.</b></article>
        </div>
        <div class="backbone-outage"><span><b>Three stable Anchors</b>Connected clients prefer Anchor discovery, relay and control replication. Ordinary clients stop their hosted Realtime channel but keep a slower metadata heartbeat.</span><span><b>Fewer than two Anchors</b>The client immediately restores Supabase discovery and opaque recipient signaling. With two Anchors it stays hybrid while stability is measured.</span><span><b>Current hard limits</b>Phrase-only cold recovery, first contact, new metadata writes and the atomic repair/deletion job state normally require Supabase.</span></div>
      ...
      <p><strong>Recovery-cache boundary:</strong> a phrase-only restore sends a deterministic lookup hash and retrieves a random salt, nonce and ciphertext. The phrase and decrypted private material are not sent.</p>`,
    },
    {
      id: "identity",
      ...
      <p>The phrase itself is not uploaded. The current app derives a high-entropy lookup hash and an encryption key from it, then stores an encrypted recovery envelope in Supabase and connected Anchor caches. A new browser with only the phrase normally uses Supabase to locate that envelope. A newly downloaded complete Mesh Key file instead carries both the phrase and its encrypted envelope locally, so the file can restore the vault without that lookup; possession of that file therefore grants vault access and it must be protected like the phrase itself. Legacy version-1 files contain only the envelope and must be combined with their matching phrase.</p>
    ...
    },
    {
      ...
      <p>When placements become unavailable, the repair coordinator can recreate missing shards from a recoverable segment and place replacements on eligible nodes. A signed repair advisory is also gossiped and retained by connected Anchors so the request survives a temporary hosted-coordination outage; when Supabase is reachable, its atomic job claim prevents browsers from treating the same repair as independently owned work. Returning surplus copies are reconciled after a grace period and do not increase the logical erasure profile.</p>`,
    },
    {
      id: "deletion",
      title: "Deletion",
...
const terms = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>Anyone with a complete active share link may be able to retrieve the shared file. Current Drive links expire after 24 hours; deleting the file revokes its known links. The protocol supports optional password and download restrictions, but the current Drive interface does not expose those controls.</p>`,
    },
    {
      id: "suspension",
...
const privacy = {
  ...
  sections: [
    ...
    },
    {
      ...
      <p>Supabase is currently used to make the service function, not merely for optional analytics: new uploads, metadata writes, cold peer discovery, phrase-only recovery lookup, signaling fallback and atomic repair/deletion jobs normally depend on it. An already-connected client prefers three stable Anchors for active discovery, relay and bounded control replication. The application does not use Supabase Storage as a central bucket for complete file contents.</p>
