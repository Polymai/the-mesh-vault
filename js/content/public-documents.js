...
const architecture = {
  ...
  sections: [
    ...
    },
    {
      ...
      <details class="architecture-timers"><summary>How the installed app refreshes mesh state</summary><p>When an installed PWA or ordinary tab becomes visible, the current client resumes its node heartbeat, refreshes peer discovery and asks the service worker to check for an updated release. While Live Mesh remains visible, discovery is refreshed every 15 seconds; the wider network overview is reconciled every 60 seconds. Windows from the same browser profile share discovered-peer metadata, but each open window owns its own direct WebRTC connections and may therefore show a different direct-link count. This foreground refresh does not keep a suspended mobile browser alive.</p></details>`,
    },
    {
      id: "data-locations",
